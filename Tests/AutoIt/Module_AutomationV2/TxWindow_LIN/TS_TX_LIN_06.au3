...
;==========================================================================Test Procedure =========================================================================
ConsoleWrite(@CRLF)
ConsoleWrite("****Start : TS_TX_LIN_06****"&@CRLF)
...
if winexists($WIN_BUSMASTER) then
	...
	if winexists($WIN_ClustrConfigLIN) then
		...
		_ConfigTXWinDetails(1,"db",1,1,"","Slave","","","a")										; $Rownum,$HSCaseNo("db","ndb"),$messageid/Index(For dbMessage),chnl,datalength,Messageype,RTR(Yes/No),Repetit
		_ConfigTXWinDetails(2,"db",2,1,"","Master","","","b")										; $Rownum,$HSCaseNo("db","ndb"),$messageid/Index(For dbMessage),chnl,datalength,Messageype,RTR(Yes/No),Repetition,Key
		_ConfigTXWinDetails(3,"db",2,1,"","Slave","","","b")										; $Rownum,$HSCaseNo("db","ndb"),$messageid/Index(For dbMessage),chnl,datalength,Messageype,RTR(Yes/No),Repetit
	...
	if $rCount >= 2 Then
		...
		if $Msg0.Item(0) <>" " and $Msg0.Item(1)="Frame1" and $Msg0.Item(2)="LIN Message" and $Msg0.Item(3)="Tx" and $Msg0.Item(4)=1 and $Msg0.Item(5)=4 and $Msg0.Item(6)= 0x001 and $Msg0.Item(7)="F3 FF FF FF" and $Msg0.Item(8)="0x4A (""Enhanced"")" Then			; Compare the Direction, Channel and Msg ID of the first row
			$FirstMsg=1
		Endif
		if $Msg1.Item(0) <>" " and $Msg1.Item(1)="Frame2" and $Msg1.Item(2)="LIN Message" and $Msg1.Item(3)="Tx" and $Msg1.Item(4)=1 and $Msg1.Item(5)=4 and $Msg1.Item(6)= 0x002 and $Msg1.Item(7)="FF FF FF FF" and $Msg1.Item(8)="0xBD (""Enhanced"")" Then			; Compare the Direction, Channel and Msg ID of the first row
			$SecondMsg=1
...
EndIf
	...
	If($FirstMsg=1 And $SecondMsg=1 ) Then
		_WriteResult("Pass","TS_TX_LIN_06")
	Else
		_WriteResult("Fail","TS_TX_LIN_06")
	EndIf
