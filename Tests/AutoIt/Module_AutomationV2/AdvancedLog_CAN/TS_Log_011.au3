...
ConsoleWrite("****Start : TS_Log_011.au3****"&@CRLF)
...
if winexists($WIN_BUSMASTER) then
	...
	sleep(2000)
	_ConfigureCANLogWithLogpath("SampleLog")
...
if winexists($WIN_BUSMASTER) then
	sleep(2000)
	_CANLogMenu()
	WinActivate($WIN_CANLog,3)
	if winexists($WIN_CANLog) then
		$CHKB_OnTransmissionHWD=controlgethandle($WIN_CANLog,"",$CHKB_OnTransmission_ConfigLOG)											;Get the handler for the check box to create log files for each connect
		$CHKB_OnTransmission_State=_GUICtrlButton_GetCheck($CHKB_OnTransmissionHWD)																			;Checking whether the check box is checked
		$CHKB_LogfileSizeHWD=controlgethandle($WIN_CANLog,"",$CHKB_LogfileSize_ConfigLOG)											    ;Get the handler for the check box to create log files after exceeding the time limit
		$CHKB_LogfileSize_State=_GUICtrlButton_GetCheck($CHKB_LogfileSizeHWD)																			    ;Checking whether the check box is checked
		$CHKB_LogOnTimeHWD=controlgethandle($WIN_CANLog,"",$CHKB_LogOnTime_ConfigLOG)													;Get the handler for the check box to create log files in certain time limit
		$CHKB_LogOnTime_State= _GUICtrlButton_GetCheck($CHKB_LogOnTimeHWD)																			    ;Checking whether the check box is checked
		$EDIT_NumberOfLogfiles_State=ControlCommand($WIN_CANLog,"",$EDIT_NumberOfLogfiles_ConfigLOG,"IsEnabled")                                                 ;Status of the checkbox to enable the number of logfiles to be created
...
EndIf
	...
	If $validationObject1 = 1  Then
		_WriteResult("Pass", "TS_Log_011")
	Else
		_WriteResult("Fail", "TS_Log_011")
	EndIf

;FileDelete ( _OutputDataPath() )
ConsoleWrite("****End : TS_LOG_11.au3****"&@CRLF)
ConsoleWrite(@CRLF)