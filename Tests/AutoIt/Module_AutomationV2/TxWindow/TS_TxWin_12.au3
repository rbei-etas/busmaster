...
;~   _launchApp()
ConsoleWrite(@CRLF)
ConsoleWrite("****Start : TS_TxWin_012.au3****"&@CRLF)
  _launchApp()
$timeDiff=0
$timeDiff1=0
WinActivate($WIN_BUSMASTER)
Local $Time_cyclic=0,$Time_cyclic1=0,$Channel_cyclic=0,$Channel_cyclic1=0,$Dir_cyclic=0,$Dir_cyclic1=0,$Data_cyclic1=0,$Data_cyclic1=0,$Id_cyclic=0,$Id_cyclic1=0
if winexists($WIN_BUSMASTER) then
   ...
   _loadConfig("TS_TxWin_01.cfx")
	...
	sleep(1000)
 _ConfigCANTXBlockDetails("Cyclic","No","","Yes","a","","Yes",2000)							                       ; Configure TX block details

	_AddMsg2TxList(0)																			                       ; Add the first msg to Tx list
	_AddMsg2TxList(1)

	_ConfigCANTXBlockDetails("Cyclic","No","","Yes","b","","Yes",2000)							                       ; Configure TX block details
	...
	$rCount=_GetCANMsgWinItemCount()                                                                                     ;Get no of items in message window
	If $rCount>=8 Then
	...
	sleep(1000)
	_DeleteMsgBlock()
	_ConfigCANTXBlockDetails("Cyclic","No","","Yes","a","All","Yes",2000)							                 ; Configure TX block details
	_AddMsg2TxList(0)																			                     ; Add the first msg to Tx list
	_AddMsg2TxList(1)

    _ConfigCANTXBlockDetails("Cyclic","No","","Yes","b","All","Yes",2000)							                 ; Configure TX block details
	_AddMsg2TxList(2)																			                     ; Add the first msg to Tx list
	_AddMsg2TxList(3)
	_CloseTxWindow()
	sleep(1000)
	...
	If $rCount1>=6 Then
    	...
    	Else
		    $Time_cyclic1=0
        EndIf
	    ConsoleWrite("$Data3_all[4]" & $Data3_all[4] & @CRLF)
    	If ($Data1_all[4]=0x015) And ($Data3_all[4]=0x20) And ($Data5_all[4]=0x025) And ($Data7_all[4]=0x066)  Then      ;Verify for message ID
	    	$Id_cyclic1=1
	    Else
	    	$Id_cyclic1=0
    	EndIf
    	...
    	Else
    		$Channel_cyclic1=0
    	EndIf
    	If ($Data1_all[7]="0000000000000000" And $Data3_all[7]="00000000000000") Then                                   ;Verify for Data bytes
	    	$Data_cyclic1=1
    	Else
	    	$Data_cyclic1=0
    	EndIf
	...
	ConsoleWrite("$Data_cyclic1 " & $Data_cyclic1 & @CRLF)
	...
	If ($Time_cyclic1=1 And $Id_cyclic1=1 And $Dir_cyclic1=1 And $Channel_cyclic1=1 And $Data_cyclic1=1 )  Then       ;Verify for message ID
		$Pass_cyclic2=1
	Else
		$Pass_cyclic2=0
	EndIf
	...
	If ($Pass_cyclic1=1 And $Pass_cyclic2=1) Then
		_WriteResult("Pass","TS_Tx_12")
	Else
		_WriteResult("Fail","TS_Tx_12")
	EndIf
...
if $isAppNotRes=1 Then
	_WriteResult("Warning","TS_Tx_12")
EndIf

ConsoleWrite("****End : TS_TxWin_12.au3****"&@CRLF)
ConsoleWrite(@CRLF)