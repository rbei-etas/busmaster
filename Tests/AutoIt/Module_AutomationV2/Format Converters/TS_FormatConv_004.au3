...
;                           3. select Bus Type as J1939
;                           4.Select Log file for input by clicking browse button. And select any log file for ex D:\logs\BUSMASTERLogFile_J1939_1.7.0.log
...
;
; Expected Results  : 		 After step3, the columns(available fields) that are present for CAN are displayed. Ie Time,channel,CAN ID,PGN,TYPE,Source,Destination,Priority,DirectionDLC,Data Bytes.
...
ConsoleWrite(@CRLF)
ConsoleWrite("****Start : TS_FormatConv_004****"&@CRLF)
...
if winexists($WIN_BUSMASTER) then
	...
	$Result=_LogToExcelConversions("J1939","TestLogToExcel_J1939.log","TestLogToExcel_J1939.xls")
    $OutputCppFilePath=$OutputFolderPath &"\"& "TestLogToExcel_J1939.xls"
    If FileExists($OutputCppFilePath) Then
	 ...
	 ConsoleWrite("$Conv" & $ConvSuccess & @CRLF)
	  If($ConvSuccess=1 And $OutputFileExist=1) Then
		_WriteResult("Pass","STS_FORMAT_CONV_004")
	Else
		_WriteResult("Fail","STS_FORMAT_CONV_004")
	EndIf
