...
ConsoleWrite(@CRLF)
ConsoleWrite(@CRLF)
ConsoleWrite("****Start : TS_Replay_14.au3****"&@CRLF)
...
if winexists($WIN_BUSMASTER) then
	...
	_loadConfig("cfxReplay_14")																	; Load Configuration

	_CANReplayMenu()																			; Open configure replay dialog

	_ADDReplayFile("Replay_14")																	; Add a replay file
	sleep(1000)
...
;~ 	_EnableHex()																				; Enable Hex mode
	...
	if $FirstMsg[4]="0x100" and $SecondMsg[4]="0x150" and $ThirdMsg[4]="0x150" Then
		_WriteResult("Pass","TS_Replay_14")
	Else
		_WriteResult("Fail","TS_Replay_14")
	EndIf
...
if $isAppNotRes=1 Then
	_WriteResult("Warning","TS_Replay_14")
EndIf


ConsoleWrite("****End : TS_Replay_14.au3****"&@CRLF)
ConsoleWrite(@CRLF)
