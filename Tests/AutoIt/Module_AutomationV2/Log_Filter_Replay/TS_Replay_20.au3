...
ConsoleWrite(@CRLF)
ConsoleWrite(@CRLF)
ConsoleWrite("****Start : TS_Replay_20.au3****"&@CRLF)
...
if winexists($WIN_BUSMASTER) then
	...
	_loadConfig("cfxReplay_20")																	; Load Configuration

	_CANReplayMenu()																			; Open configure replay dialog

	_ADDReplayFile("Replay_20")																	; Add a replay file
	sleep(1000)
...
;~ 	_EnableHex()																				; Enable Hex mode
	...
	EndIf
	#ce
	;--------Added by Srinibas Das-----------------updated below to avoid virtual controller issue on Tx and Rx messges.
	...
	if $MsgIDs=1 and $MsgDir=1  and $MsgCh=1 Then
		_WriteResult("Pass","TS_Replay_20")
	Else
		_WriteResult("Fail","TS_Replay_20")
	EndIf
...
if $isAppNotRes=1 Then
	_WriteResult("Warning","TS_Replay_20")
EndIf


ConsoleWrite("****End : TS_Replay_20.au3****"&@CRLF)
ConsoleWrite(@CRLF)
