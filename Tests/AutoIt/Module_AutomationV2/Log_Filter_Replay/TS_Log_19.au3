...
ConsoleWrite(@CRLF)
ConsoleWrite(@CRLF)
ConsoleWrite("****Start : TS_Log_19.au3****"&@CRLF)
...
if winexists($WIN_BUSMASTER) then
	_loadConfig("cfxLog_19")															; Load Configuration

	_ConfigureCANLog("LOG_19_Sample","System","ALL","Decimal","Overwrite","","","True")	; Configure Logging details
...
if _FileCountLines($GetLogFile_Path & "\Log_19_Sample.log")>=19 Then
	$MsgIDRx1=FileReadLine ($GetLogFile_Path & "\LOG_19_Sample.log",16)						; Read the 16th line from the Log file
	...
	$MsgIDRx2=FileReadLine ($GetLogFile_Path & "\LOG_19_Sample.log",17)						; Read the 17th line from the Log file
	...
	$MsgIDRx3=FileReadLine ($GetLogFile_Path & "\LOG_19_Sample.log",18)						; Read the 18th line from the Log file
	...
	$MsgIDRx4=FileReadLine ($GetLogFile_Path & "\LOG_19_Sample.log",19)						; Read the 19th line from the Log file
...
if $ExtMsgDir="Found" and $ExtMsgCh="Found" and $ExtMsgID="Found" and $ExtMsgType="Found" Then
	_WriteResult("Pass","TS_Log_19")
Else
	_WriteResult("Fail","TS_Log_19")
EndIf

$isAppNotRes=_CloseApp()															; Close the app

if $isAppNotRes=1 Then
	_WriteResult("Warning","TS_Log_19")
EndIf

ConsoleWrite("****End : TS_Log_19.au3****"&@CRLF)
ConsoleWrite(@CRLF)