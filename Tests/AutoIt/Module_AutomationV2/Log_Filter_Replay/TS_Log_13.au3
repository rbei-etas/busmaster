...
ConsoleWrite(@CRLF)
ConsoleWrite(@CRLF)
ConsoleWrite("****Start : TS_Log_13.au3****"&@CRLF)
...
if winexists($WIN_BUSMASTER) then
	...
	_loadConfig("cfxLog_13")																; Load Configuration

	_ConfigureCANLog("LOG_13_Sample","System","ALL","Decimal","Overwrite","","","True")		; Configure Logging details
...
if _FileCountLines($GetLogFile_Path & "\Log_13_Sample.log")>=17 Then
	$EndOfLog=FileReadLine ($GetLogFile_Path & "\LOG_13_Sample.log",17)						; Read the 17th line from the Log file
	consolewrite("$EndOfLog :"&$EndOfLog&@CRLF)
EndIf

if $EndOfLog="***[STOP LOGGING SESSION]***" Then
	_WriteResult("Pass","TS_Log_13")
Else
	_WriteResult("Fail","TS_Log_13")
EndIf

$isAppNotRes=_CloseApp()															; Close the app

if $isAppNotRes=1 Then
	_WriteResult("Warning","TS_Log_13")
EndIf


ConsoleWrite("****End : TS_Log_13.au3****"&@CRLF)
ConsoleWrite(@CRLF)