...
ConsoleWrite(@CRLF)
ConsoleWrite(@CRLF)
ConsoleWrite("****Start : TS_Replay_27.au3****"&@CRLF)
...
if winexists($WIN_BUSMASTER) then
	...
	_loadConfig("cfxReplay_27")																	; Load Configuration
...
;~ 	_EnableHex()																				; Enable Hex mode
	...
	EndIf
	#cs
	if $FirstMsg[2]=1 and $SecondMsg[2]=2 and $ThirdMsg[2]=2 and $FourthMsg[2]=1 and $FifthMsg[2]=2 and  $SixthMsg[2]=1 Then
		$MsgDir=1
	EndIf

	if $FifthMsg[1]="Tx" and  $SixthMsg[1]="Rx" Then
		$MsgCh=1
	EndIf
	#Ce
	...
	ElseIf $FirstMsg[1]= "Rx" Then
		if $FirstMsg[2]=2 and $SecondMsg[2]=1 and $ThirdMsg[2]=1 and $FourthMsg[2]=2 and $FifthMsg[2]=1 and  $SixthMsg[2]=2 Then
			$MsgDir=1
		EndIf
	EndIf
	...
	if $MsgIDs=1 and $MsgDir=1  and $MsgCh=1 Then
		_WriteResult("Pass","TS_Replay_27")
	Else
		_WriteResult("Fail","TS_Replay_27")
	EndIf
...
if $isAppNotRes=1 Then
	_WriteResult("Warning","TS_Replay_27")
EndIf


ConsoleWrite("****End : TS_Replay_27.au3****"&@CRLF)
ConsoleWrite(@CRLF)
