...
;~ 							8. Choose 'Channel' as '2' (if available).
...
ConsoleWrite(@CRLF)
ConsoleWrite(@CRLF)
ConsoleWrite("****Start : TS_Replay_31.au3****"&@CRLF)
...
if winexists($WIN_BUSMASTER) then
	...
	_loadConfig("cfxReplay_31")																	; Load Configuration
...
;~ 	_EnableHex()																				; Enable Hex mode
	...
	;_Connect_CAN_Menu()																		; Connect the tool
	_Connect_CAN_Menu()
	...
	EndIf
	#cs
	if $FirstMsg[2]=1 and $SecondMsg[2]=2 and $ThirdMsg[2]=1 and $FourthMsg[2]=2 and $FifthMsg[2]=1 and  $SixthMsg[2]=2 and $SeventhMsg[2]=2 and $EightMsg[2]=1 Then
		$MsgDir=1
	EndIf
	...
	ElseIf $FirstMsg[1] = "Rx" Then
		if $FirstMsg[2]=2 and $SecondMsg[2]=1 and $ThirdMsg[2]=2 and $FourthMsg[2]=1 and $FifthMsg[2]=2 and  $SixthMsg[2]=1 and $SeventhMsg[2]=1 and $EightMsg[2]=2 Then
			$MsgDir=1
		EndIf
	EndIf
	if $FirstMsg[1]="Tx" and  $SecondMsg[1]="Rx" and $ThirdMsg[1]="Tx" and  $FourthMsg[1]="Rx" Then
	...
	if $MsgIDs=1 and $MsgDir=1  and $MsgCh=1 Then
		_WriteResult("Pass","TS_Replay_31")
	Else
		_WriteResult("Fail","TS_Replay_31")
	EndIf
...
if $isAppNotRes=1 Then
	_WriteResult("Warning","TS_Replay_31")
EndIf


ConsoleWrite("****End : TS_Replay_31.au3****"&@CRLF)
ConsoleWrite(@CRLF)
