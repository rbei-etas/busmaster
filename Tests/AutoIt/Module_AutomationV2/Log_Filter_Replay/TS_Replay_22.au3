...
ConsoleWrite(@CRLF)
ConsoleWrite(@CRLF)
ConsoleWrite("****Start : TS_Replay_22.au3****"&@CRLF)


_launchApp()

Dim $FirstMsg[10],$SecondMsg[10],$ThirdMsg[10],$FourthMsg[10],$FifthMsg[10],$SixthMsg[10]
local $MsgIDs=0, $MsgDir=0, $MsgCh=0

if winexists($WIN_BUSMASTER) then

	_loadConfig("cfxReplay_22")																	; Load Configuration

	_CANReplayMenu()																			; Open configure replay dialog

	_ADDReplayFile("Replay_22")																	; Add a replay file
	sleep(1000)
...
;~ 	_EnableHex()																				; Enable Hex mode
	...
	EndIf
	#cs
	if $FirstMsg[2]=1 and $SecondMsg[2]=2 and $ThirdMsg[2]=1 and $FourthMsg[2]=2 and $FifthMsg[2]=2 and  $SixthMsg[2]=1 Then
		$MsgDir=1
	EndIf
	...
	EndIf
	#ce
	;--------Added by Srinibas -----------------updated below to avoid virtual controller issue of Tx and Rx messages.
	if $FirstMsg[1] = "Tx" Then
		if $FirstMsg[2]=1 and $SecondMsg[2]=2 and $ThirdMsg[2]=1 and $FourthMsg[2]=2 and $FifthMsg[2]=2 and  $SixthMsg[2]=1 Then
			$MsgDir=1
		EndIf
	ElseIf $FirstMsg[1]= "Rx" Then
		if $FirstMsg[2]=2 and $SecondMsg[2]=1 and $ThirdMsg[2]=2 and $FourthMsg[2]=1 and $FifthMsg[2]=1 and  $SixthMsg[2]=2 Then
			$MsgDir=1
		EndIf
	EndIf
	if $FirstMsg[1]="Tx" and  $SecondMsg[1]="Rx" Then
...
if $MsgIDs=1 and $MsgDir=1  and $MsgCh=1 Then
	_WriteResult("Pass","TS_Replay_22")
Else
	_WriteResult("Fail","TS_Replay_22")
EndIf

$isAppNotRes=_CloseApp()																			; Close the app

if $isAppNotRes=1 Then
	_WriteResult("Warning","TS_Replay_22")
EndIf


ConsoleWrite("****End : TS_Replay_22.au3****"&@CRLF)
ConsoleWrite(@CRLF)
