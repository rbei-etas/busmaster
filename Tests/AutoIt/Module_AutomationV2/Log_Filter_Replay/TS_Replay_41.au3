...
ConsoleWrite(@CRLF)
ConsoleWrite(@CRLF)
ConsoleWrite("****Start : TS_Replay_41.au3****"&@CRLF)
...
if winexists($WIN_BUSMASTER) then
	...
	_loadConfig("cfxReplay_41")																	; Load Configuration
...
;~ 	_EnableHex()																				; Enable Hex mode
	...
	if $MsgIDs=1 and $MsgDir=1  and $MsgCh=1 Then
		_WriteResult("Pass","TS_Replay_41")
	Else
		_WriteResult("Fail","TS_Replay_41")
	EndIf
...
if $isAppNotRes=1 Then
	_WriteResult("Warning","TS_Replay_41")
EndIf


ConsoleWrite("****End : TS_Replay_41.au3****"&@CRLF)
ConsoleWrite(@CRLF)
