...
ConsoleWrite(@CRLF)
ConsoleWrite(@CRLF)
ConsoleWrite("****Start : TS_Replay_45.au3****"&@CRLF)


_launchApp()

Dim $FirstMsg[10],$SecondMsg[10],$ThirdMsg[10],$FourthMsg[10],$FifthMsg[10],$SixthMsg[10],$SeventhMsg[10],$EightMsg[10],$NinthMsg[10],$TenthMsg[10]

if winexists($WIN_BUSMASTER) then

	_loadConfig("cfxReplay_45")																	; Load Configuration
...
;~ 	_EnableHex()																				; Enable Hex mode
	...
	if $MsgWinCount=10 Then
		...
		$NinthMsg=_GetMsgWinCANInfo(8)															; Fetch the msg win ninth row info
		$TenthMsg=_GetMsgWinCANInfo(9)															; Fetch the msg win tenth row info
	EndIf
	...
	consolewrite("$NinthMsg[4] :"&$NinthMsg[4]&@CRLF)
	consolewrite("$NinthMsg[2] :"&$NinthMsg[2]&@CRLF)
	consolewrite("$NinthMsg[1] :"&$NinthMsg[1]&@CRLF)
	consolewrite("$TenthMsg[4] :"&$TenthMsg[4]&@CRLF)
	consolewrite("$TenthMsg[2] :"&$TenthMsg[2]&@CRLF)
	consolewrite("$TenthMsg[1] :"&$TenthMsg[1]&@CRLF)

	if $FirstMsg[4]="0x100" and $SecondMsg[4]="0x100" and $ThirdMsg[4]="0x100" and $FourthMsg[4]="0x100" and $FifthMsg[4]="0x150" and $SixthMsg[4]="0x150" and $SeventhMsg[4]="0x150" and $EightMsg[4]="0x150" and $NinthMsg[4]="0x200" and $TenthMsg[4]="0x200" Then
		$MsgIDs=1
	EndIf
	#cs
	if $FirstMsg[2]=1 and $SecondMsg[2]=2 and $ThirdMsg[2]=2 and $FourthMsg[2]=1 and $FifthMsg[2]=1 and  $SixthMsg[2]=2 and $SeventhMsg[2]=2 and $EightMsg[2]=1 and $NinthMsg[2]=2 and $TenthMsg[2]=1 Then
		$MsgDir=1
	EndIf

	if $NinthMsg[1]="Tx" and  $TenthMsg[1]="Rx" Then
		$MsgCh=1
	EndIf
	#ce
	...
	ElseIf $FirstMsg[1] = "Rx" Then
		if $FirstMsg[2]=2 and $SecondMsg[2]=1 and $ThirdMsg[2]=1 and $FourthMsg[2]=2 and $FifthMsg[2]=2 and  $SixthMsg[2]=1 and $SeventhMsg[2]=1 and $EightMsg[2]=2 and $NinthMsg[2]=1 and $TenthMsg[2]=2 Then
			$MsgDir=1
		EndIf
	EndIf
	if $NinthMsg[1]="Tx" and  $TenthMsg[1]="Rx" Then
		$MsgCh=1
	ElseIf $NinthMsg[1]="Rx" and  $TenthMsg[1]="Tx" Then
		$MsgCh=1
	EndIf
	;----------------------------------------------------------------------------------------
	...
	if $MsgIDs=1 and $MsgDir=1  and $MsgCh=1 Then
		_WriteResult("Pass","TS_Replay_45")
	Else
		_WriteResult("Fail","TS_Replay_45")
	EndIf
...
if $isAppNotRes=1 Then
	_WriteResult("Warning","TS_Replay_45")
EndIf


ConsoleWrite("****End : TS_Replay_45.au3****"&@CRLF)
ConsoleWrite(@CRLF)
