...
ConsoleWrite(@CRLF)
ConsoleWrite(@CRLF)
ConsoleWrite("****Start : TS_Replay_44.au3****"&@CRLF)
...
if winexists($WIN_BUSMASTER) then
	...
	_loadConfig("cfxReplay_44")																	; Load Configuration
...
;~ 	_EnableHex()																				; Enable Hex mode
	...
	if $FirstMsg[4]="0x100" and $SecondMsg[4]="0x100" and $ThirdMsg[4]="0x100" and $FourthMsg[4]="0x100" and $FifthMsg[4]="0x150" and $SixthMsg[4]="0x150" and $SeventhMsg[4]="0x150" and $EightMsg[4]="0x150" Then
		$MsgIDs=1
	EndIf
	#cs
	if $FirstMsg[2]=1 and $SecondMsg[2]=2 and $ThirdMsg[2]=2 and $FourthMsg[2]=1 and $FifthMsg[2]=1 and  $SixthMsg[2]=2 and $SeventhMsg[2]=2 and $EightMsg[2]=1 Then
		$MsgDir=1
	EndIf
	...
	ElseIf $FirstMsg[1] = "Rx" Then
		if $FirstMsg[2]=2 and $SecondMsg[2]=1 and $ThirdMsg[2]=1 and $FourthMsg[2]=2 and $FifthMsg[2]=2 and  $SixthMsg[2]=1 and $SeventhMsg[2]=1 and $EightMsg[2]=2 Then
			$MsgDir=1
		EndIf
	EndIf
	if $FifthMsg[1]="Tx" and  $SixthMsg[1]="Rx" and $SeventhMsg[1]="Tx" and  $EightMsg[1]="Rx" Then
	...
	if $MsgIDs=1 and $MsgDir=1  and $MsgCh=1 Then
		_WriteResult("Pass","TS_Replay_44")
	Else
		_WriteResult("Fail","TS_Replay_44")
	EndIf
...
if $isAppNotRes=1 Then
	_WriteResult("Warning","TS_Replay_44")
EndIf


ConsoleWrite("****End : TS_Replay_44.au3****"&@CRLF)
ConsoleWrite(@CRLF)
