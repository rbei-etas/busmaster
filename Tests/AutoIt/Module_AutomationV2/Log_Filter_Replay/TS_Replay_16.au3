...
ConsoleWrite(@CRLF)
ConsoleWrite(@CRLF)
ConsoleWrite("****Start : TS_Replay_16.au3****"&@CRLF)
...
if winexists($WIN_BUSMASTER) then
	...
	_loadConfig("cfxReplay_16")																	; Load Configuration

	_CANReplayMenu()																			; Open configure replay dialog

	_ADDReplayFile("Replay_16")																	; Add a replay file
	sleep(1000)
...
;~ 	_EnableHex()																				; Enable Hex mode
	...
	if $MsgIDs=1 and $MsgDir=1  and $MsgCh=1 Then
		_WriteResult("Pass","TS_Replay_16")
	Else
		_WriteResult("Fail","TS_Replay_16")
	EndIf
...
if $isAppNotRes=1 Then
	_WriteResult("Warning","TS_Replay_16")
EndIf


ConsoleWrite("****End : TS_Replay_16.au3****"&@CRLF)
ConsoleWrite(@CRLF)
