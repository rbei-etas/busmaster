/* This file is generated by BUSMASTER */
/* VERSION [1.1] */
/* BUSMASTER VERSION [1.9.0] */
/* PROTOCOL [CAN] */

/* Start BUSMASTER include header */
#include <Windows.h>
#include <CANIncludes.h>

/* End BUSMASTER include header */


/* Start BUSMASTER global variable */

/* End BUSMASTER global variable */


/* Start BUSMASTER Function Prototype  */
GCC_EXTERN void GCC_EXPORT OnKey_All(unsigned char KeyValue);
GCC_EXTERN void GCC_EXPORT OnKey_p(unsigned char KeyValue);
/* End BUSMASTER Function Prototype  */

/* Start BUSMASTER generated function - OnKey_All */
void OnKey_All(unsigned char KeyValue)
{
STCAN_MSG sMsg;

sMsg.m_unMsgID = 0x57;              // Message ID

sMsg.m_ucEXTENDED = FALSE;           // Standard Message type

sMsg.m_ucRTR = FALSE;                // Not RTR type

sMsg.m_ucDLC = 8;                    // Length is 8 Bytes

sMsg.m_sWhichBit.m_aulData[0] = 13;  // Lower 4 Bytes

sMsg.m_sWhichBit.m_aulData[1] = 23;  // Upper 4 Bytes

sMsg.m_ucChannel = 1;                // First CAN channel

SendMsg(sMsg);

Trace("Key Handler All");
}/* End BUSMASTER generated function - OnKey_All */
/* Start BUSMASTER generated function - OnKey_p */
void OnKey_p(unsigned char KeyValue)
{
STCAN_MSG sMsg;

sMsg.m_unMsgID = 0x60;              // Message ID

sMsg.m_ucEXTENDED = FALSE;           // Standard Message type

sMsg.m_ucRTR = FALSE;                // Not RTR type

sMsg.m_ucDLC = 8;                    // Length is 8 Bytes

sMsg.m_sWhichBit.m_aulData[0] = 13;  // Lower 4 Bytes

sMsg.m_sWhichBit.m_aulData[1] = 23;  // Upper 4 Bytes

sMsg.m_ucChannel = 1;                // First CAN channel

SendMsg(sMsg);

Trace("Key Handler P");
}/* End BUSMASTER generated function - OnKey_p */
