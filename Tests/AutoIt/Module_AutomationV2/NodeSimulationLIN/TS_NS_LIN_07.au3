...
ConsoleWrite(@CRLF)
ConsoleWrite(@CRLF)
ConsoleWrite("****Start : TS_NS_LIN_07.au3****"&@CRLF)
...
if winexists($WIN_BUSMASTER) then
	sleep(2000)
	_createConfig("cfxLINNS_07")															 ; Create new Configuration
...
EndIf
	...
	If $WinNodeDetailsDisplayed=1 and $childNode=0 Then
		_WriteResult("Pass","TS_NS_LIN_07")
		ConsoleWrite("PASS")
	Else
	   _WriteResult("Fail","TS_NS_LIN_07")
	   ConsoleWrite("FAIL")
	EndIf
...
if $isAppNotRes=1 Then
	_WriteResult("Warning","TS_NS_LIN_07")
EndIf

ConsoleWrite("****End : TS_NS_LIN_07.au3****"&@CRLF)
ConsoleWrite(@CRLF)
