	...
	if winexists("Configure Simulated Systems - LIN Bus") then
		...
		if $KeyFunCount=1 Then
			for $i=0 to $KeyFunCount-1
				...
				ControlSend($WIN_BUSMASTER, "", $TXT_TimeHandler_CPPFile, "sMsgM1.m_ucMsgTyp = 0;    // master Response" & @CRLF)			; Write in the editor of Node sim
		...
		if $rCount = 1 Then
			...
			if $Msg0.Item(0) <>" " and $Msg0.Item(1)="0x1" and $Msg0.Item(2)="LIN Message" and $Msg0.Item(3)="Tx" and $Msg0.Item(4)=1 and $Msg0.Item(5)=8 and $Msg0.Item(6)= 0x001 and $Msg0.Item(7)="0A 00 00 00 14 00 00 00" and $Msg0.Item(8)="0x20 (""Enhanced"")" Then			; Compare the Direction, Channel and Msg ID of the first row
				$FirstMsg=1
