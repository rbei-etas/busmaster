...
ConsoleWrite(@CRLF)
ConsoleWrite(@CRLF)
ConsoleWrite("****Start : TS_NS_LIN_02.au3****"&@CRLF)
...
if winexists($WIN_BUSMASTER) then
	...
	If $name="ALT_LINALT_R_01" and $name1="ALT_LINALT_R_02" and ($count2==$count3) and $count2 = 5 and $count3 = 5 Then
		_WriteResult("Pass","TS_NS_LIN_02")
		ConsoleWrite("PASS")
	Else
	   _WriteResult("Fail","TS_NS_LIN_02")
	   ConsoleWrite("FAIL")
	EndIf
	#ce
EndIf
...
if $isAppNotRes=1 Then
	_WriteResult("Warning","TS_NS_LIN_02")
EndIf

ConsoleWrite("****End : TS_NS_LIN_02.au3****"&@CRLF)
ConsoleWrite(@CRLF)
