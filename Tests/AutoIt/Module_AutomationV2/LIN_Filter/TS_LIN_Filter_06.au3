...
if winexists($WIN_BUSMASTER) then
	...
	if _FileCountLines($GetLogFile_Path &"\"& $LoGFiltername&".log")=18 Then
		$Read_LogFirstEnter=FileReadLine ($GetLogFile_Path &"\"& $LoGFiltername&".log",15)			; Read the 16th line from the Log file
		$Read_LogSecEnter=FileReadLine ($GetLogFile_Path &"\"& $LoGFiltername&".log",16)				; Read the 17th line from the Log file


		$Read_LogEndDate=FileReadLine ($GetLogFile_Path &"\"& $LoGFiltername&".log",17)				; Read the 19th line from the Log file
		$Read_EndSession=FileReadLine ($GetLogFile_Path &"\"& $LoGFiltername&".log",18)				; Read the 19th line from the Log file
		...
		if $FirstMsg[2]="Tx" and $SecMsg[2]="Tx" Then
			$txMsg =1
		EndIf
		if $FirstMsg[4]="0001" and $SecMsg[4]="0001"  Then
			$msgidCheck =1
		EndIf
	EndIf
