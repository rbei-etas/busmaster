...
ConsoleWrite(@CRLF)
ConsoleWrite(@CRLF)
ConsoleWrite("****Start :TS_LIN_Filter_05.au3****"&@CRLF)
...
if winexists($WIN_BUSMASTER) then
	...
	_loadConfig("cfxLINFilter_05")																	; Load Configuration
	...
	_ConfigureLINLog("LIN_Log_Filter_05","System","ALL","Decimal","Append","","","True")
	...
	if $rCount = 1 Then
		...
		for $i=0 to $Count1-2
			$Msg=_GetMsgWinInfo($LVC_CID_LINMsgWin,0,10)
			$Value =$Msg.Item($i)
...
if $FirstMsgInMsgWindow=1 and $txMsg=1 and $msgidCheck=1 and $endOfFIle=1 Then												; Validate the result
	_WriteResult("Pass","TS_LIN_Filter_05")
	ConsoleWrite("Pass"&@CRLF)
Else
	_WriteResult("Fail","TS_LIN_Filter_05")
...
if $isAppNotRes=1 Then
	_WriteResult("Warning","TS_LIN_Filter_05")
EndIf

ConsoleWrite("****End : TS_LIN_Filter_05.au3****"&@CRLF)
ConsoleWrite(@CRLF)
