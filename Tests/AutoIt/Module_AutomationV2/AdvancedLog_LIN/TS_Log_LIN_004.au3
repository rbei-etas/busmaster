...
ConsoleWrite("****Start : TS_Log_LIN_004.au3****"&@CRLF)

Local $validationObject1=0                                                               					    	  				   ;To validate whether the log file is adding to the listview
Local $validationObject2=0																				 							   ;To validate whether new file is getting created after exceeding the time given
Local $validationObject3=0                                                                                                             ;To validate whether it is overwriting the file
Local $validationObject4=0																											   ;To validate the  start of comment and end of comment entry.
$logfile1CreatedTime1=0
$logfile1CreatedTime2=0
;Local $validationObject5=0
...
if winexists($WIN_BUSMASTER) then
	...
	if winexists($WIN_LINLog) then
		...
		sleep(1000)
		$logfilehwd = ControlGetHandle($WIN_LINLog, "", $LSTC_LogFile_ConfigLOG)
		...
		$LogMinutesHWD=controlgethandle($WIN_LINLog,"",$EDIT_Minutes_ConfigLOG)
	...
	If $validationObject1 = 1 And $validationObject2 = 1 And $validationObject3 = 1 And $validationObject4 = 1 Then
		_WriteResult("Pass", "TS_Log_LIN_004")
	Else
		_WriteResult("Fail", "TS_Log_LIN_004")
	EndIf
...
ConsoleWrite("****End : TS_LOG_LIN_04.au3****"&@CRLF)
ConsoleWrite(@CRLF)