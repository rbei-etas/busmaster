...
ConsoleWrite("****Start : TS_Log_LIN_008.au3****"&@CRLF)
...
if winexists($WIN_BUSMASTER) then
	...
	if winexists($WIN_LINLog) then
		...
		ControlSend ( $WIN_LINLog, "", $EDIT_NumberOfLogfiles_ConfigLOG, "0" )															;Set the value to 0
		...
		ControlSend ( $WIN_LINLog, "", $EDIT_NumberOfLogfiles_ConfigLOG, "1100")														;Set the value to 1100
	...
	If $validationObject1 = 1 And $validationObject2 = 1 And $validationObject3 = 1 Then
		_WriteResult("Pass", "TS_Log_LIN_008")
	Else
		_WriteResult("Fail", "TS_Log_LIN_008")
	EndIf
	_CloseApp()
...
ConsoleWrite("****End : TS_LOG_LIN_08.au3****"&@CRLF)
ConsoleWrite(@CRLF)