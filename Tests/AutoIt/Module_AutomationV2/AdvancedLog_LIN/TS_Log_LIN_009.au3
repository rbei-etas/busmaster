...
ConsoleWrite("****Start : TS_Log_LIN_009.au3****"&@CRLF)
...
if winexists($WIN_BUSMASTER) then
	...
	if winexists($WIN_LINLog) then
		...
		Sleep(1000)
		$LogfileSizeHWD=controlgethandle($WIN_LINLog,"",$EDIT_Filesize_ConfigLOG)
		...
		ControlSend ( $WIN_LINLog, "", $EDIT_Filesize_ConfigLOG, "0" )															        ;Set the value to 0
		...
		_GUICtrlEdit_SetSel($LogfileSizeHWD, 0, -1)																				        ;Select the text in the edit box for the size limit
		ControlSend ( $WIN_LINLog, "", $EDIT_Filesize_ConfigLOG, "5000" )															    ;Set the value to 5000
		WinWait("BUSMASTER","",2)																									;Wait till the pop up comes
            if winexists("BUSMASTER","") Then
				$txt=ControlGetText ( "BUSMASTER", "","[CLASS:Static; INSTANCE:2]" )												;Get the text in the pop up window
				ConsoleWrite("$txt - "&$txt & @CRLF)
	...
	If $validationObject1 = 1 And $validationObject2 = 1 And $validationObject3 = 1 Then
		_WriteResult("Pass", "TS_Log_LIN_009")
	Else
		_WriteResult("Fail", "TS_Log_LIN_009")
	EndIf
...
ConsoleWrite("****End : TS_LOG_LIN_09.au3****"&@CRLF)
ConsoleWrite(@CRLF)