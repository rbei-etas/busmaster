...
ConsoleWrite("****Start : TS_Log_LIN_007.au3****"&@CRLF)
...
$logfile1CreatedTime1ForTime=0
$logfile1CreatedTime2ForTime=0
...
if winexists($WIN_BUSMASTER) then
	...
	if winexists($WIN_LINLog) then
		...
		_GUICtrlEdit_SetText($LogMinutesHWD,"10")                                                                                      ;Set the value '10' for the minutes
		sleep(1000)
	...
	if $logFile3=1 Then
		...
		$size=$sizeinBytes/1048576
		if $size >1 Then																		                                     	;Check whether the size is greater than 5 MB
			$validationObject3=1
		EndIf
	EndIf
...
;~     _CloseTxWindowArg("CAN")
	...
	If WinExists($WIN_LINLog) Then
		$LogfileSizeHWD=controlgethandle($WIN_LINLog,"",$EDIT_Filesize_ConfigLOG)													   ;Get the handler for the edit box to enter the logfilesize
		_GUICtrlEdit_SetText($LogfileSizeHWD,"10")																					   ;Set the value '10' for the size of the log file
		$LogMinutesHWD=controlgethandle($WIN_LINLog,"",$EDIT_Minutes_ConfigLOG)														   ;Geth the handler for the edit box to enter minutes
	...
	If $validationObject1 = 1 And $validationObject2 = 1 And $validationObject3 = 1 And $validationObject4 = 1 And $validationObject5 = 1 And $validationObject6 = 1 And $validationObject7 = 1 Then
		_WriteResult("Pass", "TS_Log_LIN_007")
	Else
		_WriteResult("Fail", "TS_Log_LIN_007")
	EndIf
...
ConsoleWrite("****End : TS_LOG_LIN_07.au3****"&@CRLF)
ConsoleWrite(@CRLF)