...
ConsoleWrite("****Start : TS_Log_LIN_010.au3****"&@CRLF)
...
if winexists($WIN_BUSMASTER) then
	...
	if winexists($WIN_LINLog) then
		...
		sleep(1000)
		$LogHoursHWD=controlgethandle($WIN_LINLog,"",$EDIT_Hours_ConfigLOG)
		...
		ControlSend ( $WIN_LINLog, "", $EDIT_Hours_ConfigLOG, "170" )															        ;Set the value to 170
		...
		ControlSend ( $WIN_LINLog, "", $EDIT_Minutes_ConfigLOG, "60" )															        ;Set the value to 60
		WinWait("BUSMASTER","",2)																									;Wait till the pop up comes
            if winexists("BUSMASTER","") Then
				$txt=ControlGetText ( "BUSMASTER", "","[CLASS:Static; INSTANCE:2]" )												;Get the text in the pop up window
				ConsoleWrite("$txt - "&$txt & @CRLF)
	...
	If $validationObject1 = 1 And $validationObject2 = 1 And $validationObject3 = 1 Then
		_WriteResult("Pass", "TS_Log_LIN_010")
	Else
		_WriteResult("Fail", "TS_Log_LIN_010")
	EndIf
	_CloseApp()
...
ConsoleWrite("****End : TS_LOG_LIN_10.au3****"&@CRLF)
ConsoleWrite(@CRLF)