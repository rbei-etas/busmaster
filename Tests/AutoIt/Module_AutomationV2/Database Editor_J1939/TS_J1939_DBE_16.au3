...
;~ 							2.After step3,the 'Message Length' will reach a max value of 1785 and there after it should ignore the spin control clicks.
;~ 							3.After step4,the 'Message Length' will reach a min value of 0 and there after it should ignore the spin control clicks.

; Critical (C)		:		Y
; TestCase No.		:		TS_J1939_DBE_15
...
; Critical (C)		:		Y
; TestCase No.		:		TS_J1939_DBE_17
; TestCases			:		Message Details
; Test Data			:
; Test Setup		:		1. Continue from above test case TS_J1939_DBE_16.
;~ 							2. Manually enter a value of 1785 in 'Message Length' edit box.
;~ 							3. Click 'OK' in 'Message Details' dialog.

; Expected Results  : 		1. After step3, a message box showing 'Please enter integer between 0 and 1785' should be shown.
;==========================================================================Test Procedure =========================================================================

ConsoleWrite(@CRLF)
ConsoleWrite(@CRLF)
ConsoleWrite("****Start : TS_J1939_DBE_16.au3****"&@CRLF)
...
;~ 	_CreateConfig("cfxj1939DB_16.cfx")																; Create Configuration
	...
	if WinExists($WIN_Msg_Details) Then
		...
		sleep(500)
		send("{UP 1790}")																			; Press the Down key 1790 times
		sleep(500)
		ControlClick($WIN_Msg_Details, "", $TXT_MsgLen_MsgDetails)									; Click on Msg Length edit box
		...
		sleep(500)
		send("{DOWN 1790}")																			; Press the Down key 1790 times
		sleep(500)
	...
	If WinExists($WIN_Msg_Details) Then
		...
		sleep(500)
		ControlSetText($WIN_Msg_Details, "", $TXT_MsgLen_MsgDetails, 1786) 							; Enter the msg len
		sleep(500)
		ControlClick($WIN_Msg_Details, "", $BTN_OK_MsgDetails) 										; Click 'OK' button
		If WinWaitActive("",$TXT_J1939MsgIDValidation_BM,3) Then
			$ValMsgID="Exists"
			ControlClick($WIN_BUSMASTER, $TXT_J1939MsgIDValidation_BM, $BTN_MsgIDValidation_BM)		; Click 'OK' button
			sleep(500)
		EndIf
		ControlSetText($WIN_Msg_Details, "", $TXT_MsgLen_MsgDetails, 8) 							; Enter the msg len
		sleep(500)
...
if $MsgLenAftSpinUp=1785 and $MsgLenAftSpinDown=0 and $DupMsgName="Exists" and $DupMsgID="Exists" and $ValMsgID="Exists" Then
	_WriteResult("Pass","TS_J1939_DBE_16")
Else
	_WriteResult("Fail","TS_J1939_DBE_16")
EndIf
...
if $isAppNotRes=1 Then
	_WriteResult("Warning","TS_J1939_DBE_16")
EndIf

ConsoleWrite("****End : TS_J1939_DBE_16.au3****"&@CRLF)
ConsoleWrite(@CRLF)