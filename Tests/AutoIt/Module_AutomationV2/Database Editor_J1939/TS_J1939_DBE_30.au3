...
; Critical (C)		:		Y
; TestCase No.		:		TS_J1939_DBE_30
; TestCases			:		Addition of message signals
; Test Data			:
; Test Setup		:		1. Continue from above test case TS_J1939_DBE_29.
...
; Critical (C)		:		Y
; TestCase No.		:		TS_J1939_DBE_31
; TestCases			:		Addition of message signals
; Test Data			:
; Test Setup		:		1. Continue from above test case TS_J1939_DBE_30.
...
; Critical (C)		:		Y
; TestCase No.		:		TS_J1939_DBE_32
; TestCases			:		Addition of message signals
; Test Data			:
; Test Setup		:		1. Continue from above test case TS_J1939_DBE_31.
;~ 							2. Click on 'New Signal...' button in 'Signal details' groupbox to get 'Signal Details' dialog.
...
; Critical (C)		:		Y
; TestCase No.		:		TS_J1939_DBE_33
; TestCases			:		Manipulation of message signals
; Test Data			:
; Test Setup		:		1. Continue from above test case TS_J1939_DBE_32.
...
; Critical (C)		:		Y
; TestCase No.		:		TS_J1939_DBE_34
; TestCases			:		Manipulation of message signals
; Test Data			:
; Test Setup		:		1. Continue from above test case TS_J1939_DBE_33.
...
ConsoleWrite(@CRLF)
ConsoleWrite(@CRLF)
ConsoleWrite("****Start : TS_J1939_DBE_30.au3****"&@CRLF)
...
If winexists($WIN_BUSMASTER) then
	...
	if WinWaitActive($WIN_Signal_Details,"",3) Then
		...
		if WinWaitActive($WIN_BUSMASTER,$TXT_DupStartBit_BM,3) Then
			$DupStartBit=1
			ControlClick($WIN_BUSMASTER,$TXT_DupStartBit_BM,$BTN_DupStartBit_BM)									; Click on OK button
			sleep(500)
		EndIf

		ControlSetText($WIN_Signal_Details, "", $TXT_ByteIndex_SigDetails, 8)				 			; Byte Index
		sleep(500)
		ControlClick($WIN_Signal_Details, "", $BTN_OK_SigDetails) 										; Click 'OK' button
		sleep(500)
	EndIf
	...
	if WinWaitActive($WIN_Signal_Details,"",3) Then
		...
		if WinWaitActive("",$TXT_InvEndBit_BM,3) Then
			...
			sleep(500)
		EndIf
		ControlSetText($WIN_Signal_Details, "", $TXT_ByteIndex_SigDetails, 14)				 			; Byte Index
		sleep(500)
	...
	sleep(500)
	ControlClick($WIN_DBEditor_J1939,"",$BTN_EditSig_DBEditor)											; Click on 'Edit Signal'
	If WinWaitActive($WIN_Signal_Details,"",3) Then
	...
	ControlClick($WIN_DBEditor_J1939,"",$BTN_SigDes_DBEditor)											; Click on 'New Desc' button
...
if $Sig2=1 and $Sig3=1 and $PopUpMenu=1 and $NullSigDesc=1 Then
	_WriteResult("Pass","TS_J1939_DBE_30")
Else
...
if $isAppNotRes=1 Then
	_WriteResult("Warning","TS_J1939_DBE_30")
EndIf

ConsoleWrite("****End : TS_J1939_DBE_30.au3****"&@CRLF)
ConsoleWrite(@CRLF)