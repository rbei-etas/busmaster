...
;~ 							4. Add the following message details: 'Message Name' as 'Msg1', 'Message ID' as '0x15', 'Message Length' as '1785'.
...
ConsoleWrite(@CRLF)
ConsoleWrite(@CRLF)
ConsoleWrite("***********J1939 DBEditor Script Execution Started************"&@CRLF)
ConsoleWrite("****Start : TS_J1939_DBE_01.au3****"&@CRLF)
_launchApp()
...
if winexists($WIN_BUSMASTER) then
	...
	_addMsgDetails("Msg1",1785,8)																	; Add the message details
...
if $msgNametxt="" and  $msgIDtxt="" and $msglentxt=0 and $msgNameTreeView="Msg1" then			; Write the status of the testcase
	_WriteResult("Pass","TS_J1939_DBE_01")
Else
	_WriteResult("Fail","TS_J1939_DBE_01")
EndIf

$isAppNotRes=_CloseApp()																		; Close the app

if $isAppNotRes=1 Then
	_WriteResult("Warning","TS_J1939_DBE_01")
EndIf



ConsoleWrite("****End : TS_J1939_DBE_01.au3****"&@CRLF)
ConsoleWrite(@CRLF)