...
; Critical (C)		:		Y
; TestCase No.		:		TS_DBE_46
; TestCases			:		Database serialization
; Test Data			:
; Test Setup		:		1. Continue from above test case TS_DBE_45.
;~ 							2. Try to close the database using menu option 'File -> Database -> Close' or by by clicking close button in the Database Editor window.
...
ConsoleWrite(@CRLF)
ConsoleWrite(@CRLF)
ConsoleWrite("****Start : TS_DBE_45.au3****"&@CRLF)
...
If winexists($WIN_BUSMASTER) then
	...
	WinMenuSelectItem("BUSMASTER - [DatabaseEditor - CAN]","",$CANMenu,$databaseMenu,$openMenu)

	$DirPath = _OutputDataPath()																		; Set the DirPath to saved dbf file path
	if WinWaitActive($WIN_SelCAN_DBFile,"",5) Then
		ControlSend($WIN_SelCAN_DBFile,"",$TXT_FileName_OpenDB,$DirPath&"\testDBE_45.DBF")				; Set Filename
		sleep(1000)
...
if $SaveDBWin=1 and $SignalCount=1 Then
	_WriteResult("Pass","TS_DBE_45")
Else
	_WriteResult("Fail","TS_DBE_45")
EndIf

$isAppNotRes=_CloseApp()																				; Close the app

if $isAppNotRes=1 Then
	_WriteResult("Warning","TS_DBE_45")
EndIf

ConsoleWrite("****End : TS_DBE_45.au3****"&@CRLF)
ConsoleWrite(@CRLF)