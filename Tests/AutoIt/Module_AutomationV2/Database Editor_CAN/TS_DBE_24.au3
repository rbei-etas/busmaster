...
ConsoleWrite(@CRLF)
ConsoleWrite(@CRLF)
ConsoleWrite("****Start : TS_DBE_24.au3****"&@CRLF)
...
If winexists($WIN_BUSMASTER) then
	...
	_CreateConfig("cfxCANDB_24.cfx")																	; Create Configuration

	_openCANDB("testDBE_24.DBF")																		; Open the DB
...
if $InvalidSigLen="Exists" and $sigName="Sig1" and $sigLen=64 and $sigMaxVal="FFFFFFFFFFFFFFFF" and $sigMinVal=0 and $sigFactor=10.000000 Then
	_WriteResult("Pass","TS_DBE_24")
Else
	_WriteResult("Fail","TS_DBE_24")
EndIf
...
if $isAppNotRes=1 Then
	_WriteResult("Warning","TS_DBE_24")
EndIf

ConsoleWrite("****End : TS_DBE_24.au3****"&@CRLF)
ConsoleWrite(@CRLF)