...
ConsoleWrite(@CRLF)
ConsoleWrite(@CRLF)
ConsoleWrite("****Start : TS_DBE_38.au3****"&@CRLF)
...
If winexists($WIN_BUSMASTER) then
	...
	_CreateConfig("cfxCANDB_38.cfx")																	; Create Configuration
	...
	ControlClick($WIN_DBEditor_CAN,"",$BTN_SigDes_DBEditor)												; Click on 'New Desc'
...
if  $DupSigDesc=1 and $InvalidSigVal=1 and $PopUpMenu=1 Then
	_WriteResult("Pass","TS_DBE_38")
Else
	_WriteResult("Fail","TS_DBE_38")
EndIf

$isAppNotRes=_CloseApp()																				; Close the app

if $isAppNotRes=1 Then
	_WriteResult("Warning","TS_DBE_38")
EndIf

ConsoleWrite("****End : TS_DBE_38.au3****"&@CRLF)
ConsoleWrite(@CRLF)