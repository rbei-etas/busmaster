...
; Critical (C)		:		Y
; TestCase No.		:		TS_LIN_Log_29                             Source: (TS_LIN_Log_13)
...
ConsoleWrite(@CRLF)
ConsoleWrite(@CRLF)
ConsoleWrite("****Start : TS_LIN_Log_29.au3****"&@CRLF)


_launchApp()

Dim $HexMsgID21Tx1[10]=["","","","","","","","","",""],$HexMsgID21Tx2[10]=["","","","","","","","","",""],$HexMsgID21Tx3[10]=["","","","","","","","","",""]
Local $filename = "\Log_29_Sample.log",$verfyCount = 0,$finddatacount=0,$finddatacount5=0,$finddatacount6=0,$finddatacount7=0,$cnt=0
...
if winexists($WIN_BUSMASTER) then
	...
	_loadConfig("cfxLogging_29_LIN")														    ; Load Configuration

	_ConfigureLINLog("LOG_29_Sample","System","1","Decimal","Overwrite","","","True")		    ; Configure Logging details
...
if $finddatacount5=$data1 and $finddatacount6=$data2 and $finddatacount7=$data3 and $totalCountlogFile=1 Then
	_WriteResult("Pass","TS_LIN_Log_29")
Else
	_WriteResult("Fail","TS_LIN_Log_29")
EndIf

$isAppNotRes=_CloseApp()																		; Close the app

if $isAppNotRes=1 Then
	_WriteResult("Warning","TS_LIN_Log_29")
EndIf


ConsoleWrite("****End : TS_LIN_Log_29.au3****"&@CRLF)
ConsoleWrite(@CRLF)