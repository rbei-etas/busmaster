...
; Critical (C)		:		Y
; TestCase No.		:		TS_LIN_Log_15                                     Source : (TS_LIN_Log_15)
...
ConsoleWrite(@CRLF)
ConsoleWrite(@CRLF)
ConsoleWrite("****Start : TS_LIN_Log_31.au3****"&@CRLF)


_launchApp()

Local $EndOfLog=""
Local $filename = "\Log_31_Sample.log",$verfyCount = 0,$finddatacount=0,$finddatacount5=0,$finddatacount6=0,$finddatacount7=0,$cnt=0
local $totalCountlogFile = 0

if winexists($WIN_BUSMASTER) then

	_loadConfig("cfxLogging_31_LIN")															; Load Configuration

	_ConfigureLINLog("LOG_31_Sample","System","1","Decimal","Overwrite","","","True")		    ; Configure Logging details
...
if $EndOfLog="***[STOP LOGGING SESSION]***" and $finddatacount5=4 and $finddatacount6=4 And $finddatacount7=4 Then                                                   ; Check the last line of the lof file
	_WriteResult("Pass","TS_LIN_Log_31")
Else
	_WriteResult("Fail","TS_LIN_Log_31")
EndIf

$isAppNotRes=_CloseApp()															            ; Close the app

if $isAppNotRes=1 Then
	_WriteResult("Warning","TS_LIN_Log_31")
EndIf


ConsoleWrite("****End : TS_LIN_Log_31.au3****"&@CRLF)
ConsoleWrite(@CRLF)