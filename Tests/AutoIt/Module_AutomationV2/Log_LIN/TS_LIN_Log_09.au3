...
; Critical (C)		:		Y
; TestCase No.		:		TS_LIN_Log_09                                      Source : (TS_Log_013)
...
ConsoleWrite(@CRLF)
ConsoleWrite(@CRLF)
ConsoleWrite("****Start : TS_LIN_Log_09.au3****"&@CRLF)
...
if winexists($WIN_BUSMASTER) then
	...
	_loadConfig("cfxLogging_09_LIN")															; Load Configuration

	_ConfigureLINLog("LOG_09_Sample","System","1","Decimal","Overwrite","","","True")		    ; Configure Logging details
...
if $cnt>=27 Then
	$EndOfLog=FileReadLine ($GetLogFile_Path & "\LOG_09_Sample.log",$cnt)						    ; Read the 17th line from the Log file
...
if $EndOfLog="***[STOP LOGGING SESSION]***" and $finddatacount=16 Then                                                ; Check the last line of the log file
	_WriteResult("Pass","TS_LIN_Log_09")
Else
	_WriteResult("Fail","TS_LIN_Log_09")
EndIf

$isAppNotRes=_CloseApp()															            ; Close the app

if $isAppNotRes=1 Then
	_WriteResult("Warning","TS_LIN_Log_09")
EndIf


ConsoleWrite("****End : TS_LIN_Log_09.au3****"&@CRLF)
ConsoleWrite(@CRLF)