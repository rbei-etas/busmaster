...
; Critical (C)		:		Y
; TestCase No.		:		TS_LIN_Log_15                                     Source : (TS_Log_013)
...
ConsoleWrite(@CRLF)
ConsoleWrite(@CRLF)
ConsoleWrite("****Start : TS_LIN_Log_15.au3****"&@CRLF)
...
Local $filename = "\Log_15_Sample.log",$verfyCount = 0,$finddatacount=0,$finddatacount1=0,$finddatacount2=0,$finddatacount8=0,$cnt=0


Local $EndOfLog=""
if winexists($WIN_BUSMASTER) then

	_loadConfig("cfxLogging_15_LIN")															; Load Configuration

	_ConfigureLINLog("LOG_15_Sample","System","1","Decimal","Overwrite","","","True")		    ; Configure Logging details
...
if $EndOfLog="***[STOP LOGGING SESSION]***" and $finddatacount1=8 and $finddatacount1=8 and $finddatacount1=8 Then                                                ; Check the last line of the lof file
	_WriteResult("Pass","TS_LIN_Log_15")
Else
	_WriteResult("Fail","TS_LIN_Log_15")
EndIf

$isAppNotRes=_CloseApp()															            ; Close the app

if $isAppNotRes=1 Then
	_WriteResult("Warning","TS_LIN_Log_15")
EndIf


ConsoleWrite("****End : TS_LIN_Log_15.au3****"&@CRLF)
ConsoleWrite(@CRLF)