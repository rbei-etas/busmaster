...
ConsoleWrite(@CRLF)
ConsoleWrite(@CRLF)
ConsoleWrite("****Start : TS_Log_24.au3****"&@CRLF)
...
if winexists($WIN_BUSMASTER) then
	_loadConfig("cfxLog_24")															; Load Configuration

	_ConfigureCANLog("LOG_24_Sample","System","ALL","Decimal","Overwrite","","","True")	; Configure Logging details
...
if _FileCountLines($GetLogFile_Path & "\Log_24_Sample.log")=17 Then
	$EndOfLog=FileReadLine ($GetLogFile_Path & "\LOG_24_Sample.log",17)						; Read the 17th line from the Log file
	consolewrite("$EndOfLog :"&$EndOfLog&@CRLF)
EndIf

if $EndOfLog="***[STOP LOGGING SESSION]***" Then
	_WriteResult("Pass","TS_Log_24")
Else
	_WriteResult("Fail","TS_Log_24")
EndIf

$isAppNotRes=_CloseApp()															; Close the app

if $isAppNotRes=1 Then
	_WriteResult("Warning","TS_Log_24")
EndIf

ConsoleWrite("****End : TS_Log_24.au3****"&@CRLF)
ConsoleWrite(@CRLF)