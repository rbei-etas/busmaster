...
ConsoleWrite(@CRLF)
ConsoleWrite(@CRLF)
ConsoleWrite("****Start : TS_Replay_19.au3****"&@CRLF)
...
if winexists($WIN_BUSMASTER) then
	...
	_loadConfig("cfxReplay_19")																	; Load Configuration

	_CANReplayMenu()																			; Open configure replay dialog

	_ADDReplayFile("Replay_19")																	; Add a replay file
	sleep(1000)
...
;~ 	_EnableHex()																				; Enable Hex mode
	...
	if $FirstMsg[2]=1 and $SecondMsg[2]=2 and $ThirdMsg[2]=1 and $FourthMsg[2]=2 Then
		$MsgDir=1
	EndIf
	...
	if $MsgIDs=1 and $MsgDir=1 Then
		_WriteResult("Pass","TS_Replay_19")
	Else
		_WriteResult("Fail","TS_Replay_19")
	EndIf
...
if $isAppNotRes=1 Then
	_WriteResult("Warning","TS_Replay_19")
EndIf


ConsoleWrite("****End : TS_Replay_19.au3****"&@CRLF)
ConsoleWrite(@CRLF)
