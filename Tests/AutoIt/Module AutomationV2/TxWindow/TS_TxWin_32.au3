...
; Critical (C)		:		Y
; TestCase No.		:		TS_TxWin_32
; TestCases			:		"Delete All" functionality using Right Click on a message item in Tx Message list
...
;==================================================Test Procedure =======================================================
ConsoleWrite(@CRLF)
ConsoleWrite("****Start : TS_TxWin_32.au3****"&@CRLF)
    _launchApp()
	...
	if winexists($WIN_BUSMASTER) Then
	    ...
	    sleep(500)
	    Send("{A}")                                                                                          ;click 'D' to delete block
	    If WinExists($WIN_BUSMASTER) Then
			$WinExists=1
		...
		If($WinExists=1 And $count=0) Then
			_WriteResult("Pass","TS_Tx_032")
		Else
		   _WriteResult("Fail","TS_Tx_032")
	    EndIf
    ...
    if $isAppNotRes=1 Then
		_WriteResult("Warning","TS_Tx_032")
    EndIf

ConsoleWrite("****End : TS_TxWin_32.au3****"&@CRLF)
ConsoleWrite(@CRLF)
