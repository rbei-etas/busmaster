...
ConsoleWrite("****Start : TC_UC7_03.au3****"&@CRLF)
_launchApp()
if $app=0 Then
	_loadConfig("UseCase4")
	if $crashRes=1 Then
		_writeCrashRes(37,10)
	Endif
...
if $channelCount=2 and $sel=1 and $gLine=1 Then
	_ExcelWriteCell($oExcel, "Pass", 37, 10)
Else
	_ExcelWriteCell($oExcel, "Fail", 37, 10)
EndIf
ConsoleWrite("****End : TC_UC7_03.au3****"&@CRLF)
ConsoleWrite(@CRLF)
ConsoleWrite(@CRLF)