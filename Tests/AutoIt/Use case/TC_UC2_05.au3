...
if $itemCount=4 and $time=1 and $transRec=1 then
	_ExcelWriteCell($oExcel, "Pass", 15, 10)
elseif $itemCount=4 and $time=0 and $transRec=1 then
	_ExcelWriteCell($oExcel, "Fail", 15, 10)
	_ExcelWriteCell($oExcel, "Sent and received time are incorrect!!!known issue", 15, 9)
Elseif $transRec=0 then
	_ExcelWriteCell($oExcel, "Fail", 15, 10)
endif
