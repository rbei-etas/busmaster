...
if WinExists("BUSMASTER") Then
	...
	_loadConfig("J1939SiSysTest")																; load cfx file
	_loadJ1939Database("SimJ1939Test",25,10,8)													; load dbf file
	send("!jdc")
	_associateDB("&J1939","SimJ1939Test.dbf")													; associate dbf
	sleep(2000)
	$cntToolhWd=ControlGetHandle("BUSMASTER","",128)
	_GUICtrlToolbar_ClickIndex($cntToolhWd,4)													; Connect
	sleep(1000)
	send("!JA")																					; Select menu : J1939->activate
	sleep(1000)
	WinMenuSelectItem("BUSMASTER","","&J1939","&Configure","&Simulated Systems")   				; select Configure->Simulated Systems
	$pos=ControlGetPos("BUSMASTER","","[CLASS:SysTreeView32; INSTANCE:1]")
	MouseClick("right",$pos[0]+30,$pos[1]+55)							 						; select new sim
	send("n")
	send(@DesktopDir&"\testJ1939Sim.sim")
	send("{ENTER}")
	sleep(1000)
	...
	if WinExists("BUSMASTER") Then
		...
		if WinExists("Select BUSMASTER Source Filename...") Then
			sleep(1000)
			send(@DesktopDir&"\addJ1939Handlers")
			send("{ENTER}")
		EndIf
;
...
;~ 		send("^s")
		...
		if winexists("Add Message Handler") then										; enter the msg contents
			controlsettext("Add Message Handler","",1155,25)							; Enter PGN ID
		...
		sleep(1000)
		if winexists("Add Message Handler") then										; enter the msg contents
			controlcommand("Add Message Handler","",1172,"Check")						; Check PGN ID range radio button
			ControlSetText("Add Message Handler","",1160,25)							; Enter PGN ID range 'From'
			ControlSetText("Add Message Handler","",1161,26)							; Enter PGN ID range 'To'
			controlclick("Add Message Handler","",1167)									; Click 'OK'
		EndIf
		$item1HWD=_GUICtrlTreeView_GetNext($nodeHWD,"")									; Get handle of first item
		$item2HWD=_GUICtrlTreeView_GetNextSibling($nodeHWD,$item1HWD)					; Get handle of second item
		...
		send("a")																		; Add a new msg
		sleep(1000)
		if winexists("Add Message Handler") then										; enter the PGN contents
			controlcommand("Add Message Handler","",1173,"Check")						; Check 'All Rx PGN' radio button
			controlclick("Add Message Handler","",1167)									; Click 'OK'
		EndIf
		$item1HWD=_GUICtrlTreeView_GetNext($nodeHWD,"")									; Get handle of first item
		$item2HWD=_GUICtrlTreeView_GetNextSibling($nodeHWD,$item1HWD)					; Get handle of second item
		...
		if winexists("Add Message Handler") then										; enter the msg contents
			...
			controlcommand("Add Message Handler","",1170,"Check")						; Check 'PGN Name' radio button
...
;~ 		ControlSend("BUSMASTER","",59664,"Trace"&"("&Chr(34)&"Time Check Handler"&Chr(34)&");")
		...
		sleep(1000)
		if WinExists("Add Event Handler") Then
			ControlCommand("Add Event Handler","",4007,"Check")							; select Data confirmation event
			ControlCommand("Add Event Handler","",4008,"Check")							; select error passive
			controlclick("Add Event Handler","",1)										;click 'OK'
		EndIf
		send("^s")
		sleep(2000)
		$item1HWD=_GUICtrlTreeView_GetNext($nodeHWD,"")
...
;~ 		ControlSend("BUSMASTER","",59664,"return sum;"&@CRLF)
	...
	_GUICtrlTreeView_ClickItem($node1HWD,$item1HWD,"Right")						; Right click on sim file
	sleep(1000)
	send("b")																	; Select 'Build Aldl' from the context menu
	sleep(1000)
	_GUICtrlToolbar_ClickIndex($cntToolhWd,4)									; Connect
	$item1HWD=_GUICtrlTreeView_GetNext($node1HWD,"")
	_GUICtrlTreeView_ClickItem($node1HWD,$item1HWD,"Right")						; Right click on sim file
