...
void CNetworkStatistics::vSendMessage(ETYPE_BUS eBus)
{
	switch(eBus)
	{
	case CAN:
		m_omStatistics[CAN]->SendMessage(WM_DISPLAY_MESSAGE, 1, 0);
		break;
	case LIN:
		m_omStatistics[LIN]->SendMessage(WM_DISPLAY_MESSAGE, 1, 0);
		break;
	}
}
