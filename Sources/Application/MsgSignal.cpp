...
BOOL CMsgSignal::bFillDataStructureFromDatabaseFile( CString strFileName, eProtocol eProtocolName)
{
                    ...
                    if ( nIndex != -1 )
                    {
                        ...
                        if(omstrDatabaseProtocol.IsEmpty() == FALSE)
                        {
                            ...
                            if(eProtocolName == PROTOCOL_CAN)
                            {
                                ...
                                {
                                    strcpy(s_acTraceStr, strFileName + " is not created for CAN. Please load CAN related dbf file.");
                                    vWriteTextToTrace();//(WM_WRITE_TO_TRACE, 0, (LPARAM)s_acTraceStr);
                                    return FALSE;
                                }
                            }
                            ...
                            else if(eProtocolName == PROTOCOL_J1939)
                            {
                                ...
                                {
                                    strcpy(s_acTraceStr, strFileName + " is not created for J1939. Please load J1939 related dbf file.");
                                    vWriteTextToTrace();//(WM_WRITE_TO_TRACE, 0, (LPARAM)s_acTraceStr);
                                    return FALSE;
                                }
                            }
    ...
    if ( bReturnValue == FALSE )
    {
        ...
        if(bIsMessageLengthExceeds == FALSE)
        {
            strFileName += " Is Invalid DBF File";
        }
        else
        {
            strFileName += " is not created for CAN. Since one of the message length exceeds 8 bytes.";
        }
        //Tobias- venkat
...
BOOL CMsgSignal::bDeleteMsg( CString omStrMsgToBeDeleted )
{
    ...
    else
    {
        ...
        else
        {
            if (!m_bAutoServerMode)
            {
                strcpy_s(s_acTraceStr, 1024, T2A(MSG_MSG_NOT_FOUND));
                vWriteTextToTrace();//(WM_WRITE_TO_TRACE, 0, (LPARAM)s_acTraceStr);
            }
            bReturnValue = FALSE;
        }
...
BOOL CMsgSignal::bIsDuplicateSignalName( CString strMsgName, CString strSgName)
{
    ...
    else
    {
        if (!m_bAutoServerMode)
        {
            strcpy_s(s_acTraceStr, 1024, "Insufficient Data");
            vWriteTextToTrace();//(WM_WRITE_TO_TRACE, 0, (LPARAM)s_acTraceStr);
        }
    }
    return (bReturnValue);
...
BOOL CMsgSignal::bIsDuplicateMessageCode( CString omStrMsgName, UINT unMsgCode)
{
    ...
    else
    {
        if (!m_bAutoServerMode)
        {
            strcpy_s(s_acTraceStr, 1024, "Invalid message code");
            vWriteTextToTrace();//(WM_WRITE_TO_TRACE, 0, (LPARAM)s_acTraceStr);
        }
    }
    return (bReturnValue);
                    ...
                    while ( p_Sg != NULL && bRetVal == FALSE )
                    {
                        ...
                        else
                        {
                            ...
                            else // bool or unsigned int
                            {
                                ...
                                if ( (un64PossibleSgDescMaxVal - un64ItemCount) > 0 )
                                {
                                    bRetVal = TRUE;
                                }
                            }
