...
//sLINBUSSTATTISTICSDATA CEthernetBusStatisticsDlg::sm_sBusSerializationData;

//xmlNodePtr CEthernetBusStatisticsDlg::m_pxmlNodePtr = NULL;

CEthernetBusStatisticsDlg::CEthernetBusStatisticsDlg(CBaseBusStatisticEthernet* pouBEthernet,CWnd* pParent,int nChannelCount)
    : CCommonStatistics(CEthernetBusStatisticsDlg::IDD, LIN),
      m_pouBSEthernet (pouBEthernet),
      m_omStrBusLoad(""),
      m_omStrPeakBusLoad(""),
      m_omStrAvgBusLoad( "")

{
    ...
    {
		SBUSSTATISTICS_Ethernet sBusStatistics;
        m_pouBSEthernet->BSL_GetBusStatistics(nChannel, sBusStatistics);
    }
}

void CEthernetBusStatisticsDlg::DoDataExchange(CDataExchange* pDX)
{
...
IMPLEMENT_DYNAMIC(CEthernetBusStatisticsDlg, CCommonStatistics)
BEGIN_MESSAGE_MAP(CEthernetBusStatisticsDlg, CCommonStatistics)
...
LRESULT CEthernetBusStatisticsDlg::vUpdateFields(WPARAM wParam, LPARAM lParam)
{
...
BOOL CEthernetBusStatisticsDlg::OnInitDialog()
{
    ...
    nIndex++;
    // Total Message Rate
    m_omStatList.InsertItem(nIndex, _("Messages/sec") );
    m_omStatList.SetItemText(nIndex, 1, omStrInitValue);
    m_omStatList.SetItemData(nIndex, nItemColor);
    nIndex++;
    // Total Errors
    ...
    nIndex++;
    // Network Load
    m_omStatList.InsertItem(nIndex, _(defSTR_PARAMETER_MSG_RATE_Ethernet) );
    m_omStatList.SetItemText(nIndex, 1, omStrInitValue);
    m_omStatList.SetItemData(nIndex, nItemColor);
    nIndex++;
    // Peak network load
    m_omStatList.InsertItem(nIndex, _(defSTR_PARAMETER_PEAK_LOAD_Ethernet) );
    m_omStatList.SetItemText(nIndex, 1, omStrInitValue);
    m_omStatList.SetItemData(nIndex, nItemColor);
    nIndex++;
    // Insert Avarage Bus Load
    m_omStatList.InsertItem(nIndex, _(defSTR_PARAMETER_AVG_LOAD_Ethernet) );
    m_omStatList.SetItemText(nIndex, 1, omStrInitValue);
    m_omStatList.SetItemData(nIndex, nItemColor);
    nIndex++;
...
BOOL CEthernetBusStatisticsDlg::OnEraseBkgnd(CDC* /*pDC*/)
{
    return TRUE;
}


void CEthernetBusStatisticsDlg::OnSize(UINT nType, int cx, int cy)
{
