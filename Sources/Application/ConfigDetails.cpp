        ...
        try
        {
            ...
            if (oCfgArchive.IsLoading())
            {
                ...
                else
                {
                    ...
                    oCfgArchive >> m_sToolBarButtonStatus.m_byMsgFilterLin;
...
static BOOL bLoadFilterDets(CArchive& romArchive, SFILTERAPPLIED_CAN& sFilterConfigured)
{
    ...
    for( int nIndex = 0; nIndex < nSize; nIndex++ )
    {
        ...
        sFilterConfigured.m_psFilters[nIndex].m_sFilterName.m_acFilterName = omStrFilterName.GetBuffer(LENGTH_FILTERNAME);
