...
#include "Application/MainFrm.h"            /* derka */
...
USAGEMODE HRESULT TS_vShowTSEditorWindow(void* pParentWnd)
{
    ...
    if( g_pomTSEditorChildWindow == NULL )
    {
        ...
        if( g_pomTSEditorChildWindow != NULL )
        {
            ...
            if(pParentWnd != NULL)
            {
                CMainFrame* pInt = static_cast<CMainFrame*>(pParentWnd);                    /* derka: cast from void* to CMainFrame* */
                g_pomTSEditorChildWindow->vSetNumberOfActiveChannels(pInt->m_nNumChannels); /* derka: store number of used channels, to show used channels only */
            }
