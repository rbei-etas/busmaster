...
void CLINScheduleDataStore::vSetBusStatus(ESTATUS_BUS eBusStatus)
{
    ...
    if ( eBusStatus == BUS_PRECONNECT && m_pouDIL_LIN_Interface != NULL )
    {
        ...
        {
            if ( m_ouClusterConfig->m_ouFlexChannelConfig[i].m_ouLinParams.m_bIsMasterMode == TRUE )
            {
...
HRESULT CLINScheduleDataStore::hGetConfigData(xmlNodePtr pNodePtr)
{
    ...
    //Channel Messages;
    for ( int nIndex = 0; nIndex < m_ouClusterConfig->m_nChannelsConfigured; nIndex++ )
    {
...
HRESULT CLINScheduleDataStore::hSetClusterConfig(ClusterConfig* objLINClusterConfig)
{
    for( int i= 0 ; i < objLINClusterConfig->m_nChannelsConfigured; i++ )
    {
        bool bTableFound = bAddTableListToList( m_ouTableData[i].m_strCurrentTable, m_ouTableData[i].m_ouScheduleTables , objLINClusterConfig->m_ouFlexChannelConfig[i].m_ouLinParams.ouLinParams);
...
int CLINScheduleDataStore::nTransmitScheduleTable(int nChannelIndex, int nTableIndex, bool bEnable )
{
    ...
    if ( nHandle != -1 && m_ouClusterConfig->m_ouFlexChannelConfig[nChannelIndex].m_ouLinParams.m_bIsMasterMode == TRUE)
    {
...
ClusterConfig* CLINScheduleDataStore::pGetClusterConfig()
{
