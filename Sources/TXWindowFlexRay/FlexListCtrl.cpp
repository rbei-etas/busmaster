...
******************************************************************************/
CFlexListCtrl::CFlexListCtrl() : m_bSingleClickActivate(TRUE)
{
...
void CFlexListCtrl::OnEndlabeledit(NMHDR* pNMHDR, LRESULT* pResult)
{
            ...
            plvItem->pszText != NULL)       // valid text
    {
        ...
        {
            unVal = strtoul(plvItem->pszText, NULL, 16);
            nOldVal  = strtoul(str, NULL, 16);
            cs = "00";
        }
        if ( plvItem->state == BASE_DECIMAL )
        {
            unVal = strtoul(plvItem->pszText, NULL, 10);
            nOldVal  = strtoul(str, NULL, 10);
            cs = "000";
        }
        ...
        else
        {
            if( plvItem->iSubItem != 0)
            {
                ...
                cs = GetItemText( plvItem->iItem,plvItem->iSubItem);
                SetItemText( plvItem->iItem, plvItem->iSubItem, cs);
                CWnd* wndParent = GetParent()->GetParent()->GetParent();
            }
        }
