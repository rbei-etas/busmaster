...
static INT bClassifyMsgType(XLevent& xlEvent, STLINDATA& sLinData)
{
    ...
    if (sLinData.m_uDataInfo.m_sLINMsg.m_ucChannel > defNO_OF_CHANNELS)
    {
        sLinData.m_uDataInfo.m_sLINMsg.m_ucChannel = defNO_OF_CHANNELS;
    }
...
static int nGetNoOfConnectedHardware(void)
{
    ...
    if (XL_SUCCESS == xlStatus)
    {
        ...
        if (!nResult)
        {
            _tcscpy(sg_omErrStr, "No available channels found! (e.g. no LINcabs...)");
            xlStatus = XL_ERROR;
        }
    }
    else
    {
        _tcscpy(sg_omErrStr, "Problem Finding Device!");
        nResult = -1;
    }
...
static int nCreateMultipleHardwareNetwork(UINT unDefaultChannelCnt = 0)
{
    ...
    for (int nCount = 0; nCount < sg_ucNoOfHardware; nCount++)
    {
        ...
        sg_HardwareIntr[nCount].m_dwVendor = g_xlDrvConfig.channel[sg_HardwareIntr[sg_anSelectedItems[nCount]].m_dwIdInterface].serialNumber;
        /*_stprintf(acTempStr, _("SN: %d, Port ID: %d"), sg_HardwareIntr[nChannels].m_dwVendor,
                                                                sg_HardwareIntr[nChannels].m_dwIdInterface);*/
        sg_HardwareIntr[nCount].m_acDescription = g_xlDrvConfig.channel[sg_HardwareIntr[sg_anSelectedItems[nCount]].m_dwIdInterface].name;
...
HRESULT CDIL_LIN_VectorXL::LIN_ListHwInterfaces(INTERFACE_HW_LIST& asSelHwInterface, INT& nCount)
{
    ...
    if ( hResult == 0)
    {
        ...
        for (UINT i = 0; i < sg_nNoOfChannels; i++)
        {
            ...
            //sg_ControllerDetails[i]. = oss1.str();
...
HRESULT CDIL_LIN_VectorXL::SetConfigData(IBMNetWorkGetService* ouConfig)
{
    if ( nullptr == ouConfig )
    {
        return S_FALSE;
    ...
    for ( int i = 0 ; i < nChannels; i++ )
    {
        ...
        sg_aodChannels[i].m_unLINMode = ouChannelSettings.m_ouLINSettings.m_bIsMasterMode ? XL_LIN_MASTER:XL_LIN_SLAVE ;//XL_LIN_MASTER;
    }
...
{
    //VALIDATE_VALUE_RETURN_VAL(sg_bCurrState, STATE_PRE_CONNECT, ERR_IMPROPER_STATE);
    HRESULT hResult = S_FALSE;
    if (sg_bCurrState == STATE_CONNECTED || sg_bCurrState == STATE_PRE_CONNECT)
    {
        ...
        if (true)
        {
            if (sMessage.m_ucChannel <= sg_nNoOfChannels)
            {
                ...
                if (nWriteMessage(sMessage, 0) == S_OK)
                {
                    hResult = S_OK;
                }
...
* \date          07.10.2011 Created
*/
HRESULT CDIL_LIN_VectorXL::LIN_GetCurrStatus(STATUSMSG& StatusData)
{
...
* \date          07.10.2011 Created
*/
HRESULT CDIL_LIN_VectorXL::LIN_SetAppParams(HWND hWndOwner)
{
    return S_OK;
}
