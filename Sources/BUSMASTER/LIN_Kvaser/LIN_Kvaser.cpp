...
static int nCreateMultipleHardwareNetwork(UINT unDefaultChannelCnt = 0)
{
    ...
    for (int nCount = 0; nCount < sg_ucNoOfHardware; nCount++)
    {
        ...
        sprintf(sg_aodChannels[nCount].m_strName , "Kvaser - %s, Serial Number- %ld, Firmware- %s",
...
static int nCreateSingleHardwareNetwork()
{
    ...
    if(sg_HardwareIntr[0].m_acDescription.find(" LIN ",0)!=std::string::npos)
    {
        ...
        sprintf(sg_aodChannels[0].m_strName , "%s, Serial Number: %ld, Firmware: %s",
...
static int nWriteMessage(STLIN_MSG sMessage, DWORD /*dwClientID*/)
{
    ...
    if ((sMessage.m_ucChannel > 0) &&
            ...
            (sMessage.m_ucChannel <= CHANNEL_ALLOWED))
    {
        //Get channel mask
...
* \date          05.29.2015 Created
*/
HRESULT CDIL_LIN_Kvaser::LIN_GetCurrStatus(STATUSMSG& StatusData)
{
...
* \date          05.29.2015 Created
*/
HRESULT CDIL_LIN_Kvaser::LIN_GetConfiguration( SCONTROLLER_DETAILS_LIN psControllerConfig[], INT& nSize )
{
