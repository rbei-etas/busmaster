...
#define defLOGGGING_ON_CONNECT_TAG      "LogOnConnect"
...
HRESULT CFrameProcessor_Common::EnableLogOnConnect(bool enable)
{
    m_bEnableLogOnConnect = enable;
    return  S_OK;
}
HRESULT CFrameProcessor_Common::EnableLogging(bool bEnable)
{
    ...
    if (ushBlocks > 0)
    {
        ...
        for (USHORT i = 0; i < ushBlocks; i++)
        {
            ...
            if (bEnable)
            {
                pouCurrLogObj->bStartLogging(m_eBusType);
            }
            else
            {
...
{
    char data[10];
    sprintf_s(data, sizeof(data), "%d", m_bEnableLogOnConnect);
    xmlNewChild(pxmlNodePtr, nullptr, BAD_CAST defLOGGGING_ON_CONNECT_TAG, BAD_CAST data);
...
std::string CFrameProcessor_Common::getConfigPath()
{
    std::string xmlPath = "";
    if (m_eBusType == CAN)
    {
        xmlPath = "//BUSMASTER_CONFIGURATION/Module_Configuration/CAN_Log";
    }
    else if (m_eBusType == J1939)
    {
        xmlPath = "//BUSMASTER_CONFIGURATION/Module_Configuration/J1939_Log";
    }
    else if (m_eBusType == LIN)
    {
        xmlPath = "//BUSMASTER_CONFIGURATION/Module_Configuration/LIN_Log";
    }

    return xmlPath;
}
void CFrameProcessor_Common::parseLogEnabledStatus(xmlDocPtr pDoc)
{
    m_bEnableLogOnConnect = FALSE;
    ...
    std::string path = getConfigPath() + "/" + defLOGGGING_ON_CONNECT_TAG;
    pchXpath = (xmlChar*)path.c_str();
...
HRESULT CFrameProcessor_Common::SetConfigData( xmlDocPtr pDoc)
{
    if (bIsEditingON() == false)
    {
        return S_FALSE;
    ...
    parseLogEnabledStatus(pDoc);

    USHORT ushLogBlks = 0;
    xmlChar* pchXpath = nullptr;
    xmlXPathObjectPtr pXpathPtr = nullptr;
    std::string logBlockPath = getConfigPath()+"/Log_Block";
    pchXpath = (xmlChar*)logBlockPath.c_str();
...
bool CFrameProcessor_Common::IsLogOnConnectEnabled()
{
    return m_bEnableLogOnConnect;
}
