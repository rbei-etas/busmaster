...
HRESULT CDIL_ISOLAR_EVE_VLIN::LIN_ListHwInterfaces(INTERFACE_HW_LIST& sSelHwInterface, INT& nCount)
{
    for (UINT i = 0; i < CHANNEL_ALLOWED; i++)
    {
        ...
        sSelHwInterface[i].m_acNameInterface = "ISOLAR_EVE_LIN";
...
HRESULT CDIL_ISOLAR_EVE_VLIN::LIN_SetConfigData(IBMNetWorkGetService* ouConfig)
{
...
* \date          07.10.2011 Created
*/
HRESULT CDIL_ISOLAR_EVE_VLIN::LIN_GetCurrStatus(STATUSMSG& StatusData)
{
...
HRESULT CDIL_ISOLAR_EVE_VLIN::LIN_GetConfiguration( SCONTROLLER_DETAILS_LIN psControllerConfig[], INT& nSize )
{
    for ( int i = 0; i < 1; i++ )
    {
        psControllerConfig[i].m_strHwUri =  "ISOLAR EVE Channel 1";
        psControllerConfig[i].m_BaudRate = 9600;
...
* \date          07.10.2011 Created
*/
HRESULT CDIL_ISOLAR_EVE_VLIN::LIN_SetAppParams(HWND hWndOwner)
{
    return S_OK;
}
