...
static void InitCanSetupDlg(HWND hdlg, TMhsCanCfg *cfg)
{
	int nResIDRadioItem = 0;
	SetDlgItemText(hdlg, IDC_SNR_EDIT, cfg->m_CanSnrStr);
	SetDlgItemHex(hdlg, IDC_BTR_EDIT, HEX_WORD, cfg->m_CanBtrValue);

	cfg->m_bBitRateSelected = FillComboBox(hdlg, IDC_CAN_SPEED, CanSpeedTabStr, (const DWORD *) CanSpeedTab,
				 CanSpeedTabSize, cfg->m_CanSpeed);
...
static BOOL SaveCanSetup(HWND hdlg, TMhsCanCfg *cfg)
{
	short unBTRVal;
	UBTR0 sBtr0Reg;
	UBTR1 sBtr1Reg;	
	...
	GetDlgItemTextCpy(cfg->m_CanSnrStr, hdlg, IDC_SNR_EDIT, MHS_STR_SIZE);

	if ( g_bBTRRadioBtnChecked )
	{
		cfg->m_CanBtrValue = GetDlgItemHex(hdlg, IDC_BTR_EDIT);
		unBTRVal = cfg->m_CanBtrValue;
	...
	{
		cfg->m_CanBtrValue = 0;
		cfg->m_CanSpeed = GetComboBox(hdlg, IDC_CAN_SPEED);

		/* Validate baud rate selection */
		if ( cfg->m_CanSpeed > 1000 )
		{
...
int ShowCanSetupDlg(HINSTANCE hInstance, HWND hwnd, TMhsCanCfg *cfg)
{
CanCfg = cfg;
