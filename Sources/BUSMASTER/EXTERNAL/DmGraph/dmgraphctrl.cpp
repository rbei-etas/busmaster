...
void CDMGraphCtrl::PlotElementPoints(HDC hDC, CGraphElement* pElement, BOOL bOptimized)
{
    ...
    HPEN hPenSave = nullptr;
    if (m_pointPen != nullptr)
    {
        hPenSave = (HPEN)SelectObject(hDC, m_pointPen);
    }
    ...
    HBRUSH hBrushSave = nullptr;
    if (m_pointBrush != nullptr)
    {
        hBrushSave = (HBRUSH)SelectObject(hDC, m_pointBrush);
    }
...
STDMETHODIMP CDMGraphCtrl::PrintGraph()
{
    ...
    nScreenWidth = nScreenHeight = nWidth = nHeight = 0;
    HDC hdcScreen = GetDC();
...
void CDMGraphCtrl::Annotate(HDC hDC)
{
    ...
    for(i=0; i<m_pAnnotationList->GetSize(); i++)
    {
        ...
        if (m_annoBrush != nullptr)
        {
            hBrushSave = (HBRUSH)SelectObject(hDC, m_annoBrush);
        }
        ...
        if (
            ...
            PtInRect(&m_axisRect, topLeft) && (m_annoBrush != nullptr)
        )
        {
