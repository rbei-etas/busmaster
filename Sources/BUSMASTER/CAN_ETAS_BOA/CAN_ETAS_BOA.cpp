...
static void vInitializeControllerConfig(UINT nChannel)
{
    ...
    sg_asChannel[nChannel].m_OCI_CANConfig.canFDEnabled = false;   
...
HRESULT CDIL_CAN_ETAS_BOA::CAN_SetAppParams(HWND hWndOwner)
{
...
static BOOL bLoadDataFromContr(PSCONTROLLER_DETAILS pControllerDetails)
{
    ...
    if (pControllerDetails != nullptr)
    {
        ...
        for( INT i = 0; i < defNO_OF_CHANNELS; i++ )
        {
            ...
            sg_asChannel[i].m_OCI_CANConfig.canFDEnabled = false;
...
HRESULT CDIL_CAN_ETAS_BOA::CAN_GetCurrStatus(STATUSMSG& StatusData)
{
