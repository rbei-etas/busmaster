...
#define BOA_ERR_DRIVER_DISCONNECTED             (BOA_RESULT_FLAG_ERROR | BOA_RESULT_TYPE_COMMUNICATION | 1)
    ...
    /** The driver for an attached hardware device has detected a communication problem (in asynchromous communication,
    by event signaling or during a previous call to the same hardware) whilst the user is using SEQ_ExecXXXSequence(…)
    and is not able to execute your commands at this time due to the device still executing previous protocol commands */
