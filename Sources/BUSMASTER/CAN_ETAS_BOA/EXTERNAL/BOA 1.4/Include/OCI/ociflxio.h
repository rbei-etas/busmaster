...
#define OCI_FLEXRAY_RESERVED_INDICATOR          (1 << 4)
    ...
    */
    ...
    /**
        ...
        A transmit queue is required to send FlexRay frames.  FlexRay controllers support multiple transmit queues.

        <ol>
        <li>Any data written to a single queue will appear in order on the bus interface.<br>
            Data written to different queues may appear in a different order on the bus interface because different
            queues are not synchronized. <p>
        <li>Any transmit queue locks exclusive access to a single or multiple send buffers.<p>
        <li>When a queue accesses multiple send buffers, the queued data may be multiplexed to all allocated buffers
            to increase the usable bandwidth. The data has then still to be in order as described under point 1.<br>
            If the driver cannot guarantee this, it may not implement this optimization and may only use a subset of
            the given send buffers. In the worst case it may use only a single buffer to guarantee in order transmission.<p>
        <li>Send buffers are active by default. Buffers assigned to a transmit queue can be explicitly deactivated
            during runtime using an OCI_FlexRaySwitchMessage. <br>
            e.g. This is needed for serial protocols like XCP if the user wants to use the LPDU-ID of a specific buffer
            for reception instead of transmission for a certain time.<p>
        </ol>
      ...
      @return
       ...
       <tr> <td> @ref OCI_WARN_PARAM_ADAPTED </td>
            <td> When a queue accesses multiple send buffers, the queued data may be multiplexed to all allocated buffers
            to increase the usable bandwidth. The data has then still to be in order.<br>
            If the driver cannot guarantee that the data will be placed on the bus in order, it may not implement this optimization and may only use a subset of
            the given send buffers. If it does so it will return this warning.</td>
       </tr>
