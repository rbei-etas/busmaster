...
************************************************************************/
CExecuteManager::CExecuteManager(ETYPE_BUS eBus, CGlobalObj* pGlobalObj, CSimSysManager* pSimSysMgr) :
    m_psFirstNodeObject(nullptr),
    m_psLastNodeObject(nullptr),
    m_odSetResetTimer(eBus, pGlobalObj, this)
{
...
{
    // Stop Monitor Node file directory thread.
...
/*************************************************************************
    ...
    Date Created     :  19.12.05
**************************************************************************/
CExecuteManager& CExecuteManager::ouGetExecuteManager(ETYPE_BUS eBus, CGlobalObj* pGlobalObj)
{
    if (sm_pouManager[eBus] == nullptr)
    {
        sm_pouManager[eBus] = new CExecuteManager(eBus, pGlobalObj, &CSimSysManager::ouGetSimSysManager(eBus, pGlobalObj));
    }
...
/*************************************************************************
    ...
    Modification     :  Robin G.K.
                        21.10.2014, Removed setting of H_DLLLOADED flag.
**************************************************************************/
...
/***************************************************************************************
    ...
    Modification     :  Robin G.K.
                        21.10.14, Removed setting of H_TIMERBUTTON flag
                        because H_TIMERBUTTON is unused.
****************************************************************************************/
...
/**************************************************************************************
    Function Name    :  vManageBusEventHandler
    Input(s)         :  Bus Events
    Output           :
    Functionality    :  Execute bus event handler for the bus event
    Member of        :  CExecuteManager
    Author(s)        :  Anish kumar
    Date Created     :  19.12.05
***************************************************************************************/
...
/***************************************************************************************
    ...
    Modifications    :  Robin G.K.
                        21.10.14, Removed dependancy on unused EXMSG_HANDLER flag.
***************************************************************************************/
...
/**************************************************************************************
    ...
    Date Created     :  19.12.05
    Modifications    :  Robin G.K.
                        21.10.14, Removed dependancy on unused EXERROR_HANDLER flag.
***************************************************************************************/
void CExecuteManager::vManageOnErrorHandlerCAN(ERROR_STATE eErrorCode,SCAN_ERR sErrorVal, DWORD dwClientId)
{
    PSNODEOBJECT psTempNodeObject=m_psFirstNodeObject;
    while(psTempNodeObject!=nullptr)
    {
...
/**************************************************************************************
    ...
    Date Created     :  01.01.2010
    Modifications    :  Robin G.K.
                        21.10.14 Removed unused CSimSysDetView updation.
***************************************************************************************/
void CExecuteManager::vManageOnAddressClaimHandlerJ1939(DWORD dwClientId, BYTE byAddress)
{
    ...
    {
        //Since shRemoteLc indicates Receiver, compare the node LCN with it
        sNODEINFO sNodeInfo(J1939, m_pGlobalObj);
...
/*  Modification on  :  21.10.14, Added bDisplaySuccessful flag.              */
/*                                                                            */
/******************************************************************************/
BOOL CExecuteManager::bExecuteDllBuildLoad(PSNODEINFO psNodeInfo, BOOL bDisplaySuccessful)
{
    ...
    if(psNodeInfo!=nullptr)
    {
        ...
        if (!psNodeInfo->m_omStrCFileName.IsEmpty())
        {
            ...
            if(m_pouBuildProgram==nullptr)
            {
                m_pouBuildProgram = new CBuildProgram(m_eBus, m_pGlobalObj);
            }
            if(m_pouBuildProgram!=nullptr)
            {
                PSNODEOBJECT psTempNodeObject=m_psFirstNodeObject;
                ...
                if(bDLLLoaded == TRUE)
                {
                    ...
                    CExecuteFunc* m_pouExecuteFunc = new CExecuteFunc(m_eBus, m_pGlobalObj, omStrDLLName);
...
BOOL CExecuteManager::bExecuteDllBuild(PSNODEINFO psNodeInfo, BOOL bDisplaySuccessful)
{
    ...
    if(psNodeInfo!=nullptr)
    {
        if(!psNodeInfo->m_omStrCFileName.IsEmpty())
        {
            ...
            if(m_pouBuildProgram==nullptr)
            {
                m_pouBuildProgram = new CBuildProgram(m_eBus, m_pGlobalObj, this);
            }
            if(m_pouBuildProgram!=nullptr)
            {
...
/***************************************************************************************
    ...
    Date Created     :  19.12.05
    Modification     :  Robin G.K.
                        21.10.14, Removed displaying of files that are successfully unloaded.
***************************************************************************************/
...
CExecuteFunc* CExecuteManager::vCreateExecuteFunc(CString omStrDLLName)
{
    CExecuteFunc* m_pouExecuteFunc = new CExecuteFunc(m_eBus, m_pGlobalObj, omStrDLLName);

    return m_pouExecuteFunc;
}
...
/***************************************************************************************
    ...
    Date Created     :  19.12.05
    Modification     :  Robin G.K.
                        21.10.14, Added bDisplaySuccessful flag.
***************************************************************************************/
...
BOOL CExecuteManager::bExecuteDllLoad(PSNODEINFO psNodeInfo, BOOL bDisplaySuccessful)
{
    ...
    if(psNodeInfo!=nullptr)
    {
        ...
        {
            CExecuteFunc* m_pouExecuteFunc = vCreateExecuteFunc(omStrDLLName);
            //CExecuteFunc* m_pouExecuteFunc = new CExecuteFunc(m_eBus, m_pGlobalObj, omStrDLLName);
            // check for successfull memory allocation
            if(m_pouExecuteFunc != nullptr)
            {
...
/***************************************************************************************
    ...
    Date Created     :  19.12.05
    Modification     :  Robin G.K.
                        21.10.14, Removed unused EXMSG_HANDLER flag check.
***************************************************************************************/
...
/***************************************************************************************
    ...
    Input(s)         :  CExecuteFunc* m_pCexecuteFunc,BOOL bEnable
    Output           :
...
/***************************************************************************************
    ...
    Date Created     :
    Modification     :  Robin G.K.
                        21.10.14, Replaced unused PSSIMSYSINFO with CSimSysNodeInfo*
***************************************************************************************/
BOOL CExecuteManager::bDLLBuildAll()
{
    ...
    BOOL bSuccess = FALSE;
    int nFailure = 0;
    CSimSysNodeInfo* pSimSysNodeInfo = m_pSimSysMgr->pomGetSimSysNodeInfo();
    ...
    while(pTempNode != nullptr)
    {
        ...
        {
            if (pTempNode->m_sNodeInfo->m_eNodeState != NODE_REQ_CLEAN_BUILT)
            {
    ...
    if((nFailure == 0) || (nFailure != nNodeCount))
    {
        ...
        CSimSysTreeView* psSimSysTree = m_pSimSysMgr->podGetSimSysTreeView();//CSimSysManager::ouGetSimSysManager(m_eBus, m_pGlobalObj).podGetSimSysTreeView();
...
/***************************************************************************************
    ...
    Input(s)         :  CStringArray* pomStrErrorFiles
    Output           :  BOOL
...
BOOL CExecuteManager::bDLLBuildLoadAll(CStringArray* pomStrErrorFiles)
{
    ...
    vStopTimer();
    vClearOutputWnd();
    if (m_pGlobalObj->m_pEditorDocTemplate != nullptr)
    {
        m_pGlobalObj->m_pEditorDocTemplate->SaveAllModified();
    }
...
/**************************************************************************************
    ...
    Output           :  bool
    Functionality    :  Calls bSetNodeState of CSimSysTreeView to set the Node State
    Member of        :  CExecuteManager
    Author(s)        :  Robin G.K.
    Date Created     :  13.10.2015
    Requirement ID   :
***************************************************************************************/
bool CExecuteManager::bSetNodeState(const PSNODEINFO psNodeInfo, eNODE_STATE eNodeState)
{
    bool bReturn = false;
    CSimSysTreeView* psSimSysTree = m_pSimSysMgr->podGetSimSysTreeView(); //CSimSysManager::ouGetSimSysManager(m_eBus, m_pGlobalObj).podGetSimSysTreeView();
    if(psSimSysTree != nullptr)
    {
        bReturn = psSimSysTree->bSetNodeState(psNodeInfo,eNodeState);
    }
    return bReturn;
}