    ...
    for (UINT i = 0; i < BUS_TOTAL; i++)
    {
        if (CExecuteManager::bIsExist((ETYPE_BUS)i) == TRUE)
        {
            CExecuteFunc* pmCEexecuteFunc =
                CExecuteManager::ouGetExecuteManager((ETYPE_BUS)i, &CGlobalObj::ouGetObj((ETYPE_BUS)i)).pmGetNodeObject(hModule);

            //If handler is found
            if (pmCEexecuteFunc != nullptr)
            {
                CExecuteManager::ouGetExecuteManager((ETYPE_BUS)i, &CGlobalObj::ouGetObj((ETYPE_BUS)i)).vManageTimerThreads();
...
//Helper function to get Coding value
ICoding* GetCoding(int bus, unsigned long long unClusterId, const char* pchCodingName)
{
    ICoding* pCoding = nullptr;

    auto pBmNetwork = CGlobalObj::ouGetObj((ETYPE_BUS)bus).m_ouClusterConfig;
    std::string strChecksum;

    ICluster* pCluster = nullptr;
    if ( nullptr == pBmNetwork )
    {
        return nullptr;
    }
    int nChannels = 0;
    pBmNetwork->GetChannelCount((ETYPE_BUS)bus, nChannels);
    for (auto i = 0; i < nChannels; i++ )
    {
        pBmNetwork->GetDBService((ETYPE_BUS)bus, i, 0, &pCluster);
        if ( nullptr != pCluster )
        {
            pCluster->GetDBFileChecksum(strChecksum);
            auto unCheckSum = _strtoui64(strChecksum.c_str(), nullptr, 16);
            if ( unCheckSum == unClusterId )
            {
                break;
            }
        }
    ...
    if ( pCluster != nullptr )
    {
        pCluster->GetElementByName(eCodingElement, pchCodingName, reinterpret_cast<IElement**>(&pCoding));
    }
...
unsigned long long gdGetSignalRawValue( int bus, double dEngValue, unsigned long long unClusterId, const char* pchCodingName )
{
    ICoding* pCoding = GetCoding(bus, unClusterId, pchCodingName);
    unsigned long long nRawValue = dEngValue;
    if ( nullptr != pCoding )
    {
        pCoding->GetRawValue(dEngValue, nRawValue);
    }
    return nRawValue;
}
...
{
    ICoding* pCoding = GetCoding(nBus, unClusterId, pchCodingName);
...
HMODULE ghGetNodeDllHandler(char* pcNodeName)
{
    ...
    for (UINT i = 0; i < BUS_TOTAL; i++)
    {
        if (CExecuteManager::bIsExist((ETYPE_BUS)i) == TRUE)
        {
            hModule = CExecuteManager::ouGetExecuteManager((ETYPE_BUS)i, &CGlobalObj::ouGetObj((ETYPE_BUS)i)).hReturnDllHandle(omNode);
            if (hModule != nullptr)
            {
                i = BUS_TOTAL;
            }
...
unsigned int sg_TimeNow(ETYPE_BUS ebus)
{
    ...
    if (CGlobalObj::sm_hWndMDIParentFrame != nullptr)
    {
        ...
        CNodeSim* Result = nullptr; //TO DO NS
