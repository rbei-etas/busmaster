...
CSimSysManager::CSimSysManager(ETYPE_BUS eBus, CGlobalObj* pGlobalObj, CSimSysNodeInfo* pSimSysNodeInfo) //: m_ouSimSysNodeInfo(eBus, pGlobalObj)
{
    ...
    m_ouSimSysNodeInfo = *pSimSysNodeInfo;
}
...
CSimSysManager& CSimSysManager::ouGetSimSysManager(ETYPE_BUS eBus, CGlobalObj* pGlobalObj)
{
    if (sm_pouSimSysManager[eBus] == nullptr)
    {
        sm_pouSimSysManager[eBus] = new CSimSysManager(eBus, pGlobalObj, new CSimSysNodeInfo(eBus, pGlobalObj));
    }
...
void CSimSysManager::pouSetSimSysNodeInfo(CSimSysNodeInfo* pSimSysNOdeInfo)
{
    m_ouSimSysNodeInfo = *pSimSysNOdeInfo;
}
...
{
    if (m_pGlobalObj->m_pomSimSysWnd != nullptr)
    {
        m_pGlobalObj->m_nSimSysDataSize = unGetStoreSIMFBufferSize(); //Get the required size

        if (m_pGlobalObj->m_pSimSysDataPtr != nullptr)
        {
            delete[] m_pGlobalObj->m_pSimSysDataPtr;
        }
        m_pGlobalObj->m_pSimSysDataPtr = new BYTE[m_pGlobalObj->m_nSimSysDataSize];
        if (m_pGlobalObj->m_pSimSysDataPtr != nullptr)
        {
            SaveSIMDataIntoBuffer(m_pGlobalObj->m_pSimSysDataPtr);// save the config into buffer
    ...
    else
    {
        if (m_pGlobalObj->m_pSimSysDataPtr == nullptr)
        {
            ...
            m_pGlobalObj->m_nSimSysDataSize = nTotalSize;
            m_pGlobalObj->m_pSimSysDataPtr = new BYTE[nTotalSize];
            BYTE* byTempData = m_pGlobalObj->m_pSimSysDataPtr;
            BYTE byVersion = 0x1;
            COPY_DATA(byTempData, &byVersion, sizeof(BYTE));
            m_pGlobalObj->bGetDefaultValue(SIMSYS_WND_PLACEMENT, m_pGlobalObj->m_wWindowPlacement);
            COPY_DATA(byTempData, &(m_pGlobalObj->m_wWindowPlacement), sizeof(WINDOWPLACEMENT));
...
bool CSimSysManager::bGetConfigData(xmlNodePtr pNodePtr)
{
    ...
    if(m_pGlobalObj->m_pomSimSysWnd != nullptr)
    {
        m_pGlobalObj->m_pomSimSysWnd->GetWindowPlacement(&sWndPlacement);
        xmlUtils::CreateXMLNodeFrmWindowsPlacement(pNodeWindowsPos,sWndPlacement);
    }
    else if(m_pGlobalObj->m_pomSimSysWnd == nullptr)
    {
        xmlUtils::CreateXMLNodeFrmWindowsPlacement(pNodeWindowsPos,m_pGlobalObj->m_wWindowPlacement);
    }
...
/******************************************************************************
    ...
    Modification     :  Robin G.K.
                        21.10.14, Removed saving of Node Info.
/*****************************************************************************/
void CSimSysManager::SaveSIMDataIntoBuffer(BYTE* DesBuffer)
{
    ...
    m_pGlobalObj->m_pomSimSysWnd->GetWindowPlacement(&WndPlacement);
...
void CSimSysManager::vSetConfigData(xmlNodePtr pNode)
{
    ...
    if( nullptr != pNode )
    {
        ...
        while (pNode != nullptr)
        {
            ...
            {
                nWndPos =  xmlUtils::ParseWindowsPlacement(pNode, m_pGlobalObj->m_wWindowPlacement);
            }
...
void CSimSysManager :: vLoadSimSysWndConfig()
{
    ...
    {
        /* Copy the retrieved config data into member variables and apply*/
        CopySIMDataFromBuffer(m_pGlobalObj->m_pSimSysDataPtr);
        //delete SrcBuffer;
    }
    //Now Populate the tree view if it is present
    if (m_pomSimSysTreeView != nullptr)
...
void CSimSysManager :: CopySIMDataFromBuffer(BYTE* SrcBuffer)
{
    ...
    COPY_DATA_2(&m_pGlobalObj->m_wWindowPlacement, tempBuffAddress, sizeof(WINDOWPLACEMENT));
    UINT UnCount = 0;
...
{
    if ( nullptr != m_pGlobalObj->m_ouClusterConfig )
    {
        NodeSimCodeGenerator ouCodeGenrator;
        return ouCodeGenrator.GenerateMakeFile(m_pGlobalObj->m_ouClusterConfig, m_eBus, strCppFile);
    }
    return S_OK;
}
...
HRESULT CSimSysManager::GenerateMakeFile()
{
    if ( nullptr != m_pGlobalObj->m_ouClusterConfig )
    {
        ...
        {
            NodeSimCodeGenerator ouCodeGenrator;
            return ouCodeGenrator.GenerateMakeFile(m_pGlobalObj->m_ouClusterConfig, m_eBus, pNodeList->m_sNodeInfo->m_omStrCFileName.GetBuffer(0));
...
sNODEINFO* CSimSysManager::vCreateBusSpecificNodeInfoPtr()
{
    return new sNODEINFO(m_eBus, m_pGlobalObj);
}

void CSimSysManager::vAddNodeToBusSpecificNodeList(PSNODELIST psNodeList)
{
    m_ouSimSysNodeInfo.vAddNodeToList(psNodeList);
}
...
{
    PSNODELIST pTempNode = new SNODELIST(m_eBus, m_pGlobalObj);
    //sNODEINFO *ousNodeInfo = new sNODEINFO(m_eBus, m_pGlobalObj);
    sNODEINFO* ousNodeInfo = vCreateBusSpecificNodeInfoPtr();
    pTempNode->vCreateNodeInfoPtr(ousNodeInfo);
    ...
    if(pTempNode != nullptr)
    {
        ...
        vAddNodeToBusSpecificNodeList(pTempNode);
        m_pGlobalObj->RegisterNodeToDIL(TRUE, (pTempNode->m_sNodeInfo));
...
CSimSysNodeInfo CSimSysManager::vGetBusSpecificSimSysNodeInfo()
{
    return m_ouSimSysNodeInfo;
}
...
/******************************************************************************
    ...
    Date Created     :  19.02.2009
    Modification     :  Robin G.K.
                        21.10.14,Removed modified check on Simulated Sytem.
/*****************************************************************************/
BOOL CSimSysManager :: bIsConfigChanged()
{
    ...
    if (m_pGlobalObj->m_pSimSysDataPtr != nullptr)
    {
        BYTE* tempBuffAddress = m_pGlobalObj->m_pSimSysDataPtr;
        //
        UINT unCount = 0;
...
void CSimSysManager::vSetDatabaseConfiguration(IBMNetWorkGetService* ouLINConfig, bool bModified)
{

    m_pGlobalObj->m_ouClusterConfig = ouLINConfig;
    GenerateMakeFile();
    if (true == bModified)
    {
        auto pTempNode = m_ouSimSysNodeInfo.m_psNodesList;
        while (pTempNode != nullptr)
        {
            pTempNode->m_sNodeInfo->m_eNodeState = NODE_REQ_CLEAN_BUILT;
            pTempNode = pTempNode->m_psNextNode;
    ...
    if ( nullptr != pCluster )
    {
        ...
        pCluster->GetDBFileChecksum(strChkSum);
        unsigned long long unCheckSum = _strtoui64(strChkSum.c_str(), nullptr, 16);
        auto pTempNode = m_ouSimSysNodeInfo.m_psNodesList;
        while(pTempNode != nullptr)
        {
            CString omStrFileName;
            if(pTempNode->m_sNodeInfo->m_ouLastDBChecksum != unCheckSum)
            {
                pTempNode->m_sNodeInfo->m_eNodeState = NODE_REQ_CLEAN_BUILT;
            }
            pTempNode = pTempNode->m_psNextNode;
...
void CSimSysManager::bUpdateNodeInfoFile(PSNODELIST psNodeList)
{
    CSimSysTreeView* pSimSysTreeView =
        CSimSysManager::ouGetSimSysManager(m_eBus, m_pGlobalObj).podGetSimSysTreeView();
    CSimSysNodeInfo* pSimSysNodeInf =
        CSimSysManager::ouGetSimSysManager(m_eBus, m_pGlobalObj).pomGetSimSysNodeInfo();
    CString omStrFileName;
    ...
    if(_tfindfirst( omStrCPPFile.GetBuffer(MAX_PATH), &fileinfo) != -1L)
    {
        ...
        GenerateMakeFile(omStrCPPFile.GetBuffer(0));
    }
