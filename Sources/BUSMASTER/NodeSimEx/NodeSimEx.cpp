                                  ...
                                  void** ppvInterface)
{
    ...
    switch (eBus)
    {
        case CAN:
        {
            ...
            {
                if ((sg_pouNS_CAN = new CNodeSim(CAN, &CGlobalObj::ouGetObj(eBus), &CExecuteManager::ouGetExecuteManager(eBus, &CGlobalObj::ouGetObj(eBus)))) == nullptr)
                {
                    ASSERT(false);
                    hResult = S_FALSE;
                }
...
if sg_pouFP_CAN is null */
        ...
        case J1939:
        {
            ...
            {
                if ((sg_pouNS_J1939 = new CNodeSim(J1939, &CGlobalObj::ouGetObj(eBus), &CExecuteManager::ouGetExecuteManager(eBus, &CGlobalObj::ouGetObj(eBus)))) == nullptr)
                {
                    ASSERT(false);
                    hResult = S_FALSE;
                }
...
if sg_pouFP_J1939 is null */
        ...
        case LIN:
        {
            ...
            {
                if ((sg_pouNS_LIN = new CNodeSim(LIN, &CGlobalObj::ouGetObj(eBus), &CExecuteManager::ouGetExecuteManager(eBus, &CGlobalObj::ouGetObj(eBus)))) == nullptr)
                {
                    ASSERT(false);
                    hResult = S_FALSE;
                }
