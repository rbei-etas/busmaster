...
void CMsgSignal::bAddDbNameEntry(const CString& omDbFileName)
{
    ...
    int nFileCount = m_omDatabaseNames.GetSize();
    for (int nCount = 0; (nCount < nFileCount) && (!bFilePresent); nCount++)
    {
        CString omstrFileName = m_omDatabaseNames.GetAt(nCount);
        if (!omstrFileName.Compare(omDbFileName))
        {
            bFilePresent = true;
        }
    }
        ...
        BOOL bDataFormat)
{
    ...
    if ( pMsg != nullptr )
    {
        ...
        bCalcBitMaskForSig( abyMaskByte, DATA_LENGTH_MAX, unByteNum, unStartBitValue, unSignalLength, ( eEndianess ) bDataFormat );
        ...
        if ( pMsg != nullptr )
        {
            ...
            while ( pSg != nullptr )
            {
                ...
                if ( pSg->m_omStrSignalName == omStrSignalName )
                {
                    ...
                    if ( pCurr != nullptr && pCurr->m_pouNextSignalSignalDescVal == nullptr )
                    {
                        pCurr = pSg->m_oSignalIDVal;
                ...
                else
                {
                    ...
                    if(p_Sg->m_bySignalType == CHAR_INT) // signed type
                    {
                        if(theApp.pouGetFlagsPtr()->nGetFlagStatus(HEX))
                        {
                            ...
                            n64SgVal = _strtoi64( omStrSignalValue, nullptr, 16 );
                        }
                        else
                        {
                    ...
                    else //unsigned / bool type
                    {
                        if(theApp.pouGetFlagsPtr()->nGetFlagStatus(HEX))
                        {
                            un64SgVal = _strtoui64( omStrSignalValue, nullptr, 16 );
                        }
                        else
                        {
                ...
                else
                {
                    ...
                    if(p_Sg->m_bySignalType == CHAR_INT) // signed type
                    {
                        ...
                        if(theApp.pouGetFlagsPtr()->nGetFlagStatus(HEX))
                        {
                            ...
                            n64TempValue = n64TempValue << (p_Sg->m_unSignalLength -1) ;
                            if(n64SgVal >= n64TempValue)
                            {
                                n64SgVal = n64SgVal - (2 * n64TempValue);
                            }
                        }
                        else
                        {
                ...
                if ( p_Sg->m_omStrSignalName == omStrSgName )
                {
                    if(p_Sg->m_bySignalType == CHAR_INT) // signed type
                    {
                        if(theApp.pouGetFlagsPtr()->nGetFlagStatus(HEX))
                        {
                            ...
                            __int64 n64Value = p_Sg->m_SignalMinValue.n64Value;
                            CUtilFunctions::s_vRemoveUnwantedBits(n64Value, p_Sg->m_unSignalLength);
                            omMinValue.Format(defFORMAT_INT64_HEX, n64Value);

                            omMaxValue.Format(defFORMAT_INT64_HEX,p_Sg->m_SignalMaxValue.n64Value);
                        }
                        else
                        {
                            //store valid sinal ranges
                    ...
                    else
                    {
                        if(theApp.pouGetFlagsPtr()->nGetFlagStatus(HEX))
                        {
                            omMinValue.Format(defFORMAT_INT64_HEX,p_Sg->m_SignalMinValue.un64Value);
                            omMaxValue.Format(defFORMAT_INT64_HEX,p_Sg->m_SignalMaxValue.un64Value);
                        }
                        else
                        {
