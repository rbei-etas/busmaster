...
extern "C" {
    ...
    */
    ...
    /**
     * This structure configures the operation of the hardware-based FIFO which is used to receive frames.
     *
     * Most fields of the structure are concerned with configuring the single hardware-based rejection filter
     * which applies to the FIFO. Frames which match this filter are rejected in hardware and processed no
     * further. Frames which are accepted by this filter are passed to the second-stage filters specified by
     * @ref OCI_AddFlexRayFrameFilter().
     */
    typedef struct OCI_FlexRayRxFifoConfiguration
    {
        /**
         * Payload length reserved for each FIFO entry. Should equal either
         * @ref OCI_FlexRayBusConfiguration::gPayloadLengthStatic or the largest
         * dynamic payload length in the cluster, whichever is greater.
         * Note that using a larger value may reduce the number of entries which can be accommodated in the FIFO.
         */
        uint32 payloadLength;

        /** This field, together with @ref rejectFrameIDMask, forms a (mask, value) pair. A frame ID
         * (i.e. slot number) matches the filter, and so is rejected, if
         * @code (<frame ID> & rejectFrameIDMask) == (<frame ID> & rejectFrameIDValue) @endcode
         *
         * If both @ref rejectFrameIDValue and @ref rejectFrameIDValue are zero, @em no frame IDs are rejected.
         */
        uint32 rejectFrameIDValue;

        /** See @ref rejectFrameIDValue. */
        ...
        /**
         * This field, together with @ref cycleOffset, specifies the set of cycle numbers to which the channel
         * and frame ID filter criteria apply (see @ref channel, @ref rejectFrameIDValue and @ref rejectFrameIDMask).
         * On cycles which are not members of the set defined by @ref cyclePeriod and @ref cycleOffset, @em all frames
         * are rejected.
         *
         * The value of @ref cyclePeriod must be a power of two between 1 and 64 inclusive.
         *
         * A cycle number is a member of the cycle set specified by @ref cyclePeriod and @ref cycleOffset if
         * @code <cycle number> % cyclePeriod == cycleOffset @endcode
         */
        uint32 cyclePeriod;

        /** See @ref cyclePeriod. Valid values of @ref cycleOffset are 0..CyclePeriod-1. */
        uint32 cycleOffset;

        /** If this value is non-zero, all frames in the static segment will be rejected. */
        uint32 rejectStaticSegment;

        /** If this value is non-zero, all null frames will be rejected. */
    ...
    typedef struct OCI_FlexRayAdvancedConfiguration
    {
        ...
        /**
        * Flags for the bus modes to be used by the controller. Any combination of values from @ref OCI_FlexRayControllerBusMode
        ...
        * Note that the controller may not support all possible bus modes. Use @ref OCI_GetFlexRayControllerCapabilities() to
        * discover the bus modes supported by the controller.
        */
        uint32 busMode;
     ...
     *
     * In case the OCI controller is used as a key slot node, i.e. OCI_FlexRayBusConfiguration::pKeySlotUsedForSync == true,
     * there must be a TX buffer in the list of OCI_FlexRayTxBufferConfiguration, which transmits on the slot ID specified by
     * parameter OCI_FlexRayBusConfiguration::pKeySlotId and which satisfies FlexRay key slot constraints, i.e. it must send
     * in every cycle of all connected channels.
     *
     * If a matching buffer for the parameter OCI_FlexRayBusConfiguration::pKeySlotId is missing in the list of
     * OCI_FlexRayTxBufferConfiguration the function OCI_OpenFlexRayController() will return @ref OCI_ERR_INCONSISTENT_PARAMETER_SET.
     */
     ...
     * in every cycle of all connected channels.
     *
     * In case the OCI controller is used in two key slot mode for starting up the bus, i.e.
     * (OCI_FlexRayBusConfiguration::pKeySlotUsedForSync == true) AND (OCI_FlexRayBusConfiguration::pKeySlotUsedForStartup == true)
     * AND (OCI_FlexRayAdvancedConfiguration includes the flag @ref OCI_FLEXRAY_BUSMODE_TWOKEYSLOTS), there must be two TX buffers
     * in the list of OCI_FlexRayTxBufferConfiguration, where one of them transmits on the slot ID specified by parameter
     * OCI_FlexRayBusConfiguration::pKeySlotId and the other one on the slot specified by
     * OCI_FlexRayBusConfigurationEx::pSecondKeySlotId. Both buffers must satisfy FlexRay key slot constraints.
     *
     * If matching buffers for the parameters OCI_FlexRayBusConfiguration::pKeySlotId or OCI_FlexRayBusConfigurationEx::pSecondKeySlotId
     * are missing in the list of OCI_FlexRayTxBufferConfiguration the function OCI_OpenFlexRayControllerEx() will return
     * @ref OCI_ERR_INCONSISTENT_PARAMETER_SET.
     *
     * The hardware buffer for the second key slot may be a buffer (depending on the hardware implementation details) which
     * cannot be used for actual data transmission, but solely for the transmission of NULL frames with asserted sync and
     * startup indicator flags. In such a case the corresponding (software/ virtual) buffer in the list of
     * OCI_FlexRayTxBufferConfiguration always has an owner in the sense of OCI_FlexRayTxBufferInfo::owner, i.e. it is always
     * in use and can therefore never be connected to an OCI FlexRay TX queue. Trying to do so anyway results in
     * the error @ref OCI_ERR_INVALID_PARAMETER from OCI_CreateFlexRayTxQueue().
     *
    ...
    /**
      ...
      @deprecated This function is deprecated, please use @ref OCI_CreateFlexRayControllerVersion instead.
    ...
    */
    ...
    /**
      Create a new FlexRay controller instance and bind it to the hardware identified by an URI location and a version.

      This is the API used by the application to bind a specific controller. The application can use information
      delivered by the CSI component to search for accessible controller. The CSI component will transform the
      (user friendly) URI-Information and version number from this call into a (system friendly) protocol stack.
      Any driver needs only to implement the OCI_CreateFlexRayControllerBind()-API call.
    ...
    */
...
#ifndef OCI_DYNAMICDLL
    OCI_DECLSPEC OCI_ErrorCode
    OCI_CALL OCI_CreateFlexRayControllerVersion(
        const char* uriLocation,
    ...
    /**
      ...
      This function is identical to @ref OCI_CreateFlexRayControllerVersion(), but, whereas @ref OCI_CreateFlexRayControllerVersion()
      searches for connected hardware every time it is invoked, @ref OCI_CreateFlexRayControllerNoSearch() does not
      ...
      several OCI_FLX API sessions by re-using the same search results.
      ...
      @return
      See @ref OCI_CreateFlexRayControllerVersion().
    */

#ifndef OCI_DYNAMICDLL
    OCI_DECLSPEC OCI_ErrorCode
    OCI_CALL OCI_CreateFlexRayControllerNoSearch(
        const char*             uriLocation,
        const BOA_Version*      apiVersion,
    ...
    */
    ...
    /**
      ...
      If the FlexRay controller is to be used as a key slot node, certain considerations apply to the value of
      @p configuration. These are discussed in the documentation for @ref OCI_FlexRayConfiguration.
      ...
      <TABLE border="0" rules="rows" cellspacing="3" cellpadding="3">
       ...
       <tr> <td> @ref OCI_ERR_INCONSISTENT_PARAMETER_SET </td>
            <td> This error may indicate that @p configuration contains incorrect configuration for a key slot node.
                 If the FlexRay controller is to be used as a key slot node, certain considerations apply to the value
                 of @p configuration. These are discussed in the documentation for @ref OCI_FlexRayConfiguration. </td>
       </tr>
      </TABLE>
    ...
    */
    ...
    OCI_CALL OCI_OpenFlexRayController(
        ...
        OCI_FlexRayControllerProperties* controllerProperties);
#endif
    ...
    /**
      ...
      If the FlexRay controller is to be used in two key slot mode for starting the bus, certain considerations apply to the
      value of @p configuration. These are discussed in the documentation for @ref OCI_FlexRayConfigurationEx.
     */
#ifndef OCI_DYNAMICDLL
    OCI_DECLSPEC OCI_ErrorCode
    ...
    */
    ...
    /**
      ...
      <TABLE border="0" rules="rows" cellspacing="3" cellpadding="3">
       ...
       <tr> <td> @ref OCI_ERR_LATER_VERSION_IN_USE </td>
            <td> Another client has already used @ref OCI_OpenFlexRayControllerEx() to configure the FlexRay controller,
                 and this client has specified a configuration which cannot be converted to an instance of
                 @ref OCI_FlexRayConfiguration (perhaps the other configuration uses @ref OCI_FLEXRAY_BUSMODE_TWOKEYSLOTS. </td>
       </tr>
      </TABLE>
    */
    ...
    */
    ...
    /**
      ...
      @param[in]  controllerProperties  FlexRay controller interface instance properties to set.
                                        Must not be @c NULL.
