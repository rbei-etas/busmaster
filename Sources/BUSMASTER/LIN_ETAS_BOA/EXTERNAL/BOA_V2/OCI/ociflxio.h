...
#define OCI_FLEXRAY_SYNC_FRAME_INDICATOR        (1 << 1)
    ...
    *
    * Note that the sense of the null frame indicator is reversed from what might be
    * expected. The indicator bit is set to 0 for a null frame, and 1 for a non-null
    * frame.
    */
...
#define OCI_FLEXRAY_RESERVED_INDICATOR          (1 << 4)
    ...
    typedef struct OCI_FlexRayRxMessage
    {
        ...
        *
        * Note that the sense of the @ref OCI_FLEXRAY_NULL_FRAME_INDICATOR flag is reversed from
        * what might be expected. The indicator bit is set to 0 for a null frame, and  1 for a
        * non-null frame.
        */
    ...
    */
     ...
     *
     * The buffer indicated by @ref bufferIndex must be assigned to the same
     * TX queue as is used to transmit this message. Buffers are assigned to
     * TX queues during @ref OCI_CreateFlexRayTxQueue().
     *
     * The buffer indicated by @ref bufferIndex must currently be active;
     * see @ref OCI_FlexRaySwitchMessage.
     *
    ...
    */
     ...
     * state.
     *
     * @ref OCI_WriteFlexRayData will return @ref OCI_ERR_INCONSISTENT_PARAMETER_SET
     * for invalid values in @ref bufferIndex.
    */
    ...
    /**
      ...
      <TABLE border="0" rules="rows" cellspacing="3" cellpadding="3">
       ...
       <tr> <td> @ref OCI_ERR_QUEUE_IS_FULL </td>
            <td> Either:
                    - The specified @a queue is full; at least one message could not be added to the queue.
                    - The payload length of at least one member of @p messages is larger than the size of the associated
                      TX buffer configured in OCI_FlexRayConfiguration::txBuffer[]
            </td>
       </tr>
