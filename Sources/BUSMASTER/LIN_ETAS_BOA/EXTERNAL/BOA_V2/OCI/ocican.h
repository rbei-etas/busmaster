...
#define     OCI_CAN_MEDIA_FAULT_TOLERANT        (2u)
    ...
    /**
    * This enum indicates various bus participation modes, which describe how a CAN controller interacts with the CAN bus.
    */

    typedef enum OCI_CANBusParticipationMode
    {
        /**
        * Specifies that the controller can only be used to listen/monitor on the communications bus. The controller will
        * not be able to transmit any CAN messages, and will not acknolwedge any CAN messages which it receives.
        */
        OCI_BUSMODE_PASSIVE = 1,

        /**
        * Specifies that the controller can be used to listen/monitor/acknowledge/transmit on the communications bus.
        * The controller will be able to transmit CAN messages, and will acknolwedge any CAN messages which it receives.
        */
        OCI_BUSMODE_ACTIVE = 2

    }
                         OCI_CANBusParticipationMode;
    ...
    */
    ...
    typedef struct OCI_CANConfiguration
    {
        ...
        */
        ...
        /**
         * This member specifies how the controller behaves on the CAN bus. Only values indicated by
         * @ref OCI_CANControllerCapabilities::canBusParticipationMode are valid.
         *
         * @version This member was added in v1.3 of OCI_CAN: do not attempt to access this member unless the current OCI_CAN
         * session uses at least v1.3.
         */
        OCI_CANBusParticipationMode busParticipationMode;

        /**
         * This member is a boolean which indicates whether the CAN-FD configuration @ref canFDConfig should be used.
         *
         * @version This member was added in v1.3 of OCI_CAN: do not attempt to access this member unless the current OCI_CAN
         * session uses at least v1.3.
         */
        ...
        /**
         * This member contains CAN-FD configuration. It is not valid unless @ref canFDEnabled is true.
         *
         * @version This member was added in v1.3 of OCI_CAN: do not attempt to access this member unless the current OCI_CAN
         * session uses at least v1.3.
         */
        OCI_CANFDConfiguration canFDConfig;
    ...
    */
    ...
    /**
      ...
      @deprecated This function is deprecated, please use @ref OCI_CreateCANControllerVersion instead.
      ...
      (user friendly) URI-Information from this call into a (system friendly) protocol stack. Any driver needs
      only to implement the OCI_CreateCANControllerBind()-API call.

      Note that this protocol stack will always implement CAN version 1.1. To select a different version, use the
      functions OCI_CreateCANControllerBind() or OCI_CreateCANControllerVersion().
    ...
    */
    ...
    /**
      ...
      to implement the OCI_CreateCANController Bind()-API call.
    ...
    */
#ifndef OCI_DYNAMICDLL
    OCI_DECLSPEC OCI_ErrorCode
    OCI_CALL OCI_CreateCANControllerVersion(
        const char*             uriLocation,
    ...
    /**
      Create a new CAN controller instance and bind it to the hardware identified by an URI location, but without
      searching for connected hardware.

      This function is identical to @ref OCI_CreateCANControllerVersion(), but, whereas @ref OCI_CreateCANControllerVersion()
      searches for connected hardware every time it is invoked, @ref OCI_CreateCANControllerNoSearch() does not
      ...
      several OCI_CAN API sessions by re-using the same search results.
      ...
      @return
      See @ref OCI_CreateCANControllerVersion().
    */
#ifndef OCI_DYNAMICDLL
    OCI_DECLSPEC OCI_ErrorCode
    OCI_CALL OCI_CreateCANControllerNoSearch(
        const char*             uriLocation,
        const BOA_Version*      apiVersion,
    ...
    */
    ...
    typedef struct OCI_CAN_VTable
    {
        ...
        PF_OCI_WriteCANDataEx   writeCANDataEx; // Added for OCI_CAN v1.3
        PF_OCI_ReadCANDataEx    readCANDataEx;  // Added for OCI_CAN v1.3
    } OCI_CAN_VTable;
