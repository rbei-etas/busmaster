...
void findLinNodes( CSI_Tree* sfsTree, OCI_URIName uriPrefix, OCI_URIName uriNames[], uint32 size, uint32* position )
{
    ...
    if (sfsTree->child)
    {
        ...
        strcat_s( newUriPrefix, sizeof(newUriPrefix), "/" );
        strcat_s( newUriPrefix, (sizeof(newUriPrefix)-sizeof("/")), sfsTree->item.uriName );
...
static int nGetCalcChecksum(int nChannel, int nId )
{
    ...
    {
        if ( sg_asChannel[nChannel ].m_OCI_LinConfig.version <= OCI_LIN_VERSION_1_3 || ( nId >= 60 && nId <= 63 ) )
        {
...
static BOOL bLoadDataFromContr(IBMNetWorkGetService*  asDeviceConfig)
{
    ...
    for ( int nChannel = 0 ; nChannel < sg_nNoOfChannels; nChannel++ )
    {
        ...
        asDeviceConfig->GetChannelSettings(LIN, nChannel, &ouChannelSettings);

        sg_asChannel[nChannel].m_OCI_LinConfig.mode               = ouChannelSettings.m_ouLINSettings.m_bIsMasterMode ? OCI_LIN_MASTER : OCI_LIN_SLAVE;;                    // Open this controller as a LIN Slave.
        sg_asChannel[nChannel].m_OCI_LinConfig.baudrate           = ouChannelSettings.m_ouLINSettings.m_nBaudRate;                            // The chosen baudrate on the LIN BUS.  Note that this must match the
        // baudrate of the other controllers connected to the same BUS.
        sg_asChannel[nChannel].m_OCI_LinConfig.version            = nGetProtocolVersion(ouChannelSettings.m_ouLINSettings.m_strProtocolVersion);
        sg_asChannel[nChannel].m_strLinVersion            =     ouChannelSettings.m_ouLINSettings.m_strProtocolVersion;
        sg_asChannel[nChannel].m_OCI_LinConfig.masterResistor     = OCI_LIN_MASTER_RESISTOR_OFF;      // LIN master resistor is not connected to LIN bus.
        sg_asChannel[nChannel].m_OCI_LinConfig.linSupply          = OCI_LIN_UBAT_SUPPLY_INTERNAL;     // LIN Port Power is supplied internally.
        sg_asChannel[nChannel].m_OCI_LinConfig.selfReceptionMode  = OCI_SELF_RECEPTION_ON;            // The Controller will receive whatever is found on the LIN Bus, including
        ...
        //3. Create Tx Buffers
        for (INT nIndex = 0; nIndex < ( sizeof( sg_asChannel[nChannel].m_OCI_LinConfig.messages ) / sizeof( sg_asChannel[nChannel].m_OCI_LinConfig.messages[0] ) ); ++nIndex )
        {
            sg_asChannel[nChannel].m_OCI_LinConfig.messages[nIndex].mode       = OCI_LIN_FRAME_DEFINED;      // The controller will not use this Frame ID for transmission of messages.
            ...
            sg_asChannel[nChannel].m_OCI_LinConfig.messages[nIndex].frameType  = OCI_LIN_UNCONDITIONAL_FRAME;
            sg_asChannel[nChannel].m_OCI_LinConfig.messages[nIndex].crcType    = nGetCalcChecksum(nChannel, nIndex+1);
            sg_asChannel[nChannel].m_OCI_LinConfig.messages[nIndex].dlc        = 8;
        }

        ICluster* pCluster;
        asDeviceConfig->GetDBService(LIN, 0, 0, &pCluster);
        if ( nullptr != pCluster )
        {
            IFrame* pTempFrame;
            unsigned int unId = -1;
            unsigned int unLength = 0;
            std::map<UID_ELEMENT, IElement*> lstMsgNames;
            pCluster->GetElementList( eFrameElement, lstMsgNames);

for ( auto itrFrame : lstMsgNames)
            {
                pTempFrame = (IFrame*)itrFrame.second;
                pTempFrame->GetFrameId(unId);
                pTempFrame->GetLength(unLength);
                if(unId >= 0 && unId <= 63)
                {
                    sg_asChannel[nChannel].m_OCI_LinConfig.messages[unId].mode       = OCI_LIN_FRAME_DEFINED;      // The controller will not use this Frame ID for transmission of messages.

                    // hard-coding values to avoid having uninitialized memory cause some unexpected issue.  These should all be ignored
                    // since the frame mode is set to undefined.
                    sg_asChannel[nChannel].m_OCI_LinConfig.messages[unId].frameType  = OCI_LIN_UNCONDITIONAL_FRAME;
                    sg_asChannel[nChannel].m_OCI_LinConfig.messages[unId].crcType    = nGetCalcChecksum(nChannel, unId);
                    sg_asChannel[nChannel].m_OCI_LinConfig.messages[unId].dlc        = unLength;
                }
            }
        }
    }
...
{
    memset ( &sg_asChannel[nChannel].m_OCI_LinConfig, 0, sizeof( sg_asChannel[nChannel].m_OCI_LinConfig ) );                    // Initialize the Controller Configuration to 0
    sg_asChannel[nChannel].m_OCI_LinConfig.mode               = OCI_LIN_SLAVE;                    // Open this controller as a LIN Slave.
    sg_asChannel[nChannel].m_OCI_LinConfig.baudrate           = 9600;                            // The chosen baudrate on the LIN BUS.  Note that this must match the
    // baudrate of the other controllers connected to the same BUS.
    sg_asChannel[nChannel].m_OCI_LinConfig.version            = OCI_LIN_VERSION_1_3;              // Use LIN v2.1
    sg_asChannel[nChannel].m_OCI_LinConfig.masterResistor     = OCI_LIN_MASTER_RESISTOR_OFF;      // LIN master resistor is not connected to LIN bus.
    sg_asChannel[nChannel].m_OCI_LinConfig.linSupply          = OCI_LIN_UBAT_SUPPLY_INTERNAL;     // LIN Port Power is supplied internally.
    sg_asChannel[nChannel].m_OCI_LinConfig.selfReceptionMode  = OCI_SELF_RECEPTION_ON;            // The Controller will receive whatever is found on the LIN Bus, including
    // the frames it transmits itself.  Note that for LIN, this cannot be set
...
UINT nGetChannel(const OCI_ControllerHandle hHandle)
{
    ...
    return (1); //Currently 1 Channel Has to e fixed
}
...
USAGEMODE HRESULT GetIDIL_LIN_Controller(void** ppvInterface)
{
    ...
    *ppvInterface = (void*) g_pouDIL_LIN_BOA;  /* Doesn't matter even if g_pouDIL_LIN_BOA is null */

    return hResult;
}

HRESULT CDIL_LIN_ETAS_BOA::LIN_SetAppParams(HWND hWndOwner)
{
...
HRESULT CDIL_LIN_ETAS_BOA::LIN_PerformInitOperations(void)
{
    ...
    DWORD dwClient = 0;
    LIN_RegisterClient(TRUE, dwClient, LIN_MONITOR_NODE);
...
HRESULT CDIL_LIN_ETAS_BOA::LIN_SelectHwInterface(const INTERFACE_HW_LIST& sSelHwInterface, INT /* nCount */)
{
    ...
    for (UINT i = 0; i < sg_nNoOfChannels; i++)
    {
        ...
        if (sOCI.createLINController != nullptr)
        {
...
HRESULT CDIL_LIN_ETAS_BOA::SetConfigData(IBMNetWorkGetService* ouAbsSFibexContainer)
{
...
HRESULT CDIL_LIN_ETAS_BOA::LIN_InitializeBuffers(void)
{
...

    {
        ...
        for ( ; itr != sg_asChannel[i].m_ouBoaLINData.end(); itr++ )
        {
            sg_asChannel[i].m_OCI_LinConfig.messages[itr->second.m_ouLinData.m_ucMsgID].frameType      = OCI_LIN_UNCONDITIONAL_FRAME;  // Declare that this will be an unconditional frame id for this controller.
            sg_asChannel[i].m_OCI_LinConfig.messages[itr->second.m_ouLinData.m_ucMsgID].crcType        = nGetCalcChecksum(i, itr->second.m_ouLinData.m_ucMsgID);//OCI_LIN_CHECKSUM_ENHANCED;    // Use enhanced checksum for transmissions on this frame id.
            sg_asChannel[i].m_OCI_LinConfig.messages[itr->second.m_ouLinData.m_ucMsgID].dlc            = itr->second.m_ouLinData.m_ucDataLen;                            // The frames will have 8 bytes of payload.
            sg_asChannel[i].m_OCI_LinConfig.messages[itr->second.m_ouLinData.m_ucMsgID].mode           = OCI_LIN_FRAME_DEFINED;        // Declare that the details of frame ID 3 are defined.
        }


        OciErrorCode = (*sBOA_PTRS.m_sOCI.openLINController)(sg_asChannel[i].m_OCI_HwHandle,
                       &(sg_asChannel[i].m_OCI_LinConfig),
                       &(sg_asChannel[i].m_OCI_CntrlProp));
        ...
        if ( sg_asChannel[i].m_OCI_LinConfig.mode == OCI_LIN_MASTER )
        {
...
HRESULT CDIL_LIN_ETAS_BOA::hRecreateLINController(INT nIndex)
{
    ...
    if (OciErrorCode == OCI_SUCCESS)
    {
        ...
        OciErrorCode = (*sBOA_PTRS.m_sOCI.openLINController)(sg_asChannel[nIndex].m_OCI_HwHandle,
                       &(sg_asChannel[nIndex].m_OCI_LinConfig),
                       &(sg_asChannel[nIndex].m_OCI_CntrlProp));
        hResult = S_OK;
    }
    else
...
HRESULT CDIL_LIN_ETAS_BOA::LIN_Send(STLIN_MSG& pouFlxTxMsg)
{
    ...
    if (m_bLINConnected == FALSE && pouFlxTxMsg.m_ucMsgTyp == LIN_SLAVE_RESPONSE )          //Controller Not active
    {
        ...
        ouData.m_ouFrameType = 0; // Not used, but initializing it
