    ...
    //: CPropertyPage(CChangeRegisters::IDD, IDS_PPAGE_CHANGE_REGISTER)
{
    ...
    m_usBTR0BTR1 = defDEFAUT_BAUDRATE;
    //defMaxBaudrate = defMAX_BAUDRATE;
...
void CChangeRegisters::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Control(pDX, IDC_CHKB_LOW_SPEED, m_omLowSpeed);
...
BEGIN_MESSAGE_MAP(CChangeRegisters, CDialog)
    ...
    ON_BN_CLICKED(IDC_CHKB_LOW_SPEED, OnLowSpeedClicked)
    //}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void CChangeRegisters::OnKillfocusEditBaudRate()
{
    ...
    // Get max baudrate
    uBTR0val.ucBTR0 = defMaxBaudrate >> 8;
    uBTR1val.ucBTR1 = defMaxBaudrate & 0xFF;
    // BRP = BRPbit+1
    ...
    dMaxBaudRate  = (DOUBLE)(_tstoi(defCLOCK)/ ( 2.0 * byBRP * byNBT ));
    dMaxBaudRate  = dMaxBaudRate * (defFACT_FREQUENCY / defFACT_BAUD_RATE);
    // covert to bps
    dMaxBaudRate*=1000;
    ...
    if(m_bDialogCancel != TRUE  )
    {
        ...
        if(unButtonState ==0 )
        {
            ...
            if(nLength == 0 || dBaudRate <= 0 || dBaudRate > dMaxBaudRate )
            {
                m_omEditBaudRate.SetWindowText(m_omStrEditBaudRate);
                //if(theApp.m_bFromAutomation == FALSE)
                CString omErr;
                omErr.Format(_("Not a valid Decimal Numerals or out of range. !!! \n\nEnter only positive Decimal Numerals up to %7.0f bps."), dMaxBaudRate);
                AfxMessageBox(omErr);
...
void CChangeRegisters:: vCalculateBaudRateNBTR1(CString omStrBtr0)
{
    ...
    else
    {
        ...
        if(nItem!= -1 )
        {
            ...
            m_omEditBTR1.GetWindowText(omStrBtr1);
            nItem=0;
            while(omStrLocalBtr1!=omStrBtr1 && omStrLocalBtr1.IsEmpty() == FALSE)
            {
                nItem++;
                omStrLocalBtr1 = m_omListCtrlBitTime.GetItemText(nItem, defBRT1_COLUNM_POS-1);
                omStrLocalBtr1.Replace(defHEX_STRING,defEMPTY_STRING);
            ...
            if(omStrLocalBtr1.IsEmpty() == FALSE && omStrLocalBtr0 == omStrBtr0)
            {
                omStrLocalBtr1 = m_omListCtrlBitTime.GetItemText(nItem, defBRT1_COLUNM_POS-1);
                omStrLocalBtr1.Replace(defHEX_STRING,defEMPTY_STRING);
                omStrNbt      = m_omListCtrlBitTime.GetItemText(nItem, defNBT_COLUNM_POS-1);
...
void CChangeRegisters:: vCalculateBaudRateNBTR0(CString omStrBtr1)
{
    ...
    else
    {
        ...
        if(nItem!= -1 )
        {
            ...
            if(omStrLocalBtr1 == omStrBtr1 && omStrLocalBtr1.IsEmpty() == FALSE )
            {
...
void CChangeRegisters::vFillControllerConfigDetails()
{
    ...
    if (pCheckLowSpeed != nullptr && m_pControllerDetails != nullptr)
    {
        char* pchMuxyBox2;
        char* pchMuxyBox;
        char* pchMuxy2010;
        ...
        {
            pCheckLowSpeed->EnableWindow(FALSE);
            pCheckLowSpeed->SetCheck(TRUE);
            defMaxBaudrate = defLOW_SPEED_BAUDRATE;
            vCalculateBaudRateNBTR1("03");
        }
    }
    //Set the self reception option
    CButton* pCheckSelfRec = (CButton*)GetDlgItem(IDC_CHKB_SELF_RECEPTION2);
...
void CChangeRegisters::vUpdateControllerDetails()
{
            ...
            && ( unWarningLimit <= unWarningLimtMax ) )
    {
        ...
        //Get the Low Speed option
        CButton* pCheckLowSpeed = (CButton*)GetDlgItem(IDC_CHKB_LOW_SPEED);
        if (pCheckLowSpeed != nullptr)
        {
            if ( pCheckLowSpeed->GetCheck() == BST_CHECKED )
            {
                m_pControllerDetails[m_nLastSelection].m_bLowSpeed = BST_CHECKED;
            }
            else
            {
                m_pControllerDetails[m_nLastSelection].m_bLowSpeed = BST_UNCHECKED;
            }
        }
...
/*******************************************************************************
/*  Function Name    :  OnLowSpeedKillFocus                                   */
/*  Input(s)         :                                                        */
/*  Output           :                                                        */
/*  Functionality    :  Validate the Low Speed on kill focus of this chekbox. */
/*  Member of        :  CChangeRegisters                                      */
/*  Friend of        :      -                                                 */
/*                                                                            */
/*  Author(s)        :  Gregory Merchat                                        */
/*  Date Created     :  06.05.2013                                            */
/******************************************************************************/
void CChangeRegisters::OnLowSpeedClicked()
{
    /*
    CString omStrBtr1Get    ="";
    CString omStrValidBtr1  ="";
    ...
    if(m_bDialogCancel != TRUE)
    {
        if(pomButtonCancel !=pomFocusWnd)
        {
            ...
            {
                defMaxBaudrate = defMAX_BAUDRATE;
                vCalculateBaudRateNBTR1("00");
            }
            m_omLowSpeed.SetCheck(nCheck);
        }
    }
    else
    {
...
DOUBLE CChangeRegisters::vValidateBaudRate(DOUBLE dBaudrate,int nItemCount,UINT )
{
    CString omStrBaudRate       = "";
