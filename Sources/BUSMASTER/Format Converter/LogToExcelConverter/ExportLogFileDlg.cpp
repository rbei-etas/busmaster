...
BEGIN_MESSAGE_MAP(CExportLogFileDlg, CPropertyPage)
    ...
    ON_MESSAGE(WM_UPDATEUI,UpdateUI)
END_MESSAGE_MAP()
...
LPARAM CExportLogFileDlg::UpdateUI(WPARAM wParam, LPARAM)
{
...
DWORD WINAPI ExportLogFileThreadProc(LPVOID pVoid)
{
    CExportLogFileDlg* pExportLogFileDlg = (CExportLogFileDlg*)pVoid;
    if(pExportLogFileDlg != nullptr)
    {
        // Log File name and Excel File name
        CString strLogFileName = pExportLogFileDlg->m_omStrLogFileName;
        CString strCSVFileName = pExportLogFileDlg->m_omStrCSVFileName;
        //fields have been selected
        //open the files
        CLogToExcel oExport( strLogFileName.GetBuffer(MAX_PATH),strCSVFileName.GetBuffer(MAX_PATH), pExportLogFileDlg->m_unNoOfFileds, pExportLogFileDlg->m_pacFields);
        //set the selected fields
        oExport.fnSetSelectedFields( &(pExportLogFileDlg->m_omSelectedList) );

        /* Set the bus type */
        oExport.vSetBUSType(pExportLogFileDlg->m_eBus);
