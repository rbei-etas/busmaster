...
void CTreeViewEx::OnTvnBeginlabeledit(NMHDR* /*pNMHDR*/, LRESULT* pResult)
{
    ...
    {
        eTYPE_ENTITY eCurrentEntity = pChildFrame->eGetCurrentEntityType();
        *pResult = 1;
...
void CTreeViewEx::OnAddEntityItem(UINT nId)
{
    ...
    //W4 Removal
    CTSEditorChildFrame* pFrameChild = GetEditorWindow();
    if (pFrameChild != nullptr)
    {
        pFrameChild->nAddNewEntity((DWORD)omTempTreeCtrl.GetItemData(hSelectedItem), eEntityType);
    }
}
