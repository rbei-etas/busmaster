    ...
    if ( sLinData.m_eLinMsgType == LIN_MSG /*IS_A_MESSAGE(sLinData.m_ucDataType)*/ )
    {
        ...
        *pStlin = sLinData;
        if (!bTobeBlocked(sLinData))
        {
            ...
            {
                m_pRxMsgCallBack->onRxMsg((void*)&sLinData);
            }
        }
    }
        ...
        int& nBufferIndex, EINTERPRET_MODE& interPretable )
{
    ...
    if (nullptr != mBmNetwork)
    {
        ...
        mBmNetwork->GetFrame(LIN, 0, pouLINData->m_uDataInfo.m_sLINMsg.m_ucMsgID, nullptr, &pouFrame);
        if (nullptr != pouFrame)
        {
        ...
        if (pouLINData->m_eLinMsgType == LIN_EVENT)
        {
            interPretable = MODE_NONE;
        }
    }
        ...
        __int64 nTimeOffset)
{
    ...
    if (hResult == S_OK)
    {
        ...
        CString name;
        GetMessageName(sLINCurrData, name, IS_NUM_HEX_SET(bExprnFlag_Disp));
...
HRESULT CMsgContainerLIN::GetMessageName(STLINDATA& sLinMsg, CString& msgName, bool formatHexForId)
{
    CString omName = "";
    unsigned int id = -1;
    if (sLinMsg.m_eLinMsgType == LIN_MSG)
    {
        id = sLinMsg.m_uDataInfo.m_sLINMsg.m_ucMsgID;
    }
    else if (sLinMsg.m_eLinMsgType == LIN_EVENT)
    {
        if (sLinMsg.m_uDataInfo.m_sErrInfo.m_eEventType == EVENT_LIN_ERRNOANS || sLinMsg.m_uDataInfo.m_sErrInfo.m_eEventType == EVENT_LIN_ERRCRC)
        {
            id = sLinMsg.m_uDataInfo.m_sErrInfo.m_ucId;
        }
    }
    if (mBmNetwork != nullptr && id >= 0)
    {
        IFrame* pouFrame = nullptr;
        mBmNetwork->GetFrame(LIN, 0, id, nullptr, &pouFrame);
        if (nullptr != pouFrame)
        {
            std::string strName;
            pouFrame->GetName(strName);
            omName = strName.c_str();
        }
        else
        {
            if (formatHexForId == true)
            {
                omName.Format("%X", sLinMsg.m_uDataInfo.m_sLINMsg.m_ucMsgID);
                omName = "0x" + omName;
            }
            else
            {
                omName.Format("%d", sLinMsg.m_uDataInfo.m_sLINMsg.m_ucMsgID);
            }
        }
    }
    msgName = omName;
...
HRESULT CMsgContainerLIN::SetIBMNetWorkGetService(void* pBmNetWork)
{
    mBmNetwork = (IBMNetWorkGetService*)pBmNetWork;
    return S_OK;
}

HRESULT CMsgContainerLIN::GetSignalInfoArray(__int64 nMapIndex, SSignalInfoArray& SigInfoArray, bool formatInHex)
{
    STLINDATA sLinMsg;
    SigInfoArray.RemoveAll();

    HRESULT hResult = hReadFromOWBuffer(&sLinMsg, nMapIndex);
    if (hResult == S_OK)
    {
        GetInterPretedSignalList(sLinMsg, SigInfoArray, formatInHex);
    }
    return S_OK;
}
HRESULT CMsgContainerLIN::GetInterPretedSignalList(STLINDATA& ouMsg, SSignalInfoArray& ouSSignalInfoArray, bool isHexMode)
{
    if(nullptr == mBmNetwork)
    {
        return S_FALSE;
    }
    if (ouMsg.m_eLinMsgType == LIN_MSG)
    {
        IFrame* pouFrame = nullptr;
        ERRORCODE eResult = mBmNetwork->GetFrame(LIN, 0, ouMsg.m_uDataInfo.m_sLINMsg.m_ucMsgID, nullptr, &pouFrame);
        ...
        if (EC_SUCCESS == eResult && nullptr != pouFrame)
        {
            ...
            pouFrame->InterpretSignals(ouMsg.m_uDataInfo.m_sLINMsg.m_ucData, ouMsg.m_uDataInfo.m_sLINMsg.m_ucDataLen, ouSignal, isHexMode, true);
...
COLORREF CMsgContainerLIN::getMessageColor(long long key, bool isAppendMode, CMessageAttrib* msgAttributes/*has to be removed*/)
{
    ...
    COLORREF col = BLACK_COLOR;
    HRESULT hResult = S_FALSE;
    STLINDATA sLinMsg;
    if (isAppendMode == true)
    {
        STLINDATASPL sSplLinMsg;
        hResult = m_ouAppendLinBuf.ReadFromBuffer(&sSplLinMsg, (int)key);
        sLinMsg.m_ucDataType = sSplLinMsg.m_ucDataType;
        sLinMsg.m_uDataInfo = sSplLinMsg.m_uDataInfo;
        sLinMsg.m_eLinMsgType = sSplLinMsg.m_eLinMsgType;
    }
    else
    {
        hResult = hReadFromOWBuffer(&sLinMsg, key);
    }
    if (CALL_SUCCESS == hResult)
    {
        ...
        else if (msgAttributes != nullptr)
        {
            col = msgAttributes->GetCanIDColour(sLinMsg.m_uDataInfo.m_sLINMsg.m_ucMsgID);
        }
        if ((col == BLACK_COLOR) && (sLinMsg.m_eLinMsgType == LIN_MSG && (sLinMsg.m_uDataInfo.m_sLINMsg.m_ucMsgID == 0X3C || sLinMsg.m_uDataInfo.m_sLINMsg.m_ucMsgID == 0x3D)))
        {
            col = RGB(0, 125, 0);
        }
    }
...
HRESULT CMsgContainerLIN::GetMessageDetails(__int64 nMapIndex, unsigned int& msgId, CString& msgName, bool formatHexForId)
{
    if (nullptr == mBmNetwork)
    {
        return S_FALSE;
    ...
    std::string strName = "";
    HRESULT hResult = hReadFromOWBuffer(&sLinMsg, nMapIndex);
    msgId = sLinMsg.m_uDataInfo.m_sLINMsg.m_ucMsgID;

    IFrame* pouFrame = nullptr;
    ERRORCODE eResult = mBmNetwork->GetFrame(LIN, 0, sLinMsg.m_uDataInfo.m_sLINMsg.m_ucMsgID, nullptr, &pouFrame);
    if (EC_SUCCESS == eResult && nullptr != pouFrame)
    {
        pouFrame->GetName(strName);
    ...
    else
    {
        if (formatHexForId == true)
        {
            msgName.Format("%d", sLinMsg.m_uDataInfo.m_sLINMsg.m_ucMsgID);
        }
        else
        {
