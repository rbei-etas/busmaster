...
#define BOA_ERR_QUEUE_IS_FULL                   (BOA_RESULT_FLAG_ERROR | BOA_RESULT_TYPE_SEMANTIC | 2)
    ...
    /**  The request cannot be completed because to do so would disrupt another user of the object. */
#define BOA_ERR_OBJECT_IN_USE                   (BOA_RESULT_FLAG_ERROR | BOA_RESULT_TYPE_SEMANTIC | 3)

    /**  The request cannot be completed because another client has already configured the object using
     * a later version of the relevant API, and it is not possible to convert between that API version
     * and the version used by the caller. */
#define BOA_ERR_LATER_VERSION_IN_USE            (BOA_RESULT_FLAG_ERROR | BOA_RESULT_TYPE_SEMANTIC | 4)
    ...
    */
    ...
    /** The request cannot be completed because it is incompatible with configuration provided via a previous
        API call. Note that in a multi-client scenario, the previous API call may have been made by a
        different client. */
...
#define BOA_ERR_NOT_INITIALIZED                 (BOA_RESULT_FLAG_ERROR | BOA_RESULT_TYPE_INTERNAL | 9)
    ...
    */
    ...
    /** The server offers the interface with the requested UUID, but cannot fullfill the required version. */
