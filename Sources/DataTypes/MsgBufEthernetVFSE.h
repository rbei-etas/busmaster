...
#if !defined MSGBUFETHERNETVFSE_H__INCLUDED_
#define MSGBUFETHERNETVFSE_H__INCLUDED_

#include "include/Error.h"
#include "BaseMsgBufAll.h"
//#include "afxtempl.h"

const int SIZE_APP_ETHERNET_BUFFER       = 10000;
...
class CMsgBufEthernetVFSE
{
protected:

    SMSGBUFFER m_asMsgBuffer[SIZE_APP_ETHERNET_BUFFER]; // The data buffer
...
/******************************************************************************
  Function Name    :  CMsgBufEthernetVFSE
  Input(s)         :  -
  Output           :  -
  Functionality    :  Standard constructor
  Member of        :  CMsgBufEthernetVFSE
  Friend of        :  -
...
******************************************************************************/
template <typename SMSGBUFFER>
CMsgBufEthernetVFSE<SMSGBUFFER>::CMsgBufEthernetVFSE()
{
...
/******************************************************************************
  ...
  Modification By  :
******************************************************************************/
template <typename SMSGBUFFER> void CMsgBufEthernetVFSE<SMSGBUFFER>::
vClearMessageBuffer(void)
{
    memset((BYTE*) m_asMsgBuffer, 0, SIZE_APP_ETHERNET_BUFFER * m_nMsgSize);
...
******************************************************************************/
template <typename SMSGBUFFER>
int CMsgBufEthernetVFSE<SMSGBUFFER>::ReadFromBuffer(SMSGBUFFER* psMsg, int nIndex)
{
    HRESULT nResult = S_OK;

#ifdef _DEBUG
    ASSERT(psMsg != NULL);
    ASSERT(!(m_nIndexRead > SIZE_APP_ETHERNET_BUFFER));
#endif
    ...
    else if (nIndex >= SIZE_APP_ETHERNET_BUFFER)
    {
    ...
    else
    {
        ...
        if (m_nIndexRead + nIndex >= SIZE_APP_ETHERNET_BUFFER)
        {
            TempIndex -= SIZE_APP_ETHERNET_BUFFER;
        }*/
		memcpy(psMsg , &(m_asMsgBuffer[nIndex]), sizeof(m_asMsgBuffer[nIndex]));
    }
    // Unlock the buffer
...
template <typename SMSGBUFFER>
int CMsgBufEthernetVFSE<SMSGBUFFER>::ReadFromBuffer(SMSGBUFFER* psMsg, __int64 nSlotId)
{
...
******************************************************************************/
template <typename SMSGBUFFER>
int CMsgBufEthernetVFSE<SMSGBUFFER>::WriteIntoBuffer(const SMSGBUFFER* psMsg,
...

{
...
#ifdef _DEBUG
    ASSERT(psMsg != NULL);
    ASSERT(!(m_nIndexWrite > SIZE_APP_ETHERNET_BUFFER));
#endif

    EnterCriticalSection(&m_CritSectionForGB); // Lock the buffer

    if (m_nMsgCount == SIZE_APP_ETHERNET_BUFFER) // Check for buffer overflow
    {
    ...
    else
    {
        if (m_omIdIndexMap.Lookup(nSlotId, nIndex))
        {
           memcpy( &(m_asMsgBuffer[nIndex]) ,psMsg, sizeof(*psMsg));
        }
        else
        {
            nIndex = m_nMsgCount;
            //m_asMsgBuffer[m_nMsgCount] = *psMsg;
			memcpy (&(m_asMsgBuffer[m_nMsgCount]), psMsg, sizeof(*psMsg));
...

{
    ...
    if (m_nMsgCount == SIZE_APP_ETHERNET_BUFFER) // Check for buffer overflow
    {
        memcpy(&(m_asMsgBuffer[m_nIndexRead]), psMsg, sizeof( *psMsg));
        m_nIndexRead++;
        if (m_nIndexRead == SIZE_APP_ETHERNET_BUFFER)
        {
            m_nIndexRead = 0;
        }
        nResult = ERR_FULL_APP_BUFFER;
...
/******************************************************************************
  ...
  Modification By  :
******************************************************************************/
template <typename SMSGBUFFER> int CMsgBufEthernetVFSE<SMSGBUFFER>::
...
/******************************************************************************
  ...
  Modification By  :
******************************************************************************/
template <typename SMSGBUFFER> HANDLE CMsgBufEthernetVFSE<SMSGBUFFER>::
