...
CHardwareListing::CHardwareListing(
    INTERFACE_HW*   psIntrHw,
    int     nSize,
    int*        pnSelList,
    CWnd*       pParent /*=nullptr*/,
    fnCallBackBlink fnBlink) :
...
void CHardwareListing::vSetHardwareList(INTERFACE_HW* /*psHwIntr*/, int nSize)
{
    ...
    for( int index = 0; index < nSize; index++)
    {
        ...
        if ( !bSelItem )
        {
            omStrFormat = m_psHwInterface[index].m_acDescription.c_str();
...
void CHardwareListing::vEnableDisableButtons()
{
    ...
    if( pWnd != nullptr )
    {
        if(bSelectEnable == FALSE || m_pfnBlinkFunction == nullptr)
        {
