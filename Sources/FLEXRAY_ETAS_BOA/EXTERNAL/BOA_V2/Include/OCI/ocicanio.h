...
extern "C" {
    ...
    */
    typedef struct OCI_CANControllerCapabilities
    {
        ...
        * @n        @ref OCI_CAN_MSG_FLAG_FD_TRUNC_AND_PAD
        ...
        */
        ...
        /** Specifies the supported CAN-FD features of this controller. Valid values are OR-gated into the member.
            This member must be set to zero for a controller which does not support CAN-FD.
            @flags        The following flags are valid:
            @n            @ref OCI_CANFDSUPPORT
            @n            @ref OCI_CANFDTX_DELAY_COMPENSATION_SUPPORT
        ...
        */
        ...
        /** The maximum payload size supported by the CAN-FD controller. This member must be zero for controllers not supporting CAN-FD.

            @version This member was added in v1.3 of OCI_CAN: do not attempt to access this member unless the current OCI_CAN
            session uses at least v1.3.
        */
        uint32  canFDMaxDataSize;

        /** The maximum data rate supported by the CAN-FD controller under all possible conditions.
            This value must be <= @ref canFDMaxDataRate, and must be zero for controllers not supporting CAN-FD.

            The CAN-FD controller is guaranteed to support the data rate specified by @ref canFDMaxQualifiedDataRate under all
            possible bus conditions and topologies. Additionally, under certain favourable conditions, the controller may be
            able to reach the data rate specified by @ref canFDMaxDataRate.

            @version This member was added in v1.3 of OCI_CAN: do not attempt to access this member unless the current OCI_CAN
            session uses at least v1.3.
        */
        uint32  canFDMaxQualifiedDataRate;

        /** The maximum Data rate supported by the CAN-FD controller. This must be zero for controllers not supporting CAN-FD.
            ...
            session uses at least v1.3.
        */
        uint32  canFDMaxDataRate;

        /** This member describes the supported modes of behaviour which a CAN-FD controller offers when receiving CAN 2.0
            frames.

            Valid values are OR-gated into the member.  This member must be zero for controllers not supporting CAN-FD.
            @flags        The following flags are valid:
            @n            @ref OCI_CAN_RXMODE_CAN_FRAMES_IGNORED
            @n            @ref OCI_CAN_RXMODE_CAN_FRAMES_USING_CAN_MESSAGE

            @version This member was added in v1.3 of OCI_CAN: do not attempt to access this member unless the current OCI_CAN
            session uses at least v1.3.
        */
        uint32  canFDRxConfig_CANMode;

        /** This member describes the supported modes of behaviour which a CAN-FD controller offers when receiving CAN-FD frames.

            Valid values are OR-gated into the member. This member must be zero for controllers not supporting CAN-FD.
            @flags        The following flags are valid:
            @n            @ref OCI_CANFDRXMODE_CANFD_FRAMES_IGNORED
            @n            @ref OCI_CANFDRXMODE_CANFD_FRAMES_USING_CAN_MESSAGE
            @n            @ref OCI_CANFDRXMODE_CANFD_FRAMES_USING_CANFD_MESSAGE
            @n            @ref OCI_CANFDRXMODE_CANFD_FRAMES_USING_CANFD_MESSAGE_PADDING

            @version This member was added in v1.3 of OCI_CAN: do not attempt to access this member unless the current OCI_CAN
            session uses at least v1.3.
        */
        uint32  canFDRxConfig_CANFDMode;

        /** This member describes the supported modes of behaviour which a CAN-FD controller offers when transmitting CAN
            messages provided by the OCI_CAN client.

            Valid values are OR-gated into the member. This member must be zero for controllers not supporting CAN-FD.
            @flags        The following flags are valid:
            @n            @ref OCI_CANFDTX_USE_CAN_FRAMES_ONLY
            @n            @ref OCI_CANFDTX_USE_CANFD_FRAMES_ONLY
            @n            @ref OCI_CANFDTX_USE_CAN_AND_CANFD_FRAMES

            @version This member was added in v1.3 of OCI_CAN: do not attempt to access this member unless the current OCI_CAN
            session uses at least v1.3.
        */
        uint32  canFDTxConfig_Mode;

        /** Specifies the possible bus participation modes supported by the CAN controller.

            Valid values are OR-gated into the member.
            @flags        The following flags are valid:
            @n            @ref OCI_BUSMODE_PASSIVE
            @n            @ref OCI_BUSMODE_ACTIVE

            @version This member was added in v1.3 of OCI_CAN: do not attempt to access this member unless the current OCI_CAN
            session uses at least v1.3.
        */
        uint32 canBusParticipationMode;

    } OCI_CANControllerCapabilities;


    /** The CAN controller has switched into the active state. */
#define OCI_CAN_STATE_ACTIVE              (0x00000001u)

    /** The CAN controller has switched into the passive state. */
#define OCI_CAN_STATE_PASSIVE             (0x00000002u)

    /** The warning limit of the error counter has exceeded. */
#define OCI_CAN_STATE_ERRLIMIT            (0x00000004u)

    /** The controller has switched into the bus off state. */
#define OCI_CAN_STATE_BUSOFF              (0x00000008u)
    ...
    */
    ...
    typedef struct OCI_CANControllerStatus
    {
        ...
        /**
        *   Current CAN controller status information (bit field) as indicated by a combination of the following values:
        *   @ref OCI_CAN_STATE_ACTIVE, @ref OCI_CAN_STATE_PASSIVE, @ref OCI_CAN_STATE_ERRLIMIT, @ref OCI_CAN_STATE_BUSOFF.
        *   Valid values may be OR-gated as flags into the field but it is assumed that @ref OCI_CAN_STATE_ACTIVE
        *   and @ref OCI_CAN_STATE_PASSIVE are mutually exclusive.
        *
        *   If the value of the field is 0 then the controller state is unknown or cannot be determined. This may be a
        *   temporary or a permanent condition.
        *
        *   @version This member was added in v1.2 of OCI_CAN: do not attempt to access this member unless the current OCI_CAN
        *   session uses at least v1.2.
        */
        uint32 stateCode;
    ...
    */
    ...
    /**
     Signature of a callback function for CAN event and message handling. A client should define a callback of this type
     when he wishes to receive instances of @ref OCI_CANMessage. This is sufficient to receive all types of message and
     event except CAN-FD messages. If the client wishes to receive CAN-FD messages as well (or instead of) CAN 2.0 messages,
     he must instead define a callback of the type @ref OCI_CANRxCallbackFunctionSingleMsgEx.
    ...
    */
    ...
    /**
     Signature of a callback function for CAN event and message handling. A client should define a callback of this type
     when he wishes to receive instances of @ref OCI_CANMessageEx. This is sufficient to receive all types of message and
     event @em including CAN-FD messages. The alternative, @ref OCI_CANRxCallbackFunctionSingleMsg, is unable to receive
     CAN-FD messages.
     ...
     @param[in] msg      The received message or event. The object is owned by the caller of the API. The reference must
                         ...
                         completion of the callback the callback must create a copy of the data.
    */
    typedef void (OCI_CALLBACK* OCI_CANRxCallbackFunctionSingleMsgEx)( void* userData, struct OCI_CANMessageEx* msg );



    /*  Microsoft Visual C will print warning C4201 when compiling an un-named union in debug mode.
        So suppress this warning */
    BOA_DECLSPEC_WARNING_DISABLE(4201)
    ...
    */
    typedef struct OCI_CANRxCallbackSingleMsg
    {
        union
        {
            /** Callback function pointer for receiving instances of @ref OCI_CANMessage. Defining a callback of this type
             * allows the client to receive all CAN messages and events @em except CAN-FD messages. */
            OCI_CANRxCallbackFunctionSingleMsg   function;
            /** Callback function pointer for receiving instances of @ref OCI_CANMessageEx. Defining a callback of this type
             * allows the client to receive all CAN messages and events @em including CAN-FD messages. */
            OCI_CANRxCallbackFunctionSingleMsgEx functionEx;
        };
    ...
    * @{ */
    ...
    /** The CAN controller has switched into the active state. */
#define OCI_CAN_BUS_EVENT_STATE_ACTIVE              OCI_CAN_STATE_ACTIVE

    /** The CAN controller has switched into the passive state. */
#define OCI_CAN_BUS_EVENT_STATE_PASSIVE             OCI_CAN_STATE_PASSIVE

    /** The warning limit of the error counter has exceeded. */
#define OCI_CAN_BUS_EVENT_STATE_ERRLIMIT            OCI_CAN_STATE_ERRLIMIT

    /** The controller has switched into the bus off state. */
#define OCI_CAN_BUS_EVENT_STATE_BUSOFF              OCI_CAN_STATE_BUSOFF
...
#define OCI_CAN_BUS_EVENT_FAULT_TOLERANT_SINGLE_WIRE (0x00000010u)
    ...
    */
    ...
    typedef enum OCI_CANMessageDataType
    {
        ...
        /** Message type is @ref OCI_CANFDRxMessage */
        OCI_CANFDRX_MESSAGE                        = 8,

        /** Message type is @ref OCI_CANFDTxMessage */
        OCI_CANFDTX_MESSAGE                        = 9
...
#define OCI_CAN_MSG_FLAG_SELFRECEPTION          (0x4u)
    ...
    *
    * @note The structure elements and memory layout are guaranteed to be identical to @ref OCI_CANFDTxMessage, except
    ...
    *
    * @note The structure elements and memory layout are guaranteed to be identical to @ref OCI_CANFDRxMessage,
    * except for the size of the data[] array and the absence of the "raw" (i.e. un-decoded) DLC field
    * @ref OCI_CANFDRxMessage::dlc.
    * The fields @ref OCI_CANFDRxMessage::size and @ref OCI_CANRxMessage::dlc have equivalent semantics, despite their
    * different names.
    */
    typedef struct OCI_CANRxMessage
    {
        ...
        *
        * Note that the CAN-FD flags @ref OCI_CAN_MSG_FLAG_FD_DATA and @ref OCI_CAN_MSG_FLAG_FD_DATA_BIT_RATE may be set.
        * The presence of these flags indicates that the message is actually a "short" CAN-FD message which has been received
        * as an instance of @ref OCI_CANRxMessage because @ref OCI_CANFDRxConfig::canFdRxMode has the value
        * @ref OCI_CANFDRXMODE_CANFD_FRAMES_USING_CAN_MESSAGE. See @ref GROUP_OCI_CAN_FD for more details.
        *
...
#define OCI_CAN_ERR_TYPE_UNKNOWN                (0x80000000u)
    ...
    */
    ...
    /**
    * This union is exactly equivalent to @ref OCI_CANMessageData, but with additional support for CAN-FD message
    * types.
    */
    typedef
#ifdef MIDL_RUNNING
    [switch_type(OCI_CANMessageDataType)]
#endif
    union OCI_CANMessageDataEx
    {
        /**
        * Read Data of a CAN message.
        * Use this part, if @ref OCI_CANMessageEx.type is
        * @ref OCI_CAN_RX_MESSAGE.
        */
#ifdef MIDL_RUNNING
    [case(OCI_CAN_RX_MESSAGE)]
    ...
    [case(OCI_CAN_QUEUE_EVENT)]
        ...
        * Read Data of a CAN message.
        * Use this part, if @ref OCI_CANMessageEx.type is
        * @ref OCI_CANFDRX_MESSAGE.
        */
#ifdef MIDL_RUNNING
    [case(OCI_CANFDRX_MESSAGE)]
#endif
            OCI_CANFDRxMessage canFDRxMessage;

        /**
        * Write data of a CAN message.
        * Use this part, if @ref OCI_CANMessageEx.type is
        * @ref OCI_CANFDTX_MESSAGE.
        */
#ifdef MIDL_RUNNING
    [case(OCI_CANFDTX_MESSAGE)]
#endif
            OCI_CANFDTxMessage canFDTxMessage;

    } OCI_CANMessageDataEx;

    /**
    * Shared structure of a CAN bus message/event.
    */
    ...
    /**
    * This struct is exactly equivalent to @ref OCI_CANMessage, but with additional support for CAN-FD message
    * types. This struct must be used whenever it is necessary to send an instance of @ref OCI_CANFDTxMessage
    * or receive an instance of @ref OCI_CANFDRxMessage.
    */

    typedef struct OCI_CANMessageEx
    {
        /** Discriminator of the @a OCI_CANMessageDataEx union. */
        OCI_CANMessageDataType type;

        /** reserved parameter necessary for 64 Bit alignment of OCI_CANMessageDataEx */
        uint32 reserved;
    ...
    */
    ...
    /**
      Queue Data for transmission on a CAN Bus Interface using standard CAN (non-CAN-FD) messages only.
      ...
      <TABLE border="0" rules="rows" cellspacing="3" cellpadding="3">
       ...
       <tr> <td> @ref OCI_ERR_INCONSISTENT_PARAMETER_SET </td>
            <td> At least one message has an invalid format (e.g. messages[i].OCI_CANMessageDataType != OCI_CAN_TX_MESSAGE).
                 Note that some implementations may only validate each message as it is transmitted, and that therefore
                 this error may only be detected after some previous messages in the array have been transmitted successfully.
                 Therefore, when receiving this error, a client should examine the value of @p remaining to determine
                 whether any messages were successfully transmitted.
                 </td>
       </tr>
       <tr> <td> @ref OCI_ERR_DRIVER_NO_RESPONSE </td>
            <td> The driver for a removable hardware received no response to a request. The hardware may be removed.
       ...
       <tr> <td> @ref OCI_ERR_QUEUE_IS_FULL </td>
            <td> There was not enough space in the specified queue to transmit all the provided messages. Zero or more
                 messages were added to the queue, but at least one message was not. The output parameter @remaining
                 indicates the number of messages which remain unsent. </td>
       </tr>
       <tr> <td> @ref OCI_ERR_UNEXPECTED_NULL </td>
            <td> A fatal internal error occured in queue handling. </td>
       </tr>
       <tr> <td> @ref OCI_ERR_INVALID_PARAMETER </td>
            <td> @a size is zero </td>
       </tr>
       </TABLE>
    ...
    */
    ...
    /**
      Queue Data for transmission on a CAN Bus Interface using any type of CAN message (CAN or CAN-FD).
      Note: To use CAN-FD messages the CAN Controller must have been configured with CAN-FD enabled (see
      @ref OCI_CANConfiguration::canFDEnabled).

      The routine returns, when all frames can be queued. Since the transmit queue has a limited capacity,
      this function may need to block until the last entry could be placed into the queue. A @ref timeout
      can be specified to limit the blocking.

      There is no information, when the frames are sent on the CAN bus. There may be a delay between storing the frames
      in the queue and sending them, because of other frames in the same queue and arbitration of the CAN-Bus. Some
      fatal error on the CAN-Bus (e.g. Controller Bus-Off) may occur before the frames can be send.
      ...
      @note: A low priority frame in front of a send queue on a busy CAN bus may lose arbitration process with other CAN
      controller and stall the transmission of higher priority frames written to the queue at a later time.

      @controllerState SUSPENDED, RUNNING. \n
                       When the controller is in the CREATED state no valid Queue can exist and the call will return
                       @ref OCI_ERR_INVALID_HANDLE.

      @version This function was added in v1.3 of OCI_CAN: do not attempt to call this function unless the current OCI_CAN
      session uses at least v1.3.
      ...
      @param[in]  timeout   Maximum time (delta, not global time stamp) to wait for messages to be queued.
                            ...
                            Use @ref OCI_NO_TIME to wait indefinitely if necessary.
                            0 will queue as many messages as possible without waiting for them to be transmitted.
      @param[in]  messages  A pointer to an array of @a size @ref OCI_CANMessageEx pointers.
      @param[in]  size      The number of entries in the @a messages array.
      @param[out] remaining Number of entries not processed. Will be 0 on success. Will be smaller or equal to @a size,
                            when an error or timeout occured.
                            May be @c NULL.
    ...
    */
...
#ifndef OCI_DYNAMICDLL
    OCI_DECLSPEC OCI_ErrorCode
    OCI_CALL OCI_WriteCANDataEx(
        OCI_QueueHandle queue,
        OCI_Time timeout,
        OCI_CANMessageEx* messages[],
        uint32 size,
        uint32* remaining);
#endif

    typedef OCI_ErrorCode
    (OCI_CALL* PF_OCI_WriteCANDataEx)(
        OCI_QueueHandle queue,
        OCI_Time timeout,
        OCI_CANMessageEx* messages[],
        uint32 size,
    ...
    /**
      ...
      @deprecated As of OCI_CAN v1.3 this function has been deprecated in favour of the "extended" version
      @ref OCI_ReadCANDataEx. OCI_CAN v1.3 introduces new message types which cannot be used with
      @ref OCI_ReadCANData, and its use is therefore not allowed for sessions bound to OCI_CAN v1.3 (or later).
      ...
      <TABLE border="0" rules="rows" cellspacing="3" cellpadding="3">
       ...
       <tr> <td> @ref OCI_ERR_NOT_IMPLEMENTED </td>
            <td> The current OCI_CAN session uses v1.3 or later of the OCI_CAN API. This function is not supported
                 for such API versions. Please use @ref OCI_ReadCANDataEx() instead.

                 Background: OCI_CAN v1.3 allows clients to receive CAN-FD messages. Such messages are not supported
                 by @ref OCI_ReadCANData().
                 </td>
       </tr>
    ...
    */
    ...
    /**
      Read any type of CAN message (CAN or CAN-FD) from a receive queue or wait for a timeout when less than requested entries are available.
      Note: To use CAN-FD messages the CAN Controller must have been configured with CAN-FD enabled (see OCI_CANConfiguration::canFDEnabled).
      ...
      @param[in]  timeout   Maximum time (delta, not global time stamp) to wait at the queue to receive @a size entries.
                            ...
                            Use @ref OCI_NO_TIME to wait indefinitely if necessary.
                            0 will fetch the queue's current content.
      @param[in]  messages  A pointer to an array of pointers that can receive @a size @ref OCI_CANMessageEx elements.
                            Must not be @c NULL.
      @param[in]  size      Number of entries allocated in the @a messages array.
                            Must not be 0.
      @param[out] count     Number of entries actually read with 0 <= @a count <= @a size.
