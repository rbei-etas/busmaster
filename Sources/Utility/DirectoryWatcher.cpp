...
CDirectoryWatcher::~CDirectoryWatcher()
{
    ...
    {
        /*std::list<IDirectoryUpdate*>& lstpDirUpdate = itr->second;
        nLstSize = itr->second.size();
        for(std::list<IDirectoryUpdate*>::iterator itrlst = lstpDirUpdate.begin();
            nLstSize>0;itrlst++)
        {
            IDirectoryUpdate* ptr = *itrlst;
            RemoveDirectoryWatch(itr->first,ptr);
            nLstSize = itr->second.size();
        }*/
