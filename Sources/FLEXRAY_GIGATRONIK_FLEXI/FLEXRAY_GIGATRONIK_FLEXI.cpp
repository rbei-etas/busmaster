...
HRESULT CDIL_FLEXRAY_GIGATRONIK_FLEXI::FLEXRAY_LoadDriverLibrary(void)
{
    ...
    if(hResult == S_OK)
    {
        ...
        for(uint16 i = 1; i < available_versions.size(); i++)
        {
            if(available_versions[i].VersionsNr[0] >= temp_version.VersionsNr[0] &&
                available_versions[i].VersionsNr[1] >= temp_version.VersionsNr[1] &&
                available_versions[i].VersionsNr[2] > temp_version.VersionsNr[2])
            {
    ...
    /* 4: complete the registry entry of the newest driver and find the directory of the newest driver */
    if(hResult == S_OK)
    {
        BYTE acGCCPath[1024];
        HKEY sKey;
    ...
    /* 5: check if a GIGABOX flex-i driver got find */
    if(hResult == S_OK)
    {
    ...
    /* 6: check if there is a GIGABOX flex-i driver found */
