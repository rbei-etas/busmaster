...
#define defSTR_FLEXRAY_HW_DISPLAY_FORMAT    "FlexRay %d"
...
void CHardwareListing::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Text(pDX, IDC_EDIT_NET_ADDITIONALINFO, m_omAdditionalInfo);
    //}}AFX_DATA_MAP
}
...
void CHardwareListing::vSetHardwareList(INTERFACE_HW* /*psHwIntr*/, int nSize)
{
    ...
    for( ; index < nSize; index++)
    {
        ...
        if ( !bSelItem )
        {
            omStrFormat.Format( defSTR_FLEXRAY_HW_DISPLAY_FORMAT, index + 1);
...
void CHardwareListing::vUpdateHwDetails(int nIndex)
{
    if(nIndex < m_nSize )
    {
        ...
        m_omAdditionalInfo = m_psHwInterface[nIndex].m_acAdditionalInfo.c_str();
    }
}
