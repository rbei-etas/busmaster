...
int LDFAdditionalView::DisplayScedulingTableAdditionalProps(IScheduleTable* pouTable, int nItemIndex )
{
    ...
    auto itrTableItem = ouTableProps.m_ouCSheduleTableItem.begin();
    std::advance(itrTableItem, nItemIndex);

    if ( ouTableProps.m_ouCSheduleTableItem.end() != itrTableItem )
    {
        DisplayScheduleTableFrameProps(*itrTableItem);
    }
}
int LDFAdditionalView::DisplayScheduleTableFrameProps(CSheduleTableItem& ouTableItem)
{
...
int LDFAdditionalView::DisplayDiagProps(CSheduleTableItem& ouTableItem)
{
    QList<QString> strColumnList;
...
int LDFAdditionalView::DisplayAssignFrameIdProps(CSheduleTableItem& ouTableItem)
{
    return 0;
}
...
void LDFAdditionalView::onConfigFrameEditChange(const QString& strText )
{
    if(currentRow() >= 0 && m_pCurrentEcu != nullptr)
    {
        ...
        item(currentRow(), 0)->setData(Qt::UserRole, qVar);
