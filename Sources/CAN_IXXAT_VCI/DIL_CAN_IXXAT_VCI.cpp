...
HRESULT CDIL_CAN_IXXAT_VCI::CAN_SelectHwInterface(const INTERFACE_HW_LIST& sSelHwInterface, INT nCount)
{
  ...
  if (m_iNumberOfCANChannelsTotal > 0)
  {
    ...
    LogMessage( TRUE, _T("------> Selected CAN controller: %s - %s (%s)\n")
                    , sSelHwInterface[0].m_acDeviceName
                    , sSelHwInterface[0].m_acNameInterface
                    , sSelHwInterface[0].m_acDescription );
#endif
  }
  m_byCurrHardwareState = STATE_HW_INTERFACE_LISTED;
 ...
 *
 */
HRESULT CDIL_CAN_IXXAT_VCI::CAN_DisplayConfigDlg(PCHAR& InitData, int& Length)
{
  ...
  if (m_iNumberOfCANChannelsTotal > 0)
  {
    ...
    SCONTROLER_DETAILS* pCtrlArray = (SCONTROLER_DETAILS*)InitData;
    ...
    {
      strcpy_s(pCtrlArray[0].m_omStrBaudrate, MAX_STRING, CANControllerConfigDlg.GetBitTimingName().c_str());
 ...
 *  is a pointer to an array with the struct SCONTROLER_DETAILS.
 * @param Length [in]
 ...
 * 	Always S_OK.
 */
HRESULT CDIL_CAN_IXXAT_VCI::CAN_SetConfigData(PCHAR pInitData, int Length)
{
#ifdef _IXXAT_DEBUG
  ...
  // int iNumOfCtrlEntries = Length / sizeof(SCONTROLER_DETAILS); -> no need this time
  SCONTROLER_DETAILS* pCtrlArray = (SCONTROLER_DETAILS*)pInitData;
  ...
  {		
    ...
    SCONTROLER_DETAILS* pCtrlDetails = (SCONTROLER_DETAILS*) &pCtrlArray[iCount];
    strcpy_s(pCtrlDetails->m_omHardwareDesc, MAX_STRING, _T("VCI CAN Interface"));
