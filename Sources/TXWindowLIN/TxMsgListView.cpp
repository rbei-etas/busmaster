...
////DIL Interface
static CBaseDIL_LIN* g_pouDIL_LIN_Interface = nullptr;
...
BEGIN_MESSAGE_MAP(CTxMsgListView, CFormView)
    ...
    ON_COMMAND(IDM_SEND_SLEEP_FRAME_MSG, OnSendSleepFrameMsg)
...
void CTxMsgListView::vSetMessageCheckValue(BOOL bCheck)
{
            ...
            pomFunctionsView != nullptr )
    {
        ...
        if(pomBlocksView->m_nSelectedMsgBlockIndex != -1 )
        {
            ...
            PSTXLINMSGLIST psMsgList = psCurrentMsgBlock->m_psTxLINMsgList;
            int nIndex = 0;
            ...
            while(psMsgList != nullptr )
            {
                if( psMsgList->m_sTxMsgDetails.m_bEnabled != bCheck )
                {
                    ...
                    /* Update to DIL if in connect state */
                    if ( !bCheck && CTxMsgManager::s_TxFlags.nGetFlagStatus(TX_CONNECTED) && g_pouDIL_LIN_Interface )
                    {
                        g_pouDIL_LIN_Interface->DILL_SetSlaveRespData(psMsgList->m_sTxMsgDetails.m_sTxMsg);
                    }
        ...
        if( pNMListView->uChanged  == LVIF_STATE )
        {
            ...
            if(unCurrentState != FALSE)
            {
                ...
                if(psTxMsgList != nullptr )
                {
                    ...
                    /* Update to DIL if in connect state */
                    if ( CTxMsgManager::s_TxFlags.nGetFlagStatus(TX_CONNECTED) && g_pouDIL_LIN_Interface )
                    {
                        psTxMsgList->m_sTxMsgDetails.m_sTxMsg.m_ucMsgTyp = LIN_SLAVE_RESPONSE;
                        g_pouDIL_LIN_Interface->DILL_SetSlaveRespData(psTxMsgList->m_sTxMsgDetails.m_sTxMsg);
                    }
            ...
            if(psTxMsgList != nullptr )
            {
                ...
                if( pView != nullptr )
                {
                            ...
                            psTxMsgList->m_sTxMsgDetails.m_bEnabled )
                    {
                        ...
                        /* Update to DIL if in connect state */
                        if ( !nCurrentState && CTxMsgManager::s_TxFlags.nGetFlagStatus(TX_CONNECTED) && g_pouDIL_LIN_Interface )
                        {
                            psTxMsgList->m_sTxMsgDetails.m_sTxMsg.m_ucMsgTyp = LIN_INVALID_RESPONSE;
                            g_pouDIL_LIN_Interface->DILL_SetSlaveRespData(psTxMsgList->m_sTxMsgDetails.m_sTxMsg);
                        }
...
/*  Modification on  :                                                        */
/******************************************************************************/
PSTXLINMSGLIST CTxMsgListView::psGetMsgDetailPointer(INT nIndex,
        SMSGBLOCKLIST* psCurrentMsgBlockList)
{
    ...
    {
        INT nCurrentIndex = 0;
        psTxMsgList = psCurrentMsgBlockList->m_psTxLINMsgList;
...
{
    PSTXSELMSGDATA psTxLinMsg = nullptr;
    ...
    if( pomBlocksView != nullptr )
    {
        ...
        if(unTotalSelection > 0 )
        {
            ...
            psTxLinMsg  = new STXSELMSGDATA;
            if(psTxLinMsg != nullptr )
            {
                // Fill the structure and create memory for array of message
                // frame to be transmitted.
                psTxLinMsg->m_unCount = unTotalSelection;
                psTxLinMsg->m_psTxMsgLin = new STLIN_MSG[unTotalSelection];
                if(psTxLinMsg->m_psTxMsgLin != nullptr )
                {
                    ...
                    if(psMsgBlock != nullptr )
                    {
                        ...
                        for(UINT i =0 ; i<unTotalSelection; i++)
                        {
                            ...
                            if(nCurrentIndex != -1 )
                            {
                                ...
                                if(psTxMsgList != nullptr )
                                {
                                    memcpy( &(psTxLinMsg->m_psTxMsgLin[i]),
                                            &(psTxMsgList->m_sTxMsgDetails.m_sTxMsg),
                                            sizeof(STLIN_MSG) );
                                }
                                else
                                {
                                    psTxLinMsg->m_psTxMsgLin[i].m_ucMsgID =
                                        static_cast <UINT> (-1);
                                }
                            }
                            else
...
/******************************************************************************/
/*  Function Name    :  OnSendSleepFrameMsg                                   */
/*  Input(s)         :                                                        */
/*  Output           :                                                        */
/*  Functionality    :  This function is called by frame work when user wants */
/*                      to Transmit selected message                          */
...
/*  Modification on  :                                                        */
/******************************************************************************/
void CTxMsgListView::OnSendSleepFrameMsg()
{
    DWORD dwClientID;
    STLIN_MSG sLinTxMsg;

    // Message Type (0 - Header / 1 - Response)
    sLinTxMsg.m_ucMsgTyp    = 0x01;
    // Checksum Type (0 - Classical / 1 - Enhanced)
    sLinTxMsg.m_ucChksumTyp = 0x00;
    // Protected Identifier
    sLinTxMsg.m_ucMsgID     = 0x3E;
    // Channel Number
    sLinTxMsg.m_ucChannel   = 0x00;
    // Data len (0..8)
    sLinTxMsg.m_ucDataLen   = 0x08;
    // Databytes 0..8
    sLinTxMsg.m_ucData[0] = 0x00;
    sLinTxMsg.m_ucData[1] = 0x11;
    sLinTxMsg.m_ucData[2] = 0x22;
    sLinTxMsg.m_ucData[3] = 0x33;
    sLinTxMsg.m_ucData[4] = 0x44;
    sLinTxMsg.m_ucData[5] = 0x55;
    sLinTxMsg.m_ucData[6] = 0x66;
    sLinTxMsg.m_ucData[7] = 0x77;
    // Checksum
    sLinTxMsg.m_ucChksum = 0x08;

    dwClientID = 0;

    //Send Sleep Frame
    g_pouDIL_LIN_Interface->DILL_SendMsg(dwClientID, sLinTxMsg);
}
...
BOOL CTxMsgListView::bDeleteMsgFromBlock(SMSGBLOCKLIST* psMsgCurrentBlock)
{
    ...
    {
        PSTXLINMSGLIST psDelTxLINMsgList = nullptr;
        PSTXLINMSGLIST psTxLINMsgList = nullptr;
        ...
        if (unSelectedCount > 0)
        {
            for (UINT i=0; i < unSelectedCount; i++)
            {
                ...
                if(nItem != -1)
                {
                    ...
                    // in between the list.
                    //  If psTxLINMsgList is null then the node is at the start
                    // and if the both are null it is invalid pointers.
                    if(psDelTxLINMsgList != nullptr && psTxLINMsgList != nullptr )
                    {
                        psTxLINMsgList->m_psNextMsgDetails =
                            psDelTxLINMsgList->m_psNextMsgDetails;
                        bReturn = TRUE;
                    }
                    else if(psDelTxLINMsgList != nullptr &&
                            psDelTxLINMsgList ==
                            psMsgCurrentBlock->m_psTxLINMsgList )
                    {
                        psMsgCurrentBlock->m_psTxLINMsgList =
                            psDelTxLINMsgList->m_psNextMsgDetails;
                        bReturn = TRUE;
                    ...
                    if(bReturn == TRUE)
                    {
                        ...
                        if ( CTxMsgManager::s_TxFlags.nGetFlagStatus(TX_CONNECTED) && g_pouDIL_LIN_Interface )
                        {
                            psDelTxLINMsgList->m_sTxMsgDetails.m_sTxMsg.m_ucMsgTyp = LIN_INVALID_RESPONSE;
                            g_pouDIL_LIN_Interface->DILL_SetSlaveRespData(psDelTxLINMsgList->m_sTxMsgDetails.m_sTxMsg);
                        }
                        delete psDelTxLINMsgList;

                        psDelTxLINMsgList = nullptr;
...
BOOL CTxMsgListView::bDeleteAllMsgFromBlock(SMSGBLOCKLIST* psMsgCurrentBlock)
{
    ...
    {
        PSTXLINMSGLIST psTxLINMsgList = nullptr;
        PSTXLINMSGLIST psTxLINMsgListDel = nullptr;
        psTxLINMsgList = psMsgCurrentBlock->m_psTxLINMsgList;
        while(psTxLINMsgList != nullptr )
        {
            psTxLINMsgListDel = psTxLINMsgList;

            /* Update to DIL if in connect state */
            if ( CTxMsgManager::s_TxFlags.nGetFlagStatus(TX_CONNECTED) && g_pouDIL_LIN_Interface )
            {
                psTxLINMsgListDel->m_sTxMsgDetails.m_sTxMsg.m_ucMsgTyp = LIN_INVALID_RESPONSE;
                g_pouDIL_LIN_Interface->DILL_SetSlaveRespData(psTxLINMsgListDel->m_sTxMsgDetails.m_sTxMsg);
            }

            psTxLINMsgList    = psTxLINMsgList->m_psNextMsgDetails;
            delete psTxLINMsgListDel;
            psTxLINMsgListDel = nullptr;


        }
        psMsgCurrentBlock->m_psTxLINMsgList = nullptr;
...
/*  Input(s)         :  sTXLINMSGDETAILS sMsgDetail: message details struct   */
        ...
        INT nCurrentIndex)
{
    ...
    CString omStrChksumTyp( STR_EMPTY );
    ...
    {
        omStrMsgID.Format(defFORMAT_MSGID_HEX,sMsgDetail.m_sTxMsg.m_ucMsgID);
        omStrFormat = defFORMAT_DATA_HEX;
    }
    else
    {
        omStrMsgID.Format( defFORMAT_MSGID_DECIMAL,
                           sMsgDetail.m_sTxMsg.m_ucMsgID );
    ...
    {
        omStrMsgName =
            pDBptr->omStrGetMessageNameFromMsgCode(sMsgDetail.m_sTxMsg.m_ucMsgID);
        //omStrUpdatedMsgLength = pDBptr->omStrGetMessageLengthFromMsgCode(sMsgDetail.m_sTxMsg.m_ucMsgID);
    }
    ...
    if(sMsgDetail.m_sTxMsg.m_ucMsgID <= 63)
    {
        omStrMsgType = "LIN SLAVE RESPONSE";
    }
    else
    {
        omStrMsgType = "LIN INVALID RESPONSE";
    }

    omStrChksumTyp = "CLASSIC CHECKSUM";
