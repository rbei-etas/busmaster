...
BEGIN_MESSAGE_MAP(CTxMsgDetailsView, CFormView)
    ...
    //ON_BN_CLICKED(IDC_BTN_SLEEP, OnBtnSendSleepFrame)
...
void CTxMsgDetailsView::OnEditchangeCombMsgIdName()
{
    ...
    //        m_omLctrSigList.EnableWindow( FALSE );
    //        // Set the message Length
    //        if( sLinInfo.m_ucRTR == FALSE)
    //        {
    //            m_odSignalMatrix.vSetMessageLength(sLinInfo.m_ucDataLen);
    //            // Set the data value
    //            m_odSignalMatrix.vSetByteValue( sLinInfo.m_ucData );
    //        }
    //        else
    //        {
    //            // For RTR Messages
...
/*  Input(s)         :  PSTLIN_MSG psMsgDetails:pointer to current msg details*/
...
/******************************************************************************/
BOOL CTxMsgDetailsView::bUpdateMessageDetail(STLIN_MSG* psMsgDetails)
{
    BOOL bReturn = TRUE;
    UpdateData(TRUE);
    vUpdateStateDataBytes();
    ...
    {
        g_pouDIL_LIN_Interface->DILL_SetSlaveRespData(*psMsgDetails);
    }
...
BOOL CTxMsgDetailsView::bUpdateSignalList(STLIN_MSG sMsg)
{
    ...
    if( podDatabase != nullptr )
    {
        ...
        m_psSelectedMsgDetails =
            podDatabase->psGetMessagePointer( sMsg.m_ucMsgID);
    ...
    if( pomBlockView != nullptr && pomListView != nullptr )
    {
        ...
        if(nIndex != -1)
        {
            ...
            else
            {
                ...
                //sLinInfo.m_ucMsgID = nMsgID; *** This is removed because of
...
//void CTxMsgDetailsView::OnBtnSendSleepFrame()
//{
//}
    ...
    if( pomBlockView != nullptr && pomListView != nullptr )
    {
        ...
        if(psMsgCurrentBlock != nullptr )
        {
            ...
            if(psTxMsgList != nullptr )
            {
                ...
                {
                    psMsgCurrentBlock->m_psTxLINMsgList = psTxMsgList;
 ...
 * message frame list.
 */
void CTxMsgDetailsView::vSetValues(STXLINMSGDETAILS* psTxMsg)
{
    CString omStr            = "";
    CString omFormat         = "";
    ...
    if ( psTxMsg != nullptr)
    {
        ...
        UINT unMsgID = psTxMsg->m_sTxMsg.m_ucMsgID;
