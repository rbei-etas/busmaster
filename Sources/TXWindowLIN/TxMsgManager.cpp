...
CRITICAL_SECTION CTxMsgManager::m_csUpdationLock;       //critical section used while copying PSTXLINMSGLIST from global to local
...
{
    g_pouDIL_LIN_Interface = (CBaseDIL_LIN*)ptrDILIntrf;
}
...
{
    return (void*)g_pouDIL_LIN_Interface;
}
...
VOID CTxMsgManager::vAssignMsgBlockList()
{
    ...
    while(psTxMsg != NULL && psMsgBlock != NULL )
    {
        ...
        if(psMsgBlock->m_unMsgCount > 0 )
        {
            psTxMsg->m_psTxLINMsgList = psMsgBlock->m_psTxLINMsgList;
...
UINT CTxMsgManager::s_unSendSelectedMsg(LPVOID pParam )
{
    ...
    if(psTxLinMsg != NULL)
    {
        ...
        {
            if (psTxLinMsg->m_psTxMsgLin[unIndex].m_ucMsgID != -1)
            {
                // Use HIL function to send CAN message
                int nReturn = g_pouDIL_LIN_Interface->DILL_SendMsg(g_dwClientID, psTxLinMsg->m_psTxMsgLin[unIndex]);
                if (nReturn != S_OK)
                {
                }
            }
    ...
    if (psTxMsg != NULL)
    {
        ...
        while ((psTxMsgList != NULL) && (bStopMsgBlockTx == FALSE))
        {
            ...
            if( dwWaitResult == WAIT_OBJECT_0 )
            {
                ...
                CTxWndDataStore::ouGetTxWndDataStoreObj().bCopyMsgList(psIntialTxMsgList, & psTxMsg->m_psTxLINMsgList); //reset the values
                    ...
                    (psTxMsgList->m_sTxMsgDetails.m_bEnabled == TRUE))
            {
                ...
                // Use HIL Function to send LIN Message
                //int nRet = g_pouDIL_LIN_Interface->DILL_SendMsg(g_dwClientID,
...
UINT CTxMsgManager::s_unSendMsgBlockOnKey(LPVOID pParam )
{
    ...
    if(psTxMsg != NULL )
    {
        ...
        //PSTXLINMSGLIST psTxMsgList = psTxMsg->m_psTxLINMsgList;
...
VOID CTxMsgManager::vSetSlaveMsgList()
{
    if(m_psTxMsgBlockList != NULL )
    {
                ...
                unCount < unMsgBlockCount)
        {
                    ...
                    //if (psTxMsgList->m_sTxMsgDetails.m_bEnabled != FALSE)
                    {
                                ...
                                nVarMsgCnt <= nMsgCnt)
                        {
                            ...
                            else if ( psTxMsgList!= NULL && (psTxMsgList->m_sTxMsgDetails.m_bEnabled == TRUE))
                            {
                                ...
                                int nRet = g_pouDIL_LIN_Interface->DILL_SetSlaveRespData(psTxMsgList->m_sTxMsgDetails.m_sTxMsg);
                            }
                            // Get Next Node in the list
                            psTxMsgList = psTxMsgList->m_psNextMsgDetails;
                        }
            ...
            {
                if (psMsgBlock->m_unMsgCount == 0 && unCount < unMsgBlockCount)
                {
                    unCount++;
                }
                else
