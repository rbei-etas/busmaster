...
BOOL CSimSysTreeView::bPopulateTree()
{
    ...
    HTREEITEM hRootItem = om_tree.InsertItem( CGlobalObj::omGetBusName(m_eBus),0,0,TVI_ROOT,TVI_LAST);
    ...
    if (pSimSysNodeInfo != nullptr)
    {
        ...
        while(pNodeInfo != nullptr)
        {
            ...
            CNodeDetailsDlg odNodeDlg(m_eBus, &pNodeInfo->m_sNodeInfo , nullptr);
...
void CSimSysTreeView::OnLButtonDown(UINT nFlags, CPoint point)
{
    ...
    if((hSelItem != nullptr) && (TVHT_ONITEMSTATEICON & uFlags))
    {
        ...
        if(CExecuteManager::ouGetExecuteManager(m_eBus).m_eBusStatus == BUS_DISCONNECTED)
        {
...
void CSimSysTreeView::vDisplayRootMenu()
{
    ...
    if(m_pomSubMenu != nullptr)
    {
        ...
        if(om_Tree.GetCount() > 1 && CExecuteManager::ouGetExecuteManager(m_eBus).m_eBusStatus == BUS_DISCONNECTED)
        {
...
void CSimSysTreeView::OnRemoveNode()
{
    ...
    if(pSimSysNodeInfo != nullptr)
    {
        if( m_hSelectedTreeItem == omTree.GetRootItem() )
        {
            int nReturnVal = AfxMessageBox(ALL_NODE_DELETE_CONFMN, MB_YESNO|MB_ICONQUESTION);
            if (nReturnVal == IDYES)
            {
...
void CSimSysTreeView::OnConfigNode()
{
    ...
    {
        CNodeDetailsDlg odNodeDlg(m_eBus, psNodeInfo , nullptr);
...
void CSimSysTreeView::OnAddNode()
{
    ...
    if (psNodeInfo != nullptr)
    {
        ...
        if ( odNodeDlg.DoModal() == IDOK )
        {
            ...
            CGlobalObj::ouGetObj(m_eBus).RegisterNodeToDIL(TRUE, psNodeInfo);
            ...
            {
                psNodeInfo->m_eNodeState = NODE_NOT_BUILT;
                //CSimSysManager::ouGetSimSysManager(m_eBus).GenerateMakeFile(psNodeInfo->m_omStrCFileName.GetBuffer(0));
            }
...
void CSimSysTreeView::OnEditNode()
{
    ...
    if(psNodeInfo != nullptr)
    {
        if(psNodeInfo->m_omStrCFileName.IsEmpty() && CExecuteManager::ouGetExecuteManager(m_eBus).m_eBusStatus == BUS_DISCONNECTED)
        {
        ...
        {
            CFunctionEditorDoc* pDoc = CGlobalObj::ouGetObj(m_eBus).pGetDocPtrOfFile(psNodeInfo->m_omStrCFileName);
            if (pDoc != nullptr)
            {
            ...
            else
            {
                ...
                if ( !CGlobalObj::ouGetObj(m_eBus).bOpenFunctioneditorfile(psNodeInfo->m_omStrCFileName) )
                {
...
void CSimSysTreeView::OnEnableDisableNode()
{
    PSNODEINFO psNodeInfo = GetNodeInfo(m_hSelectedTreeItem);
    CTreeCtrl& omTree = GetTreeCtrl();
    if(omTree.GetItemText(m_hSelectedTreeItem).CompareNoCase(CGlobalObj::omGetBusName(m_eBus)) == 0)
    {
        CSimSysNodeInfo* pSimSysNodeInfo =
            CSimSysManager::ouGetSimSysManager(m_eBus).pomGetSimSysNodeInfo();
        if(pSimSysNodeInfo != nullptr)
...
void CSimSysTreeView::OnBuildNode()
{
    ...
    if(m_hSelectedTreeItem == omTree.GetRootItem())
    {
        ...
        if(pSimSysNodeInfo != nullptr)
        {
            ...
            while(pTempNode != nullptr)
            {
                CFunctionEditorDoc* pDoc = CGlobalObj::ouGetObj(m_eBus).pGetDocPtrOfFile(pTempNode->m_sNodeInfo.m_omStrCFileName);
                if (pDoc)
                {
                    pDoc->OnSaveDocument(pTempNode->m_sNodeInfo.m_omStrCFileName);
                }
                pTempNode = pTempNode->m_psNextNode;
            }
        }
        ...
        //Build all nodes.
        CExecuteManager::ouGetExecuteManager(m_eBus).bDLLBuildAll();
    }
    else
    {
        PSNODEINFO psNodeInfo = GetNodeInfo(m_hSelectedTreeItem);
        ...
        if( psNodeInfo != nullptr )
        {
            ...
            if (pDoc)
            {
                pDoc->OnSaveDocument(psNodeInfo->m_omStrCFileName);
            }
            //Clear Output Window.
            CExecuteManager::ouGetExecuteManager(m_eBus).vClearOutputWnd();
            //Build the file
            CExecuteManager::ouGetExecuteManager(m_eBus).bExecuteDllBuild(psNodeInfo,TRUE);
        }
    }
    bPopulateTree();
}
...
{
    PSNODEINFO psNodeInfo = GetNodeInfo(m_hSelectedTreeItem);
    if(psNodeInfo == nullptr || CExecuteManager::ouGetExecuteManager(m_eBus).m_eBusStatus == BUS_CONNECTED)
    {
        pCmdUI->Enable(FALSE);
    }
    else
...
void CSimSysTreeView::vModifyToolbarIcon(BYTE bytItemIndex, bool bItemON, UINT nTBIDON, UINT nTBIDOFF)
{
    ...
    int Result = 0;
    m_hModAdvancedUILib = nullptr;
    CToolBarCtrl& ToolbarCtrl = CGlobalObj::ouGetObj(m_eBus).m_pomSimSysWnd->m_toolbar.GetToolBarCtrl();
    ...
    if ( m_hModAdvancedUILib )
    {
        ...
        if ( hIcon )
        {
            ...
            CGlobalObj::ouGetObj(m_eBus).m_pomSimSysWnd->m_toolbar.Invalidate();
        }
    }
...
PSNODEINFO CSimSysTreeView::GetNodeInfo(HTREEITEM hTreeItem)
{
    ...
    if(pSimSysNodeInfo != nullptr)
    {
        ...
        if(omstrSelectedItemText.CompareNoCase(CGlobalObj::omGetBusName(m_eBus)) != 0)
        {
