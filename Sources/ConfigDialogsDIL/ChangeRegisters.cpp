...
void CChangeRegisters::vChangeListBoxValues(INT nflag) 
{
    ...
    if(nReturn != -1)
    {
         ...
         nColumnCount = m_omListCtrlBitTime.GetHeaderCtrl()->GetItemCount();
         // Get the row number selected.
         nItem        = m_omListCtrlBitTime.GetNextItem(-1, LVNI_SELECTED) ;     
...
//			if(theApp.m_bFromAutomation == FALSE)
    ...
    {
         nColumnCount = m_omListCtrlBitTime.GetHeaderCtrl()->GetItemCount();
         nItemCount   = m_omListCtrlBitTime.GetItemCount();
...
void CChangeRegisters::vFillControllerConfigDetails()
{
    ...
    m_sAccFilterInfo.m_ucACC_Code0 = static_cast<UCHAR> (lFromStrTCHAR_2_Long(
                    m_pControllerDetails[nIndex].m_omStrAccCodeByte1, 
                    &pcStopStr, defHEXADECIMAL));
    m_sAccFilterInfo.m_ucACC_Code1 = static_cast<UCHAR> (lFromStrTCHAR_2_Long(
                    m_pControllerDetails[nIndex].m_omStrAccCodeByte2, 
                    &pcStopStr, defHEXADECIMAL));
    m_sAccFilterInfo.m_ucACC_Code2 = static_cast<UCHAR> (lFromStrTCHAR_2_Long(
                    m_pControllerDetails[nIndex].m_omStrAccCodeByte3, 
                    &pcStopStr, defHEXADECIMAL));
    m_sAccFilterInfo.m_ucACC_Code3 = static_cast<UCHAR> (lFromStrTCHAR_2_Long(
                    m_pControllerDetails[nIndex].m_omStrAccCodeByte4, 
                    &pcStopStr, defHEXADECIMAL));

    m_sAccFilterInfo.m_ucACC_Mask0 = static_cast<UCHAR> (lFromStrTCHAR_2_Long(
                    m_pControllerDetails[nIndex].m_omStrAccMaskByte1, 
                    &pcStopStr, defHEXADECIMAL));
    m_sAccFilterInfo.m_ucACC_Mask1 = static_cast<UCHAR> (lFromStrTCHAR_2_Long(
                    m_pControllerDetails[nIndex].m_omStrAccMaskByte2, 
                    &pcStopStr, defHEXADECIMAL));
    m_sAccFilterInfo.m_ucACC_Mask2 = static_cast<UCHAR> (lFromStrTCHAR_2_Long(
                    m_pControllerDetails[nIndex].m_omStrAccMaskByte3, 
                    &pcStopStr, defHEXADECIMAL));
    m_sAccFilterInfo.m_ucACC_Mask3 = static_cast<UCHAR> (lFromStrTCHAR_2_Long(
                    m_pControllerDetails[nIndex].m_omStrAccMaskByte4, 
                    &pcStopStr, defHEXADECIMAL));

    m_sAccFilterInfo.m_ucACC_Filter_Type = 
            static_cast<UCHAR> (m_pControllerDetails[ nIndex ].m_bAccFilterMode );
...
void CChangeRegisters::vUpdateControllerDetails()
{
       ...
       && ( unWarningLimit <= unWarningLimtMax ) )
    {
        ...
        ucBtr0 = (UCHAR) lFromCString_2_Long(m_omStrEditBTR0, &pcStopStr, defHEXADECIMAL);
        ucBtr1 = (UCHAR) lFromCString_2_Long(m_omStrEditBTR1, &pcStopStr, defHEXADECIMAL);
								   ...
								   DWORD dEndMsgId)
{
	...
	unIndex++ )
    {
        ...
        _tcscpy(m_pControllerDetails[ unIndex ].m_omStrAccCodeByte4, omStrTempByte.GetBuffer(MAX_PATH));

		omStrTempByte.Format(_T("%02X"),(dTemp & ( dBeginMsgId >> nShift)));
        _tcscpy(m_pControllerDetails[ unIndex ].m_omStrAccCodeByte3, omStrTempByte.GetBuffer(MAX_PATH));

		omStrTempByte.Format(_T("%02X"),(dTemp & ( dBeginMsgId >> nShift * 2)));
        _tcscpy(m_pControllerDetails[ unIndex ].m_omStrAccCodeByte2, omStrTempByte.GetBuffer(MAX_PATH));

		omStrTempByte.Format(_T("%02X"),(dTemp & ( dBeginMsgId >> nShift * 3)));
        _tcscpy(m_pControllerDetails[ unIndex ].m_omStrAccCodeByte1, omStrTempByte.GetBuffer(MAX_PATH));

        // Create Mask
		omStrTempByte.Format(_T("%02X"),(dTemp & ( dEndMsgId )));
        _tcscpy(m_pControllerDetails[ unIndex ].m_omStrAccMaskByte4, omStrTempByte.GetBuffer(MAX_PATH));

		omStrTempByte.Format(_T("%02X"),(dTemp & ( dEndMsgId >> nShift)));
        _tcscpy(m_pControllerDetails[ unIndex ].m_omStrAccMaskByte3, omStrTempByte.GetBuffer(MAX_PATH));

		omStrTempByte.Format(_T("%02X"),(dTemp & ( dEndMsgId >> nShift * 2)));
        _tcscpy(m_pControllerDetails[ unIndex ].m_omStrAccMaskByte2, omStrTempByte.GetBuffer(MAX_PATH));

		omStrTempByte.Format(_T("%02X"),(dTemp & ( dEndMsgId >> nShift * 3)));
        _tcscpy(m_pControllerDetails[ unIndex ].m_omStrAccMaskByte1, omStrTempByte.GetBuffer(MAX_PATH));
								   ...
								   double &dEndMsgId)
{
	...
	if(m_pControllerDetails != NULL)
	{
        ...
        nAccCodeByte1 = lFromStrTCHAR_2_Long(m_pControllerDetails[0].m_omStrAccCodeByte1, &pcStopStr, defHEXADECIMAL);
        nAccCodeByte2 = lFromStrTCHAR_2_Long(m_pControllerDetails[0].m_omStrAccCodeByte2, &pcStopStr, defHEXADECIMAL);
        nAccCodeByte3 = lFromStrTCHAR_2_Long(m_pControllerDetails[0].m_omStrAccCodeByte3, &pcStopStr, defHEXADECIMAL);
        nAccCodeByte4 = lFromStrTCHAR_2_Long(m_pControllerDetails[0].m_omStrAccCodeByte4, &pcStopStr, defHEXADECIMAL);

        nMaskCodeByte1 = lFromStrTCHAR_2_Long(m_pControllerDetails[0].m_omStrAccMaskByte1, &pcStopStr, defHEXADECIMAL);
        nMaskCodeByte2 = lFromStrTCHAR_2_Long(m_pControllerDetails[0].m_omStrAccMaskByte2, &pcStopStr, defHEXADECIMAL);
        nMaskCodeByte3 = lFromStrTCHAR_2_Long(m_pControllerDetails[0].m_omStrAccMaskByte3, &pcStopStr, defHEXADECIMAL);
        nMaskCodeByte4 = lFromStrTCHAR_2_Long(m_pControllerDetails[0].m_omStrAccMaskByte4, &pcStopStr, defHEXADECIMAL);
