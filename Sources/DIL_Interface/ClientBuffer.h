...
public:
    ...
    int NumOfSEBuffers()
    {
        return m_BaseFLEXRAYBufFSEVector.size();
    }
...
protected:
    typedef std::vector<CBaseFLEXBufFSE*> BaseFLEXRAYBufFSEVector;
    BaseFLEXRAYBufFSEVector m_BaseFLEXRAYBufFSEVector;

    //DWORD m_dwUniqueBufferID; ///< Key value for the clients, never decement this variable

    BOOL MsgBufExist(CBaseFLEXBufFSE* pBufObj);
};