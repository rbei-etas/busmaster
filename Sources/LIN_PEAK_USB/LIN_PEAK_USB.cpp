...
static BOOL bLoadDataFromContr(ClusterConfig&  asDeviceConfig)
{
    ...
    {
        for( int nIndex = 0; nIndex < asDeviceConfig.m_nChannelsConfigured; nIndex++ )
        {
            CChannel& odChannel = sg_odHardwareNetwork.m_aodChannels[ nIndex ];

            // Baudrate in BTR0BTR1 format
            odChannel.m_usBaudRate = static_cast <USHORT>(asDeviceConfig.m_ouFlexChannelConfig[nIndex].m_ouLinParams.m_nBaudRate);
            odChannel.m_unBaudrate = static_cast <UINT>(asDeviceConfig.m_ouFlexChannelConfig[nIndex].m_ouLinParams.m_nBaudRate);

            // Set LIN mode
            odChannel.m_unLINMode = modSlave;
            if (asDeviceConfig.m_ouFlexChannelConfig[nIndex].m_ouLinParams.m_bIsMasterMode)
            {
                odChannel.m_unLINMode = modMaster;
            }

            odChannel.m_unLINVersion = LIN_VER_2_0;
            strLinVer = asDeviceConfig.m_ouFlexChannelConfig[nIndex].m_ouLinParams.m_strProtocolVersion;
            ...
            asDeviceConfig.m_ouFlexChannelConfig[nIndex].m_ouClusterInfo.GetFrames(ouFrameList);
...
static int nCreateMultipleHardwareNetwork(UINT unDefaultChannelCnt = 0)
{
    ...
    else if ( ListHardwareInterfaces(sg_hOwnerWnd, DRIVER_LIN_PEAK_USB, sg_IntrHw, sg_anSelectedItems, nHwCount) != 0 )
    {
...
HRESULT CDIL_LIN_PeakUSB::LIN_SelectHwInterface(const INTERFACE_HW_LIST& asSelHwInterface, INT /*nCount*/)
{
    ...
    //if (bGetDriverStatus())
    {
        ...
        //if (nCreateClient(LIN_MONITOR_NODE, NULL, sg_hClient) == defERR_OK)
        {
...
HRESULT CDIL_LIN_PeakUSB::SetConfigData(ClusterConfig& ouConfig)
{
    HRESULT hResult = S_FALSE;
...
HRESULT CDIL_LIN_PeakUSB::LIN_DisplayConfigDlg(PSCONTROLLER_DETAILS_LIN InitData, int& Length)
{
    VALIDATE_VALUE_RETURN_VAL(sg_bCurrState, STATE_HW_INTERFACE_SELECTED, ERR_IMPROPER_STATE);
...
HRESULT CDIL_LIN_PeakUSB::LIN_GetCurrStatus(s_STATUSMSG& StatusData)
{
    ...
    else if ( sg_bCurrState == STATE_CONNECTED)
    {
        if (odChannel.m_unLINMode == modMaster)
        {
            ...
            if ( itrData != odChannel.m_IdSlaveData.end() )     //Both Master and Slave Task
            {
                ...
                memcpy(msg.Data, itrData->second.ucData, sizeof(itrData->second.m_nLength));
...
HRESULT CDIL_LIN_PeakUSB::LIN_SetAppParams(HWND hWndOwner, Base_WrapperErrorLogger* pILog)
{
...
HRESULT CDIL_LIN_PeakUSB::LIN_FilterFrames(FILTER_TYPE /*FilterType*/, TYPE_CHANNEL /*Channel*/, UINT* /*punFrames*/, UINT /*nLength*/)
{
    return S_OK;
}
...
HRESULT CDIL_LIN_PeakUSB::LIN_GetControllerParams(LONG& lParam, UINT nChannel, ECONTR_PARAM eContrParam)
{
    ...
    if ((sg_bCurrState == STATE_HW_INTERFACE_SELECTED) || (sg_bCurrState == STATE_CONNECTED))
    {
        switch (eContrParam)
        {
            ...
            {
                int nConHwCnt = 0;
                if (nGetNoOfConnectedHardware(nConHwCnt) == defERR_OK)
                {
                    lParam = nConHwCnt;
                }
                else
            ...
            case CON_TEST:
            {
                ...
                {
                    if (nTestHardwareConnection(sg_hHardware[nChannel - 1]) == defERR_OK)
                    {
                        lResult = (LONG)1;
                    }
                    lParam = lResult;
...
HRESULT CDIL_LIN_PeakUSB::LIN_GetConfiguration(sCONTROLLERDETAILSLIN psControllerConfig[], INT& nSize)
{

    for ( int i = 0; i < sg_ucNoOfHardware; i++ )
    {
        psControllerConfig[i].m_nBaudRate =  sg_odHardwareNetwork.m_aodChannels[i].m_unBaudrate;
